...
//			}
			...
			if ( !packageName.contains(MetaHelper.prefixNonPackageDir)
				...
				try {
					CompilerManager.loadMetaobjectsFromPackage(
							packageCanonicalPath, packageName, metaobjectList,
							this);
				}
				catch (final RuntimeException e) {
					// e.printStackTrace();
				...
				final CyanPackage cyanPackage = createCyanPackage(program,
		...
		final CyanPackage cyanLangPackage = createCyanPackage(program, project,
		...
		if ( i < si.length() && Character.isUpperCase(si.charAt(i)) ) {
			...
			if ( j <= 0 ) {
				error(true, importPackage.getFirstSymbol(),
						"Package '" + importedPackageName + "' was not found",
...
//			}
		...
		default:
			if ( isBasicType(symbol.token) || symbol.token == Token.STRING
				...
				error2(symbol, "type expected." + foundSuch()
						+ ". Note that in the declaration of a field the order of keywords is fixed: [ private | public ] [ shared ] [ var | let ] Type Id ';'");
				return null;
			}
	...
	// , Compiler compiler
		 ...
		 *
		 * SymbolIdent symbolIdent = new SymbolIdent(Token.IDENT, prototypeName,
		 * sym.getStartLine(), sym.getLineNumber(), sym.getColumnNumber(),
		 * sym.getOffset() ); ExprIdentStar typeIdent = new
		 * ExprIdentStar(symbolIdent);
		 *
...
//				new HashSet<meta.CompilationInstruction>(), comp);
	...
	private static Compiler compilerFromString(String typeAsString) {
		...
		final CompilationUnitDSL dslCompilationUnit = new CompilationUnitDSL("not a source code, this is a type whose text is '"
				+ typeAsString + "'", "no path, this is a type as string", null);
			...
			case SHARED:
				...
				if ( isBasicType(symbol.token) || symbol.token == Token.STRING
					...
					if ( symbol.token != Token.VAR
						...
						this.error2(symbol, "'var', 'let', or a type expected."
								+ foundSuch()
								+ ". Note that in the declaration of a field "
								+ "the order of keywords is fixed: [ private | public ] [ shared ] [ var | let ] Type Id ';'");
					}
					else {
						if ( visibility == null ) visibility = Token.PRIVATE;
						if ( symbol.token == Token.VAR ) {
...
//						|| !this.cyanMetaobjectContextStack.isEmpty(),
		...
		if ( currentMethod.getNameWithoutParamNumber()
			...
			if ( currentMethod.getIsFinal()
				...
				error2(ms.getFirstSymbol(),
						"'initOnce' methods cannot be 'final', 'abstract', or indexing method");
			if ( ms.getReturnTypeExpr() != null ) {
				error2(ms.getReturnTypeExpr().getFirstSymbol(),
						"'initOnce' methods cannot declare return value type, even if it is 'Nil'");
			}
			initOnce = true;
		}
	...
	private void checkInitNewMethods(MethodSignature ms) {
		...
		if ( isInit || isInitOnce || name.equals("new")
			...
			if ( this.currentMethod.getHasOverride() ) error(true, symbol,
					"Methods 'init', 'init:', and 'initOnce' cannot be declared with 'override'",
					"init",
					ErrorKind.init_should_not_be_declared_with_override);
			if ( currentMethod.isAbstract() ) error(true, symbol,
					"Methods 'init', 'init:', and 'initOnce' cannot be declared 'abstract'",
			...
			if ( !isInit && !isInitOnce && !this.parsingPackageInterfaces ) {
	...
	private StatementList initOnceBody() {
		...
		while (symbol.token == Token.IDENT || symbol.token == Token.SELF) {
			...
			if ( symbol.token != Token.ASSIGN ) error2(symbol,
					"'=' expected. 'initOnce' methods can have only assignments"
							+ foundSuch());
			next();
	...
	public StatementList statementList() {
		...
		if ( symbol.token != Token.RIGHTCB && symbol.token != Token.PUBLIC
				&& symbol.token != Token.PRIVATE
				&& symbol.token != Token.PROTECTED ) {
			boolean pushedFirstSymbolStatement = false;
			try {
//				lexer.inStatement = false;
//				if ( this.compilationStep == CompilationStep.step_1 &&
//						cyanMetaobjectContextStack.isEmpty()
//						&& !insideCyanMetaobjectCompilationContextPushAnnotation
//						&& symbol.token != Token.SEMICOLON && symbol.token != Token.RIGHTCB ) {
//					lexer.inStatement = true;
//					++lexer.numberNestedStatements;
//					pushedFirstSymbolStatement = true;
//		            lexer.firstStatementSymbolStack.push(symbol);
//				}
...
//					}
					lastStatement.setNextSymbol(symbol);
					/*
					 * if ( ! thereWasCompilationError && symbol.token !=
					 * Token.SEMICOLON && symbol.token != Token.RIGHTCB ) {
					 * previousSym = previousSymbol(); this.warning(previousSym,
					 * "; expected");
					 *
					 * if ( ask(symbol, "Should I insert ';' after " +
					 * previousSym.getSymbolString() + " ? (y, n)") )
					 * compilationUnit.addAction( new ActionInsert(";",
					 * compilationUnit, previousSym.getOffset() +
					 * previousSym.getSymbolString().length(),
					 * previousSym.getLineNumber(),
					 * previousSym.getColumnNumber())); }
					 */
				}
	...
	public Statement statement() {
		...
		case BREAK:
			if ( whileForCount <= 0 ) {
				try {
					this.error2(symbol,
							"'break' outside any 'while' or 'for' command");
				}
				catch (final error.CompileErrorException e) {
					if ( !(this.compilationUnitSuper instanceof CompilationUnit) ) {
						throw e;
		...
		case IF:
			ret = ifStatement();
			/*
			 * if ( (sif.getFirstSymbol().getColumnNumber() ==
			 * sif.getRightCBEndsIf().getColumnNumber()) ||
			 * (sif.getFirstSymbol().getLineNumber() ==
			 * sif.getRightCBEndsIf().getLineNumber()) || (sif.getLastElse() !=
			 * null && (sif.getLastElse().getColumnNumber() ==
			 * sif.getRightCBEndsIf().getColumnNumber() ||
			 * sif.getLastElse().getLineNumber() ==
			 * sif.getRightCBEndsIf().getLineNumber() ) ) ) { // if (
			 * this.compilationStep == CompilationStep.step_1 )
			 * sif.setShouldBeFollowedBySemicolon(false); } else { if (
			 * this.project.getCompilerManager().getCompilationStep() ==
			 * CompilationStep.step_1 ) error2(sif.getRightCBEndsIf(),
			 * "The '}' that closes an 'if' statement should be either in the same line as the 'if' or in the same column"
			 * ); else sif.setShouldBeFollowedBySemicolon(false); }
			 */
			break;
		case WHILE:
				...
				if ( symbol.token == Token.BACKQUOTE ) {
					if ( numUnarySends != 0 ) {
						if ( numBackquotes > 0 )
							...
							// receiver s `p
							error(true, symbol,
									"Illegal use of backquote. Unary message chain should have only one unary message when character ` is used",
									symbol.getSymbolString(),
...
//					}
			...
			while (symbol.token == Token.IDENTCOLON
				...
				if ( symbol.token == Token.BACKQUOTE ) {
					...
					if ( symbol.token != Token.IDENTCOLON ) error2(symbol,
							"A message keyword like 'add:' was expected after '`' (backquote)."
									+ foundSuch());
				}
	...
	public MethodSignature methodSignature() {
		...
		case IDENT:
			...
			if ( name.equals("initOnce") ) isInitOnce = true;
	...
	public static boolean isOperator(Token t) {
		...
		return t == Token.AND || t == Token.BITAND || t == Token.BITNOT
				|| t == Token.BITOR || t == Token.BITXOR || t == Token.DIV
				...
				|| t == Token.NEQ || t == Token.NOT || t == Token.OR
	...
	public static boolean isBinaryOperator(Token t) {
		return t == Token.AND || t == Token.BITAND || t == Token.BITOR
