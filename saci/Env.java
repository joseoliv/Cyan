...
//			}
		...
		if ( this.currentCompilationUnit == null || this.currentPrototype == null ) {
			...
			while (true) {
				...
				if ( this.currentMethod == null ) {
					if ( this.searchField(name) == null ) break;
				}
				else {
					if ( this.searchLocalVariableParameter(name) == null && this.searchField(name) == null )
						break;
				}
			}
			return name;
			...
			if ( annotation instanceof AnnotationMacroCall ) {
				...
				if ( numLinesToDelete > 0 ) {
					changeList.add(new meta.SourceCodeChangeAddText(offsetStart,
							new StringBuffer(" @" + MetaHelper.markDeletedCodeName + "(" + numLinesToDelete + ") "), annotation));
	...
	*/
	 ...
	 * @return
	 */
	public boolean replaceStatementByCode(Statement stat, AnnotationAt annotation,
			StringBuffer code, Type codeType) {
	...
	public boolean removeAddCodeFromToOffset(Statement statement, int offsetNext,
			AnnotationAt annotation, StringBuffer codeToAdd, Type codeType) {
	 ...
	 *
	 */
	private boolean removeCodeExpr(INextSymbol expr, CompilationUnitSuper compilationUnit,
	...
	public void errorInMetaobjectCatchExceptions(CyanMetaobject cyanMetaobject) {
		...
		if ( errorList != null ) {
			for (final CyanMetaobjectError moError : errorList) {
				try {
					error( meta.GetHiddenItem.getHiddenSymbol(moError.getSymbol()),
						   moError.getMessage());
				}
	...
	public void errorInMetaobject(CyanMetaobject cyanMetaobject, Symbol symbolWithError) {
		...
		if ( errorList != null ) {
			for (final CyanMetaobjectError moError : errorList) {
				try {
					if ( meta.GetHiddenItem.getHiddenCompilationUnitSuper(moError.getSymbol().getCompilationUnit())
							instanceof CompilationUnitDSL ) {
						error(symbolWithError, moError.getMessage());
					}
					else {
						error(moError.getSymbol() != null ?
								meta.GetHiddenItem.getHiddenSymbol(moError.getSymbol())
	...
	public void thrownException(Annotation annotation, Symbol firstSymbol, RuntimeException e) {
		...
		error(firstSymbol,
				"Metaobject annotation '" + annotation.getCyanMetaobject().getName() + "' at line number " + lineNumber
						...
						+ e.getClass().getName() + "'" + (e.getMessage() != null ? ". The exception message is '" + e.getMessage() + "'" : "") );
		...
		if ( currentPrototype != null && currentPrototype instanceof ObjectDec ) {
			...
			if ( count > 0 ) {
				...
				for (final FieldDec fieldDec : fieldDecList) {
					stringFieldList = stringFieldList + "\"" + fieldDec.getName()
							+ "\"";
					if ( --count > 0 ) stringFieldList = stringFieldList + ", ";
				}
				stringFieldList += " ] ";
	...
	public TypeJavaRef searchVisibleJavaClass(String className) {
		...
		if ( lessIndex >= 0 ) rawPrototypeName = className.substring(0, lessIndex);
		final int lastDot = rawPrototypeName.lastIndexOf('.');
		if ( lastDot >= 0 ) {
			/*
			 * there is a package preceding the prototype name
	...
	public Type searchPackagePrototype(String packagePrototypeName, Symbol symUsedInError) {
		...
		return Compiler.singleTypeFromString(packagePrototypeName, symUsedInError,
				"Prototype '" + packagePrototypeName + "' was not found", cunit,
				pu, this);
	}
	...
	public static String getCodeToAddWithContext(CyanMetaobject cyanMetaobject, String codeToAdd,
		...
		if ( strSlotList != null ) {
			strSlotList = changeNewLineToSpaces(strSlotList);
		}
		if ( codeType != null ) {
	...
	public Prototype getCyException() {
		if ( cyException == null ) {
			...
			if ( cyException == null ) this.error(null, "Prototype '" + MetaHelper.cyanLanguagePackageName + "."
	...
	public void setLocalVariableAsInitializedWith(StatementLocalVariableDec localVar, Expr expr) {
		...
		if ( size > 0 ) {
			...
			this.error(localVar.getFirstSymbol(), "Internal error: variable '" + localVar.getName() + "' was not found in stack stackLocalVarInfo");
		}
	}
	...
	public List<LocalVarInfo> getLocalVarInfoPreviousLevel() {
		...
		if ( size > 0 ) {
			for ( int i = size - 1; i >= 0; --i) {
				 final LocalVarInfo varInfo = this.stackLocalVarInfo.get(i);
				 if ( varInfo.lexicalLevel == this.lexicalLevel + 1 && varInfo.localVar.getLevel() <= this.lexicalLevel) {
					 if ( list == null ) {
						 list = new ArrayList<>();
					 }
					 list.add(varInfo);
	...
	public String getCyanRuntimeJarFile() {
		return this.project.getProgram().getCyanLangDir() + NameServer.fileSeparator +
				NameServer.cyanRuntime;
	}
	...
	public String javaCodeForCastException(Expr exprToCast, Type leftType) {
		return  "new _ExceptionCast( new CyString(\"Cannot cast expression '" + MetaLexer.escapeJavaString(exprToCast.asString()) +
	 ...
	 * to a prototype in phase AF_RES_TYPES (phase 3). If this stack is not empty and there is a
				...
				this.error(slotToCheck.getFirstSymbol(), "Annotation '" + t.f2 +
				    ...
				    "The field or method '" + slotToCheck.getNameAsInSourceCode() + "' is not among them. It should."
						);
			}
		}
	}
	...
	public Tuple5<FileError, char[], String, String, WrCyanPackage> readTextFileFromPackage(String fileName, String extension, String packageName, DirectoryKindPPP hiddenDirectory, int numParameters,
			List<String> realParamList) {
				return this.project.getCompilerManager().
	...
	public Tuple4<FileError, char[], String, String> readTextFileFromProgram(String fileName, String extension, DirectoryKindPPP hiddenDirectory, int numParameters, List<String> realParamList) {
		return this.project.getCompilerManager().readTextFileFromProject(fileName, extension, hiddenDirectory, numParameters, realParamList);
	}
	...
	public FileError writeTextFile(char[] charArray, String fileName, String prototypeFileName, String packageName, DirectoryKindPPP hiddenDirectory) {
		return this.project.getCompilerManager().writeTextFile(charArray, fileName, prototypeFileName, packageName, hiddenDirectory);
	}
	...
	public FileError writeTextFile(String str, String fileName, String prototypeFileName, String packageName, DirectoryKindPPP hiddenDirectory) {
		return this.project.getCompilerManager().writeTextFile(str, fileName, prototypeFileName, packageName, hiddenDirectory);
	}

	public String getPathFileHiddenDirectory(String prototypeFileName, String packageName, DirectoryKindPPP hiddenDirectory) {
		return this.project.getCompilerManager().getPathFileHiddenDirectory(prototypeFileName, packageName, hiddenDirectory);
	...
	public void pushCheckUsePossiblyNonInitializedPrototype(
			boolean checkUse) {
