...
//	}
	...
	*/
	...
	public boolean changeCheckProgram() {
		...
		/**
		 * for each compilation unit that should be changed by metaobjects or by adding "#AF_RES_TYPES" to metaobject annotations,
		...
		final HashMap<CompilationUnitSuper, List<Tuple2<Integer, Annotation>>> setOfSuffixToChange2 = new HashMap<>();
		   ...
		   * has been changed during the compiler phase "AF_RES_TYPES". Then these prototypes should be
		   * compiled again.
		   */
			...
			if ( pu == null ) {
				...
				if ( ! annotation.getInsideProjectFile() ) {
					...
					if ( puAnnotation == null )
						...
						/*
						 * the metaobject annotation will be changed to use the suffix "AF_RES_TYPES". Then all
		...
		for ( final Map.Entry<CompilationUnitSuper, List<Tuple2<Integer, Annotation>>>
			...
			for ( final Tuple2<Integer, Annotation> t : offsetList ) {
				changeList.add(new SourceCodeChangeShiftPhase(
						compUnitEntry.getText(), env, t.f1, CompilerPhase.AF_RES_TYPES, compUnitEntry,
						t.f2));
			}
		}
...
//				changeList.add(new SourceCodeChangeShiftPhase(compUnitEntry.getText(), env, offset, CompilerPhase.AF_RES_TYPES, compUnitEntry,
//						null));
//			}
//		}
	...
	*/
	...
	private final List<Tuple5<CyanMetaobject, String, String, String, String[]>> renameMethodList;
	  /*
	   * set with the compilation units that have been changed during the compiler phase "AF_RES_TYPES".
