...
public class CompilerManager_afti  {

	public CompilerManager_afti(Env env) {
		this.env = env;
		...
		codeToAddAtMetaobjectAnnotationList = new ArrayList<Tuple4<CyanMetaobjectAtAnnot, String, String, StringBuffer>>();
	...
	/*
	public List<IPackage_afti> getPackageList() {
		final List<IPackage_afti> ipackage_aftiList = new ArrayList<IPackage_afti>();
		for ( final CyanPackage cyanPackage : env.getProject().getPackageList() ) {
			final IPackage_afti ipackage = new WrPackage_afti(cyanPackage);
			ipackage_aftiList.add(ipackage);
		}
		return ipackage_aftiList;
	}
	*/
...
//	}
	...
	public boolean addMethod( CyanMetaobjectAtAnnot cyanMetaobject, String packageName, String prototypeName,
		...
		if ( previousCyanMetaobject == null ) {
			 ...
			 */
			final String packageName1 = cyanMetaobject.getMetaobjectAnnotation().getPackageOfAnnotation();
			final String prototypeName1 = cyanMetaobject.getMetaobjectAnnotation().getPrototypeOfAnnotation();

			final String packageName2 = previousCyanMetaobject.getMetaobjectAnnotation().getPackageOfAnnotation();
			final String prototypeName2 = previousCyanMetaobject.getMetaobjectAnnotation().getPrototypeOfAnnotation();

			error(meta.GetHiddenItem.getHiddenSymbol(cyanMetaobject.getMetaobjectAnnotation().getFirstSymbol()),
	...
	*/
	...
	public boolean renameMethods(CyanMetaobject cyanMetaobject, String packageName,
		...
		if ( previousCyanMetaobject == null ) {
			...
			error(meta.GetHiddenItem.getHiddenSymbol(cyanMetaobject.getMetaobjectAnnotation().getFirstSymbol()), "metaobject annotation of line " +
					cyanMetaobject.getMetaobjectAnnotation().getFirstSymbol().getLineNumber() + " of prototype " +
	 ...
	 */
	public boolean addCodeAtMetaobjectAnnotation(CyanMetaobjectAtAnnot cyanMetaobject, StringBuffer codeToAdd) {
		/*
		 * 			String cyanMetaobjectName = t.f2;
			...
			int lineNumber = t.f5;
		 */
		final Annotation cyanMetaobjectAnnotation =
				meta.GetHiddenItem.getHiddenCyanMetaobjectAnnotation(cyanMetaobject.getMetaobjectAnnotation());
		codeToAddAtMetaobjectAnnotationList.add(new Tuple4<CyanMetaobjectAtAnnot, String, String, StringBuffer>(
	...
	public boolean changeCheckProgram() {
		...
		/**
		 * for each compilation unit that should be changed by metaobjects or by adding "#afti" to metaobject annotations,
		 * there is associated list of code changes
		 */
		final HashMap<CompilationUnitSuper, List<SourceCodeChangeByMetaobjectAnnotation>> setOfChanges = new HashMap<>();
		/**
		...
		final HashMap<CompilationUnitSuper, List<Tuple2<Integer, Annotation>>> setOfSuffixToChange2 = new HashMap<>();
		   ...
		   * has been changed during the compiler phase "afti". Then these prototypes should be
		   * compiled again.
		   */
		...
		for ( final Tuple4<CyanMetaobjectAtAnnot, String, String, StringBuffer> t : codeToAddList ) {
			...
			if ( pu == null ) {
				...
				if ( ! annotation.getInsideProjectFile() ) {
					 ...
					 */
					final ProgramUnit puMetaobjectAnnotation = env.searchPackagePrototype(annotation.getPackageOfAnnotation(),
					...
					if ( puMetaobjectAnnotation == null )
						...
						/*
						 * the metaobject annotation will be changed to use the suffix "afti". Then all
						 * metaobject annotations should be collected
						 */
						addSuffixToChange2(setOfSuffixToChange2, puMetaobjectAnnotation.getCompilationUnit(), cyanMetaobject);
					}
		...
		for ( final Tuple5<CyanMetaobject, String, String, StringBuffer, String> t : codeToAddList2 ) {
			...
			if ( pu == null ) {
				...
				final String codeToAddWithContext = Env.getCodeToAddWithContext(cyanMetaobject,
						"\n" + codeToAdd + "\n", null, t.f5);
				final Annotation annotation = meta.GetHiddenItem.getHiddenCyanMetaobjectAnnotation(
						cyanMetaobject.getMetaobjectAnnotation());
				addChange(setOfChanges, compUnit,
						new SourceCodeChangeAddText(smallerOffset, new StringBuffer(codeToAddWithContext),
						annotation
		...
		for ( final Tuple6<CyanMetaobjectAtAnnot, String, String, String, String, String> t : fieldToAddList ) {
			...
			if ( pu == null ) {
				if ( cyanMetaobjectAnnotation.getInsideProjectFile() ) {
					...
					final String packageName1 = cyanMetaobjectAnnotation.getPackageOfAnnotation();
					final String prototypeName1 = cyanMetaobjectAnnotation.getPrototypeOfAnnotation();
					error(  cyanMetaobjectAnnotation.getFirstSymbol(), "metaobject annotation of line " +
				...
				if ( ! cyanMetaobjectAnnotation.getInsideProjectFile() )  {
					final ProgramUnit puMetaobjectAnnotation = env.searchPackagePrototype(cyanMetaobject.getMetaobjectAnnotation().getPackageOfAnnotation(),
							cyanMetaobject.getMetaobjectAnnotation().getPrototypeOfAnnotation());
					if ( puMetaobjectAnnotation == null )
						error(null, "Internal error: I cannot find a metaobject annotation of package " +
					            cyanMetaobject.getMetaobjectAnnotation().getPackageOfAnnotation() + " and prototype" +
		...
		for ( final Tuple5<CyanMetaobjectAtAnnot, String, String, String, StringBuffer> t : methodToAddList ) {
			...
			if ( pu == null ) {
				 ...
				 * add change to the list of changes
				 */
				// Annotation cyanMetaobjectAnnotation = cyanMetaobject.getMetaobjectAnnotation();
				final String codeToAddWithContext = Env.getCodeToAddWithContext(cyanMetaobject,
						"\n" + methodCode + "\n", null);
		...
		for ( final Tuple4<CyanMetaobjectAtAnnot, String, String, StringBuffer> t : codeToAddAtMetaobjectAnnotationList ) {
			final CyanMetaobjectAtAnnot cyanMetaobject = t.f1;
			final Annotation cyanMetaobjectAnnotation =
					meta.GetHiddenItem.getHiddenCyanMetaobjectAnnotation(cyanMetaobject.getMetaobjectAnnotation());
			...
			if ( pu == null ) {
				...
				if ( sameNameMethodList == null || sameNameMethodList.size() == 0 ) {
					...
					if ( sameNameMethodList.size() > 1 ) {
						...
						final String packageName1 = cyanMetaobject.getMetaobjectAnnotation().getPackageOfAnnotation();
						final String prototypeName1 = cyanMetaobject.getMetaobjectAnnotation().getPrototypeOfAnnotation();
						env.error(meta.GetHiddenItem.getHiddenSymbol(cyanMetaobject.getMetaobjectAnnotation().getFirstSymbol()),
								"metaobject annotation of line " +
								cyanMetaobject.getMetaobjectAnnotation().getFirstSymbol().getLineNumber() + " of prototype " +
								prototypeName1 + " of package " + packageName1 + " is trying to rename method " + oldMethodName
								+ " of prototype " +
		...
		for ( final Map.Entry<CompilationUnitSuper, List<Tuple2<Integer, Annotation>>>
			...
			List<SourceCodeChangeByMetaobjectAnnotation> changeList = setOfChanges.get(compUnitEntry);
			if ( changeList == null ) {
			...
			for ( final Tuple2<Integer, Annotation> t : offsetList ) {
				changeList.add(new SourceCodeChangeShiftPhase(
						compUnitEntry.getText(), env, t.f1, CompilerPhase.AFTI, compUnitEntry,
						t.f2));
			}
		}
...
//				changeList.add(new SourceCodeChangeShiftPhase(compUnitEntry.getText(), env, offset, CompilerPhase.AFTI, compUnitEntry,
//						null));
//			}
//		}
		...
		if ( changeSomething ) {
			Saci.makeChanges(setOfChanges, env, CompilationPhase.afti);
		}
	...
	static private void addChange(HashMap<CompilationUnitSuper, List<SourceCodeChangeByMetaobjectAnnotation>> setOfChanges,
			CompilationUnit compUnit, SourceCodeChangeByMetaobjectAnnotation change) {

		List<SourceCodeChangeByMetaobjectAnnotation> changeList = setOfChanges.get(compUnit);
		if ( changeList == null ) {
			changeList = new ArrayList<SourceCodeChangeByMetaobjectAnnotation>();
	...
	private static void addSuffixToChange(
		...
		final int newOffset = cyanMetaobject.getMetaobjectAnnotation().getFirstSymbol().getOffset();
	...
	private static void addSuffixToChange2(
        ...
        WrAnnotation wrannot = cyanMetaobject.getMetaobjectAnnotation();
        Annotation annot = meta.GetHiddenItem.getHiddenCyanMetaobjectAnnotation(wrannot);
        final int newOffset = cyanMetaobject.getMetaobjectAnnotation().getFirstSymbol().getOffset();
	...
	*/
	 ...
	 */
	private final List<Tuple4<CyanMetaobjectAtAnnot, String, String, StringBuffer>> codeToAddAtMetaobjectAnnotationList;
	...
	*/
	...
	private final List<Tuple5<CyanMetaobject, String, String, String, String[]>> renameMethodList;
	  /*
	   * set with the compilation units that have been changed during the compiler phase "afti".
