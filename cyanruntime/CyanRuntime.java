...
public class CyanRuntime {
    ...
    */
     ...
     */
    public static void addDynParameterInfo(String prototypeFullName, String sha256,
    		String methodName, String parameterName, int line, int column,
			int fileOffset, boolean wasTypeSupplied, String typeToAddToParameter) {
		DynamicTypeInfoPrototype ip =
				dynamicTypeInfoProgram.prototypeSet.get(prototypeFullName);
		if ( ip == null ) {
		...
		if ( par == null ) {
			par = new DynamicTypeInfoVarParamField(parameterName, line,
    ...
    public static void addDynLocalVariableInfo(String prototypeFullName, String sha256,
    		String methodName, String localVariableName, int line, int column,
			int fileOffset, boolean wasTypeSupplied, String typeToAddToParameter) {
		DynamicTypeInfoPrototype ip =
				dynamicTypeInfoProgram.prototypeSet.get(prototypeFullName);
		if ( ip == null ) {
		...
		DynamicTypeInfoVarParamField localVariable = im.localVariableSet.get(localVariableName);
		if ( localVariable == null ) {
			localVariable = new DynamicTypeInfoVarParamField(localVariableName, line,
					column, fileOffset, wasTypeSupplied);
    ...
    public static void addDynFieldInfo(String prototypeFullName, String sha256,
    		String fieldName, int line, int column,
			int fileOffset, boolean wasTypeSupplied, String typeToAddToParameter) {
		DynamicTypeInfoPrototype ip =
				dynamicTypeInfoProgram.prototypeSet.get(prototypeFullName);
		if ( ip == null ) {
		...
		DynamicTypeInfoVarParamField prototypeField = ip.fieldSet.get(fieldName);
 ...
 * 		try (FileWriter fosFileToAddTypeInfo = new FileWriter(
				...
				PrintWriter fileToAddTypeInfo = new PrintWriter(fosFileToAddTypeInfo)) {
			this.fileNameToAddTypeInfo = fileNameToAddTypeInfo;

		CyanRuntime.fileToAddTypeInfo = fileToAddTypeInfo;
 ...
 */
		...
		if ( size > 0 ) {
	 		try (FileWriter fosFileToAddTypeInfo = new FileWriter(
	 			...
	 			System.out.println("Error when creating or writing to the JSON "
	 					...
	 					+ "the program. This program is named '" + dynamicTypeInfoProgram.fileNameToAddTypeInfo +
	...
	/**
	 * taken from https://stackoverflow.com/questions/5531455/how-to-hash-some-string-with-sha256-in-java
	 */
    ...
    public static void catchException(Object[] catchList, ExceptionContainer__ t) {
