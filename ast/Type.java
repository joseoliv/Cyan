...
abstract public class Type implements ASTNode {
	...
	*/
	...
	public boolean isNonTaggedUnionType() {
		String name = getName();
		/*
		 * very bad way of doing things. Marked to be changed
		 */
		if ( name.startsWith("Union<") ) {
			...
			if ( indexOfComma < 0 ) {
				...
				if ( s.indexOf('.') >= 0) {
					// there is a type in the first position
					// the '.' separates package names
					return false;
				}
				else {
					  // if the first parameter is a symbol, it starts with a lower-case letter
