...
/**
 * This class keeps information on everything related to metaobjects and
 * supply methods used in several other classes
   @author José
...
public class MetaInfoServer {
	...
	/**
	 * replace the text of the expression 'iexpr' by 'sb'. The 'sb' string is the code of an
	...
	public static void checkMessageSendWithMethodMetaobject(
			List<MethodSignature> methodSignatureList, Type exprType,
			Expr expr, ExprReceiverKind receiverKind,
			MessageWithKeywords message,
							...
							try {
								...
								if ( other == null ) {
									to.run( Executors.callable(
											...
											timeoutMilliseconds, "afterSemAn_checkKeywordMessageSend",
											cyanMO, env);
//									((ICheckMessageSend_afterSemAn ) cyanMO).afterSemAn_checkKeywordMessageSend(
//											expr.getI(), puExpr, receiverKind,
//											message.getI(), ms.getI(), env.getI());
...
//											message.getI(), ms.getI(), env.getI());
							...
							if ( inSubprototype ) {
								...
								if ( mostSpecificMS.getMethod() != null ) {
									...
									mostSpecificReceiver = mostSpecificMS.getDeclaringInterface();
								}
								...
								try {
									...
									if ( other == null ) {
										to.run( Executors.callable(
												...
												timeoutMilliseconds, "afterSemAn_checkKeywordMessageSendMostSpecific",
												cyanMO, env);
...
//												mostSpecificReceiver.getI(), env.getI());
	...
	public static void checkMessageSendWithMethodMetaobject(
			List<MethodSignature> methodSignatureList, Type exprType,
			Expr expr, ExprReceiverKind receiverKind,  Env env, Symbol unarySymbol) {
		/*
						...
						if ( cyanMO instanceof ICheckMessageSend_afterSemAn
							...
							if ( inSubprototype ) {
								...
								if ( subMethodSignature.getMethod() == null ) {
									...
									mostSpecificReceiver = subMethodSignature.getMethod().getDeclaringObject();
								}
								...
								try {
									...
									if ( other == null ) {
										to.run( Executors.callable(
												...
												timeoutMilliseconds, "afterSemAn_checkUnaryMessageSendMostSpecific",
												cyanMO, env);
							...
							try {
								...
								if ( other == null ) {
									to.run( Executors.callable(
											...
											timeoutMilliseconds, "afterSemAn_checkUnaryMessageSend",
											cyanMO, env);
...
//											   new CyString(receiverKind.toString()), unarySymbol.getI(), env.getI());
	...
	/**
	 * This method do the checking demanded by metaobjects whose annotations are attached to types. This is
	 * the Cyan implementation of pluggable type systems. The left and right types correspond to the types
	 * of the expressions of an 'assignment':<br>
	 * <code> left = right </code><br>
	 * rightExpr is the expression being assigned. Here 'assignment' means any statement in which an expression
	 * may change it type and transfered elsewhere. For example, in an assignment, return value of method,
	 * parameter passing, object creation (a form of method call), creation of a literal array, etc.
	 */
	public static void checkAssignmentPluggableTypeSystem(Env env,
			...
			for ( final AnnotationAt annot : leftTypeWithAnnot.getAnnotationToTypeList() ) {
				...
				if ( cyanMetaobject instanceof IActionAttachedType_semAn
					...
					if ( other == null ) {
						...
						if ( sizeArray > 0 ) {
							...
							for (int i = 0; i < sizeArray; ++i) {
								_Tuple_LT_GP_CyString_GP_CyString_GT t = array._at_1(new CyInt(i));
								tupleArray.add( new Tuple2<String, String>(t._f1().s, t._f2().s));
							}
						}
						else {
							tupleArray = null;
					...
					if ( allowDoNotCheckInList ) {
						...
						final Set<String> strSet2 = env.getCurrentCompilationUnit().getCyanPackage().getPackageKeyValueSet(cyanMetaobject.getName() + "DoNotCheckIn");
						if ( strSet2 != null ) {
							final Set<String> strSet3 = new HashSet<>();
							if ( strSet != null ) { strSet3.addAll(strSet); }
							strSet3.addAll(strSet2);
							strSet = strSet3;
						}
						if ( strSet != null ) {
							for ( final String str : strSet ) {
								final Tuple2<String, String> packagePrototypenName =
										NameServer.splitPackagePrototype(MetaHelper.removeQuotes(str));
								if ( tupleArray == null ) {
									tupleArray = new ArrayList<>();
								}
					...
					if ( tupleArray != null ) {
						for ( final Tuple2<String, String> t : tupleArray ) {
							if ( t.f1.equals(currentPackageName) && t.f2.equals(currentPrototypeName) ) {
								foundProto = true;
								break;
							}
						}
		...
		if ( rightType instanceof TypeWithAnnotations ) {
			...
			for ( final AnnotationAt annot : rightTypeWithAnnot.getAnnotationToTypeList() ) {
				final CyanMetaobjectAtAnnot cyanMetaobject = annot.getCyanMetaobject();
				_CyanMetaobject other = cyanMetaobject.getMetaobjectInCyan();
					...
					try {
						...
						if ( other == null ) {
							...
							if ( messageSendExpr instanceof ExprMessageSendWithKeywordsToExpr ) {
								codeType = to.run(
										() ->  {
											return doesNot.semAn_analyzeReplaceKeywordMessage(
													((ExprMessageSendWithKeywordsToExpr ) messageSendExpr).getI(), env.getI());
										},
										timeoutMilliseconds, "semAn_analyzeReplaceKeywordMessage",
										cyanMetaobject, env);
...
//												((ExprMessageSendWithKeywordsToExpr ) messageSendExpr).getI(), env.getI());
								...
								codeType = to.run(
										() ->  {
											return doesNot.semAn_analyzeReplaceUnaryMessage(
													((ExprMessageSendUnaryChainToExpr ) messageSendExpr).getI(), env.getI());
										},
										timeoutMilliseconds, "semAn_analyzeReplaceUnaryMessage",
										cyanMetaobject, env);
...
//												((ExprMessageSendUnaryChainToExpr ) messageSendExpr).getI(), env.getI());
								...
								codeType = to.run(
										() ->  {
											return doesNot.semAn_analyzeReplaceUnaryMessageWithoutSelf(
													((ExprIdentStar ) messageSendExpr).getI(), env.getI());
										},
										timeoutMilliseconds, "semAn_analyzeReplaceUnaryMessageWithoutSelf",
										cyanMetaobject, env);
...
//										((ExprIdentStar ) messageSendExpr).getI(), env.getI());
								...
								env.error(symForError, "Internal error in MetaInfoServer.replaceMessageSendIfAsked: "
										...
										messageSendExpr.getClass().getName());
							}
						}
						else {
							...
							if ( messageSendExpr instanceof ExprMessageSendWithKeywordsToExpr ) {
								_Tuple_LT_GP_CyString_GP_CyString_GP_CyString_GT tdd =
								    to.run(
										  () ->  {
											return ((_IActionMessageSend__semAn ) other)._semAn__analyzeReplaceKeywordMessage_2(
													((ExprMessageSendWithKeywordsToExpr ) messageSendExpr).getI(), env.getI());
										  },
										  timeoutMilliseconds, "semAn_analyzeReplaceKeywordMessage",
										  cyanMetaobject, env);
...
//												((ExprMessageSendWithKeywordsToExpr ) messageSendExpr).getI(), env.getI());
								...
								_Tuple_LT_GP_CyString_GP_CyString_GP_CyString_GT tdd =
									    to.run(
											  () ->  {
												return ((_IActionMessageSend__semAn ) other)._semAn__analyzeReplaceUnaryMessage_2(
														((ExprMessageSendUnaryChainToExpr ) messageSendExpr).getI(), env.getI());
											  },
											  timeoutMilliseconds, "semAn_analyzeReplaceUnaryMessage",
											  cyanMetaobject, env);
...
//													((ExprMessageSendUnaryChainToExpr ) messageSendExpr).getI(), env.getI());
								...
								_Tuple_LT_GP_CyString_GP_CyString_GP_CyString_GT tdd =
									    to.run(
											  () ->  {
												return ((_IActionMessageSend__semAn ) other)._semAn__analyzeReplaceUnaryMessageWithoutSelf_2(
														((ExprIdentStar ) messageSendExpr).getI(), env.getI());
											  },
											  timeoutMilliseconds, "semAn_analyzeReplaceUnaryMessageWithoutSelf",
											  cyanMetaobject, env);
...
//													((ExprIdentStar ) messageSendExpr).getI(), env.getI());
					...
					if ( codeType != null  ) {
						...
						if ( codeType.f2 == null || codeType.f3 == null ||
							...
							if ( typeOfCode == null ) {
								env.error(symForError, "This message send was replaced by an expression that has type '" +
												codeType.f2 + "." +
...
//	   @return
//	 */
//	public static Type replaceMessageSendIfAsked(List<MethodSignature> allMethodSignatureList,
//			ExprMessageSend messageSendExpr,
//			Env env, Symbol symForError, Type originalType) {
//
//		List<AnnotationAt> list = new ArrayList<>();
//		/*
//		 * collect into 'list' all annotations attached to all methods of allMethodSignatureList
//		 */
		...
		for (final AnnotationAt annotation : list ) {
			...
			if ( cyanMetaobject instanceof IActionMessageSend_semAn
				...
				if ( dec instanceof WrMethodDec ) {
					puWhereAnnotWasFound = ((WrMethodDec ) dec).getDeclaringObject();
				}
				else {
				...
				if ( codeType != null  ) {
					...
					if ( lastPrototypeWhereAnnotWithCodeReplacementWasFound ==
						...
						env.error(symForError, "Two annotations attached to the same method are "
								...
								"' of line " + annotation.getFirstSymbol().getLineNumber() +
								" of prototype '" +
					...
					if ( lastPrototypeWhereAnnotWithCodeReplacementWasFound == null ) {
						 ...
						 * is not allowed to replace the message passing. If it is
