...
public class StatementLocalVariableDec extends Statement implements VariableDecInterface, Declaration {
	...
	public void genJava(PWInterface pw, Env env) {
		...
		if ( expr == null )
    		...
    		if ( env.getAddTypeInfo() &&  (type == null || type == Type.Dyn)  ) {
    			pw.println( this.codeAddsRuntimeTypeInfo() );
    		}
	...
	public boolean getCastLocalVariable() {
		return castLocalVariable;
	}
	...
	public String codeAddsRuntimeTypeInfo()  {
		...
		ObjectDec proto = this.getDeclaringMethod().getDeclaringObject();
		ObjectDec protoOuter = this.getDeclaringMethod().getDeclaringObject().getOuterObject();
		MethodDec outerCurrentMethod;
		if ( protoOuter != null ) {
			proto = protoOuter;
			outerCurrentMethod = proto.getExprFunctionForThisPrototype().getCurrentMethod();
		}
		else {
			outerCurrentMethod =  this.getCurrentMethod();
		}
		String prototypeFullName = proto.getFullName();
		String methodName = outerCurrentMethod.getMethodSignature().getFullNameWithReturnType();
		String fullJavaName = getJavaName() + (this.refType ? ".elem" : "");
		/*
		 * addDynLocalVariableInfo(String prototypeFullName,
	...
	/**
	 * true if this variable is a 'cast' statement variable
	 */
	private boolean castLocalVariable = false;



	public void setCastLocalVariable(boolean castLocalVariable) {
		this.castLocalVariable = castLocalVariable;
	}
