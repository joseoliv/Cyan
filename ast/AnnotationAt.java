...
public class AnnotationAt extends Annotation {
	...
	public void genCyanReal(PWInterface pw, boolean printInMoreThanOneLine,
		...
		if ( cyanEnv.getGenInterfacesForCompiledCode() ) {
			/*
			 * when generating the file with interfaces (for separate
			 * compilation), generate only the annotations that will be useful
			 * when the package or prototype is imported. That is, those
			 * annotations that can act in the prototype that imports the
			 * current prototype. These are the metaobjects whose classes
			 * implement IStayPrototypeInterface or have non-private visibility
			 */
			if ( !((cyanMetaobject instanceof IStayPrototypeInterface
				...
				for (final WrExpr e : this.realParameterList) {
					...
					if ( cyanEnv.getCreatingInstanceGenericPrototype() ) {
						if ( e instanceof WrExprAnyLiteral ) {
							final ExprAnyLiteral ie = meta.GetHiddenItem
									.getHiddenExprAnyLiteral(
											(WrExprAnyLiteral) e);
							ie.genCyanReplacingGenericParameters(pw,
									printInMoreThanOneLine, cyanEnv,
									genFunctions);
						}
						else
							meta.GetHiddenItem.getHiddenExpr(e).genCyan(pw,
									printInMoreThanOneLine, cyanEnv,
									genFunctions);
					}
					else
			...
			if ( leftCharSeqSymbol != null ) {
				...
				if ( !cyanEnv.getCreatingInstanceGenericPrototype() )
					...
					case REPLACE_BY_CYAN_VALUE:
						replacePrint(textAttachedDSL, pw,
					...
					case REPLACE_BY_JAVA_VALUE:
						// final String genProto =
						// cyanEnv.getExprGenericPrototypeInstantiation().getName();
						replacePrint(textAttachedDSL, pw,
								cyanEnv.getFormalParamToRealParamTable(),
								cyanEnv.getExprGenericPrototypeInstantiation()
										.getName(),
				...
				/*
				 * StringBuffer strText = new
				 * StringBuffer(textAttachedDSL.toString()); if ( strText !=
				 * null && strText.length() > 0 ) { strText.trimToSize(); int
				 * last = strText.length() - 1; if ( strText.charAt(last) ==
				 * '\0' ) strText.setCharAt(last, ' '); if ( cyanEnv == null )
				 * pw.println(strText); else { char []charText = new
				 * char[strText.length()]; strText.getChars(0, strText.length(),
				 * charText, 0); replacePrint(charText, pw,
				 * cyanEnv.getFormalRealTable()); } }
				 */
	...
	public String genJavaExpr(PWInterface pw, Env env) {
		...
		env.error(this.getFirstSymbol(),
				"Internal error: a metaobject is being used as an expression and the compiler wants it to generate Java code."
	...
	public void calcInternalTypes(Env env) {
		...
		if ( realParameterList != null ) {
			for (final WrExpr e : this.realParameterList) {
				...
				if ( ee instanceof ExprLiteralArray
						|| ee instanceof ExprLiteralTuple ) {
					/*
					 * if ( e instanceof ExprLiteralTuple ) { ExprLiteralTuple t
					 * = (ExprLiteralTuple ) e; }
					 */
		...
		if ( cyanMetaobject instanceof CyanMetaobjectCompilationContextPush ) {
			/**
			 * The metaobject annotation may have only two parameters. See
			 * {@link meta#CyanMetaobjectCompilationContextPush}. In this case
			 * the compiler has introduced <code>null</code> in the last three
			 * positions of the javaParameterList.
			 */
			...
			int lineNumber = 0;
			if ( javaParameterList.size() <= 2
					|| javaParameterList.get(2) == null ) {
				if ( env.getCurrentPrototype() == null )
					packageName = "Unidentified";
				else
					packageName = env.getCurrentCompilationUnit()
							.getPackageName();
			}
			else {
			...
			if ( javaParameterList.size() <= 3
					|| javaParameterList.get(3) == null ) {
				if ( env.getCurrentPrototype() == null )
			...
			if ( javaParameterList.size() <= 4
					|| javaParameterList.get(4) == null ) {
				lineNumber = -1;
			}
			else {
			...
			if ( javaParameterList.size() > 5
					&& javaParameterList.get(5) != null ) {
				String strSlotList = meta.MetaHelper
						.removeQuotes((String) javaParameterList.get(5));
				/*
				 * six parameters mean we are in a program unit
				 */
				Prototype currentPU = env.getCurrentPrototype();
				slotList = currentPU.extractSlotListFrom(strSlotList, this, env,
						this.getFirstSymbol());
			}

			env.pushCompilationContext(id, cyanMetaobjectName, packageName,
			...
			env.addToLineShift(
					-((CyanMetaobjectCompilationMarkDeletedCode) cyanMetaobject)
							.getNumLinesDeleted());
		}
		...
		/*
		 * else if ( cyanMetaobject instanceof
		 * CyanMetaobjectCompilationMarkDeletedCode ) {
		 * env.markDeletedCodeCompilation(
		 * ((CyanMetaobjectCompilationMarkDeletedCode)
		 * cyanMetaobject).getNumLinesDeleted() ); }
		 */

		semAnActions(env);

		Prototype pu;
		if ( cyanMetaobject.getPackageOfType() == null
				|| cyanMetaobject.getPrototypeOfType() == null ) {
			pu = env.searchPackagePrototype(MetaHelper.cyanLanguagePackageName,
					"Nil");
		}
		else {
			pu = env.searchPackagePrototype(cyanMetaobject.getPackageOfType(),
					cyanMetaobject.getPrototypeOfType());
		}

		if ( pu == null ) {
			type = env.searchPackageJavaClass(cyanMetaobject.getPackageOfType(),
					cyanMetaobject.getPrototypeOfType());
			if ( type == null ) {
		...
		if ( env.getCompInstSet()
			...
			if ( cyanMetaobject instanceof IAction_semAn
				...
				try {
						...
						if ( other == null ) {
							...
							cyanCode = to.run(() -> {
								return new StringBuffer(
										((_IAction__semAn) other)
												._semAn__codeToAdd_1(
														compiler_semAnFinal).s);
							}, timeoutMilliseconds, "semAn_codeToAdd",
									this.cyanMetaobject, env);

							// cyanCode = new StringBuffer( ((_IAction__semAn )
							// other)._semAn__codeToAdd_1(compiler_semAn).s );
						}
					 ...
					 *
					 * env.error(this.getFirstSymbol(),
					 * "This metaobject implements interface '" +
					...
					if ( cyanCode == null || cyanCode.length() == 0
							|| cyanCode.charAt(0) == '\0' ) {

						if ( this.symbolCyanAnnotation.getPostfix() == null ) {
							// this metaobject did not generated code in
								...
								if ( this.cyanMetaobject instanceof meta.ICodeg ) {
									...
									env.error(this.getFirstSymbol(),
											"Metaobject '"
													+ this.cyanMetaobject
															.getName()
													+ "' is not generating code in phase SEM_AN as expected");
								}
							}
						}
					}
						...
						/*
						 * metaobjects attached to declarations cannot generate
						 * code
						 */
						if ( this.declaration != null ) {
							env.error(this.getFirstSymbol(), "Metaobject '"
									+ this.cyanMetaobject.getName()
						...
						if ( this.isParsedWithCompiler() && env
								.sizeStackAnnotationParseWithCompiler() > 1 ) {
							/*
							 * this metaobject annotation is a literal object
							 * that is inside other literal object
							 */
							...
							this.codeThatReplacesThisStatement = new StringBuffer(
									this.asString() + " "
											+ env.addCodeAtAnnotation(
													cyanMetaobject, cyanCode,
													-1));
			...
			if ( cyanMetaobject instanceof IActionNewPrototypes_semAn
				...
				if ( prototypeNameCodeList != null ) {
					CyanPackage currentPackage = env.getCurrentCompilationUnit()
							.getCyanPackage();
			...
			if ( cyanMetaobject instanceof IActionVariableDeclaration_semAn
					|| (other != null
							&& other instanceof _IActionVariableDeclaration__semAn) ) {
				/*
				...
				if ( !(dec instanceof WrStatementLocalVariableDec) ) {
					env.error(this.getFirstSymbol(), "Metaobject '"
							...
							+ "' can only be attached to declaration of local variables");
				}
				...
				try {
					...
					if ( other == null ) {
						...
						CyString cys;
						cys = to.run(() -> {
							return ((_IActionVariableDeclaration__semAn) other)
									._semAn__codeToAddAfter_1(env.getI());
						}, timeoutMilliseconds, "semAn_codeToAddAfter",
								cyanMetaobject, env);
						if ( cys != null && cys.s != null
								&& cys.s.length() != 0 ) {
							code = new StringBuffer(cys.s);
						}
						// ((_IActionVariableDeclaration__semAn )
						// other)._semAn__codeToAddAfter_1(env.getI());
					}
				}
				catch (final error.CompileErrorException e) {
				}
			...
			// _IActionStatement__semAn__afterSemAn)
			) {
	...
	public static void replacePrint(char[] text, PWInterface pw,
		...
		pw.print(CompilerManager.replaceOnly(text, formalRealTable,
				currentPrototypeName, replacementPolicy));
	}
	...
	/*
	 * @Override public WrExpr getDeclaration() { return new WrExpr(this); }
	 */
	...
	/**
	 * return the declaration or expression associated to this metaobject
	 * annotation or null if there is no one. A declaration can be a prototype,
	 * field, or method. Therefore 'declaration' can be an object of Prototype,
	 * FieldDec, MethodDec, OR an expression.
	 */
	...
	/**
	 * Return the Java parameter of this metaobject annotation as a valid Java
	 * string. That is, if the parameter is an int, as 123, "123" is returned.
	 * If it is a string "ok", "\"ok\"" is returned.
	 *
	 * @param i
	 * @return
	 */
	...
	public void setSymbolCyanAnnotation(
			SymbolCyanAnnotation symbolCyanAnnotation) {
		this.symbolCyanAnnotation = symbolCyanAnnotation;
	}
	...
	/**
	 * return the information store at editing time by the visual interface of
	 * the Codeg
	 *
	 * @return
	 */
	...
	/**
	 * the complete name of the metaobject associated to this annotation. If the
	 * metaobject is a codeg, it is returned the metaobject name and the first
	 * parameter. Something like<br>
	...
	public String getCompleteName() {
		if ( this.cyanMetaobject instanceof ICodeg ) {
			final ExprAnyLiteral ea = meta.GetHiddenItem
					.getHiddenExprAnyLiteral(realParameterList.get(0));
			String id;
	...
	/**
	 * return the complete path of the file that keeps information on this
	 * metaobject annotation. Usually this is only used for codegs. Returns null
	 * if this annotation is outside a compilation unit (this should never
	 * happens
	 */
	public String filenameAnnotationInfo(String firstParameter) {
		...
		if ( firstParameter == null ) {
			_CyanMetaobject other = cyanMetaobject.getMetaobjectInCyan();
			if ( this.cyanMetaobject instanceof ICodeg
					|| (other != null && other instanceof _ICodeg) ) {
				if ( other == null ) {
				...
				final ExprAnyLiteral ea = meta.GetHiddenItem
						.getHiddenExprAnyLiteral(
								this.getRealParameterList().get(0));
				if ( ea instanceof ExprAnyLiteralIdent ) {
					id = ((ExprAnyLiteralIdent) ea).getIdentExpr().getName();
				}
			}
		...
		final String codegPath = this.compilationUnit.getCanonicalPathUpDir()
				+ NameServer.getCodegDirFor(this.getPrototypeOfAnnotation());

		final String codegInfoFilename = codegPath
				+ NameServer.fileSeparatorAsString
				+ this.cyanMetaobject.getName() + "(" + id + ")." + ext;
		return codegInfoFilename;
	}
	...
	public void setColorTokenList(
			List<Tuple4<Integer, Integer, Integer, Integer>> colorTokenList) {
	...
	public void replaceAnnotationBy(String newAnnotationText) {
		...
		if ( this.rightCharSeqSymbol != null ) {
			offsetEnd = this.rightCharSeqSymbol.getOffset()
					+ this.rightCharSeqSymbol.getSymbolString().length();
		}
		else {
			// no DSL between sequence delimiteres. Do the annotation have
			// parameters?
			if ( this.rightParenthesis != null ) {
				...
				offsetEnd = offsetStart
						+ this.symbolCyanAnnotation.getSymbolString().length()
						+ 1;
			}
		}
		this.compilationUnit.setOriginalText(Lexer.replaceTextByNewText(
				offsetStart, offsetEnd, this.compilationUnit.getOriginalText(),
				newAnnotationText.toCharArray()));
	}

	/**
	 * return a string with the annotation but with a new first parameter. That
	 * is, if the annotation is<br>
	 ...
	 * Note that the annotation is NOT changed. If the annotation has not any
	 * parameter, a new parameter is inserted
	 *
	 * @param newFirstParameter
	 * @return
	 */
	public String newAnnotationText(String newFirstParameter) {
		String s = "@" + this.cyanMetaobject.getName() + "("
				+ newFirstParameter;
		...
		if ( this.rightCharSeqSymbol != null ) {
			...
			s += this.leftCharSeqSymbol.getSymbolString()
					+ new String(this.getTextAttachedDSL())
					+ this.rightCharSeqSymbol.getSymbolString();
		}
		return s;
	}

	/**
	 * return a string with the annotation but with a new DSL textAttachedDSL,
	 * the textAttachedDSL attached to the annotation. That is, if the
	 * annotation is<br>
	...
	public String newAnnotationTextReplaceDSL(String newDSLText) {
		...
		if ( this.rightCharSeqSymbol != null ) {
			// there is a DSL attached to the annotation
			s += this.leftCharSeqSymbol.getSymbolString() + newDSLText
					+ this.rightCharSeqSymbol.getSymbolString();
		}
		return s;
	}
	...
	public void setOriginalDeclaration(
			AttachedDeclarationKind originalDeclaration) {
	...
	/**
	 * the declaration or expression associated to this metaobject annotation or
	 * null if there is no one. A declaration can be a prototype, field, or
	 * method. Therefore 'declaration' can be an object of Prototype, FieldDec,
	...
	/**
	 * the arguments to this metaobject annotation. The elements of List may be
	 * objects of String, Integer, Float etc. In<br>
	 * <code>
	 *      {@literal @}feature( "author", "José")<br>
	 * </code> there would be a list of two literal strings *
	 */

	private List<Object>			javaParameterList;

	/**
	 * the arguments to this metaobject annotation as AST objects. In
	 *
	 * @feature<<* "author", "José" *>> there would be a list of two literal
	 *             strings, objects of ExprLiteralString
	 */
	 ...
	 * that comes before the parameters @textAttachedDSL(trim_spaces)<** ... **>
	 *
	 */
	...
	/**
	 * if this metaobject annotation takes parameters, this symbol is the ')'
	 * that comes after the parameters @textAttachedDSL(trim_spaces)<** ... **>
	 *
	 */
	private Symbol					rightParenthesis;

	/**
	 * this is the symbol '<**' in @textAttachedDSL(trim_spaces)<** ... **>
	 */
	...
	/**
	 * the textAttachedDSL of the metaobject annotation. It is the
	 * textAttachedDSL between <** and **> in the annotation below.
	 *
	 * @javacode<** return _add_dot(_a); **>
	 */
	private char[]					textAttachedDSL;

	/**
	 * if the metaobject is a Codeg, variable {@link #codegInfo} is not null and
	 * receives the data of the file associated to this metaobject annotation.
	 * This data is produced at editing time and stored in a file. During
	 * compiling time this data is retrieved from the file and stored in this
	 * variable. The compiler discovers the name of this file using the current
	 * prototype name, its directory, the metaobject name, and the first
	 * parameter of the metaobject.
	 *
	 */
	...
	/**
	 * true if this annotation is inside a method. If it is, the associated
	 * metaobject is allowed to generated code in phase SEM_AN
	 */
	private boolean					insideMethod;

	/**
	 * if the value of this variable is AttachedDeclarationKind.PROGRAM_DEC, the
	 * annotation is attached to a Program. If it is
	 * AttachedDeclarationKind.PACKAGE_DEC, it is attached to a package. Field
	 * <code>declaration</code> of this class is changed to refer to each of the
	 * program units during the compilation. This field never changes.
	 */
