...
/**
 * Represents a message sent to an expression. Example: nil println; (get + 1)
 * println;
 *
 * The receiver may be an implicit self. In this case, the first parameter to
 * the constructor should be null.
 *
 * @author José
 *
 */
public class ExprMessageSendWithKeywordsToExpr extends ExprMessageSendWithKeywords {
	...
	/**
	 * @param the
	 *            receiver and message. If the receiver is an implicit self, it
	 *            should be null
	 */
	public ExprMessageSendWithKeywordsToExpr(Expr receiverExpr,
			MessageWithKeywords message, Symbol nextSymbol,
	...
	public WrExprMessageSendWithKeywordsToExpr getI() {
		if ( iExprMessageSendWithKeywordsToExpr == null ) {
			iExprMessageSendWithKeywordsToExpr = new WrExprMessageSendWithKeywordsToExpr(
					this);
		}
	...
	public boolean isNRE(Env env) {
		...
		for (final MessageKeywordWithRealParameters s : message
				.getkeywordParameterList()) {
			for (final Expr e : s.getExprList()) {
				if ( !e.isNRE(env) ) return false;
			}
		}
		return true;
	}
	...
	public boolean isNREForInitShared(Env env) {
		...
		final Prototype pu = env.getProject().getCyanLangPackage()
		...
		for (final MessageKeywordWithRealParameters s : message
				.getkeywordParameterList()) {
			for (final Expr e : s.getExprList()) {
				if ( !e.isNREForInitShared(env) ) return false;
			}
		}
		return true;
	}
	...
	public void genCyanReal(PWInterface pw, boolean printInMoreThanOneLine,
		 ...
		 * pw.print(this.codeThatReplacesThisExpr); return ; }
		 */

		if ( printInMoreThanOneLine ) {
			if ( receiverExpr != null ) {
				receiverExpr.genCyan(pw,
						PWCounter.printInMoreThanOneLine(receiverExpr), cyanEnv,
						genFunctions);
				pw.print(" ");
			}
	...
	public String genJavaExpr(PWInterface pw, Env env) {
		...
		// env.getCurrentMethod().getName().contains("eval") &&
		// env.getCurrentMethod().getMethodSignature().getParameterList().size()
		// == 1 &&
		...
		final boolean isThrow = message.getMethodNameWithParamNumber()
				.equals("throw:1");
		...
		if ( this.javaReceiver ) {
			...
			final String[] varTmpArray = new String[message
					.getkeywordParameterList().get(0).getExprList().size()];
			int i = 0;
			for (final Expr paramExpr : message.getkeywordParameterList().get(0)
					.getExprList()) {
				...
				if ( javaMethod != null ) {
					javaTypeName = javaMethod.getParameters()[i].getType()
							.getSimpleName();
				}
				else {
					javaTypeName = constructor.getParameters()[i].getType()
							.getSimpleName();
				}
				...
				if ( typeParamExpr instanceof TypeJavaClass
						...
						&& (javaMethod != null && javaMethod.getParameters()[i]
								.getType() == ((TypeJavaClass) typeParamExpr)
										.getTheClass()
								|| constructor != null
						...
						try {
							if ( ta.loadJavaClass() != null ) {
								env.error(this.getFirstSymbol(),
										"Unable to load the Java class "
												+ typeParamExpr.getFullName());
								return null;
							}
						}
					...
					if ( javaMethod != null
							&& javaMethod.getParameters()[i]
									.getType() == classTypeArray
							|| constructor != null
									&& constructor.getParameters()[i]
											.getType() == classTypeArray ) {
						equalJavaTypes = true;
				...
				if ( !equalJavaTypes && (javaTypeName
						.equals(Character.toLowerCase(cyanTypeName.charAt(0))
								+ cyanTypeName.substring(1))
						|| javaTypeName.equals("String")
								&& javaTypeName.equals("String")
						||
						// NameServer.javaWrapperClassToCyanName(javaTypeName).equals(cyanTypeName)
						(cyanNameFromWrapper = NameServer
								.javaWrapperClassToCyanName(
										javaTypeName)) != null
					...
					varTmpArray[i] = varTmpArray[i] + "."
							+ NameServer.getFieldBasicType(cyanTypeName);
				}
				else {
					if ( javaMethod != null ) {
						javaTypeName = javaMethod.getParameters()[i].getType()
					...
					if ( !javaTypeName.equals("java.lang.Object") ) {
						if ( !(exprType instanceof TypeJavaRef) ) {
							env.error(paramExpr.getFirstSymbol(),
									"Cannot cast this Cyan value of type '"
											+ exprType.getFullName() + "' to "
							...
							Class<?> paramType;
							if ( constructor != null ) {
								paramType = constructor.getParameters()[i]
										.getType();
							}
							else {
								paramType = javaMethod.getParameters()[i]
										.getType();
							}
							final Class<?> exprJavaClass = ((TypeJavaRef) exprType)
									.getClassLoad(env, this.getFirstSymbol());

							// if ( ! paramType.isAssignableFrom(exprJavaClass)
							// ) {
							if ( !ClassUtils.isAssignable(exprJavaClass,
									paramType) ) {
								env.error(paramExpr.getFirstSymbol(),
										"Cannot cast this Cyan value of type '"
				...
				// varTmpArray[i] = Type.genJavaExpr_CastJavaCyan(env,
				// varTmpArray[i], exprType, javaMethod.getParameters()[i],
				// paramExpr.getType());
				/*
				 * if ( exprType.ge instanceof Prototype ) { String puName =
				 * ((Prototype) exprType).getName(); if (
				 * NameServer.isBasicType(puName) ) { /* cast Cyan basic type
				 * value to Java basic type value / varTmpArray[i] =
				 * varTmpArray[i] + "." + NameServer.getFieldBasicType(puName);
				 * } }
				 */
			...
			if ( isNew ) {
				if ( exceptionList.length == 0 ) {
					...
					pw.println("new "
							+ ((TypeJavaClass) receiverExpr.getType()).getName()
							+ "(");
				}
				else {
				...
				if ( exceptionList.length == 0 ) {
					...
					if ( isVoid ) {
						...
						pw.printIdent(typeJavaName + " " + resultTmpVar + " = ("
								+ typeJavaName + " ) ");
					}
					...
					if ( isVoid ) {
						pw.printIdent(messageSendTmpVar + "."
								+ message.getJavaMethodNameReceiverJavaObject()
								+ "(");
					}
					else {
		...
		final boolean precededbyInter = tokenFirstkeyword == Token.INTER_DOT_ID_COLON
				|| tokenFirstkeyword == Token.INTER_ID_COLON;

		Type receiverExprType = receiverExpr != null
				? this.receiverExpr.getType()
				: null;
		// only messages == and != are allowed to union. Thus, any message to an
		// union
		// can only be == and !=
		...
		final boolean isMessageToDynExpr = receiverExpr != null
				&& (receiverExprType == Type.Dyn
						|| messageSendEqualEqualToUnion);
		final boolean checkParameters = !isMessageToDynExpr
				&& !this.getBackquote() && !precededbyInter;
		...
		if ( paramDecList == null ) {
			 ...
			 */
			for (final MessageKeywordWithRealParameters keyword : message
					.getkeywordParameterList()) {
			...
			for (final String tmp : stringArray) {
				...
				if ( --size > 0 ) paramPassing.append(", ");
			}
		}
		else {
			/*
			 * if the type of the real argument is Dyn and the type of the
			 * formal parameter is not Dyn, first generate code that cast the
			 * real argument to the correct type
			 */
			...
			for (final MessageKeywordWithRealParameters keyword : message
				...
				for (final Expr e : keyword.getExprList()) {
					...
					if ( checkParameters
							&& paramDecList.get(ii).getType() != Type.Dyn
							&& (rightType == Type.Dyn
									|| rightType instanceof TypeUnion) ) {
						final String otherTmpVar = NameServer
								.nextJavaLocalVariableName();
						String strFormalParamType = paramDecList.get(ii)
								.getType().getJavaName();
						pw.printlnIdent(
								strFormalParamType + " " + otherTmpVar + ";");
						if ( rightType == Type.Dyn ) {
							pw.printlnIdent("if ( " + tmpVar + " instanceof "
									+ strFormalParamType + " ) ");
							pw.printlnIdent("    " + otherTmpVar + " = ("
									+ strFormalParamType + " ) " + tmpVar
									+ ";");
							pw.printlnIdent("else");

							pw.printlnIdent(
									"    throw new ExceptionContainer__("
											+ env.javaCodeForCastException(e,
													paramDecList.get(ii)
															.getType())
											+ " );");
						}
							...
							// union
							pw.printlnIdent(
									otherTmpVar + " = (" + strFormalParamType
											+ " ) " + tmpVar + ";");
						}
						...
						if ( leftType == Type.Any
							...
							tmpVar = " (" + MetaHelper.AnyInJava + " ) "
									+ tmpVar;
						}
						else if ( rightType instanceof TypeJavaRef
								|| leftType instanceof TypeJavaRef ) {
							tmpVar = Type.genJavaExpr_CastJavaCyan(env, tmpVar,
									rightType, leftType, e.getFirstSymbol());
						}
			...
			/*
			 * A metaobject attached to the type of the formal parameter may
			 * demand that the real argument be changed. The new argument is the
			 * return of method changeRightHandSideTo
			 */
			ii = 0;
			ParameterDec param;
			int size = stringArray.size();
				...
				if ( cyanMetaobjectPrototype != null ) {
					...
					try {
						tmp = changeCyanMetaobject.cge_changeRightHandSideTo(
								prototypeFoundMetaobject, tmp,
								realParamExprList.get(ii).getType(env));
					}
						...
						env.thrownException(
								meta.GetHiddenItem
										.getHiddenCyanAnnotation(annotation),
								meta.GetHiddenItem.getHiddenSymbol(
										annotation.getFirstSymbol()),
								e);
					}
		...
		for (final MessageKeywordWithRealParameters sel : this.message
				.getkeywordParameterList()) {
		...
		if ( isNew && checkParameters ) {
			...
			if ( this.getBackquote() ) {
				...
				final String cyanMethodNameTmpVar = NameServer
						.nextJavaLocalVariableName();
				...
				for (final VariableDecInterface varDec : quotedVariableList) {
					...
					if ( varDec.getType() == Type.Dyn ) {
						...
						paramArray += s0;
						pw.printIdent("if ( ! (" + varDec.javaNameWithRef()
								+ " instanceof CyString) ) ");

						pw.printlnIdent("    throw new ExceptionContainer__("
								+ env.javaCodeForCastException(varDec,
										Type.String)
								+ " );");
					}
					else {
					...
					final String tmpChar = NameServer
							.nextJavaLocalVariableName();

					pw.printlnIdent("char " + tmpChar + " = "
							+ cyanMethodNameTmpVar + ".charAt(0);");
					pw.printlnIdent("if ( (" + tmpChar
							+ " == '_' || Character.isAlphabetic(" + tmpChar
							+ ") ) && " + "!" + cyanMethodNameTmpVar
							+ ".endsWith(\":\") ) { " + cyanMethodNameTmpVar
							+ " += \":\"; }");
					if ( --size2 > 0 ) paramArray += ", ";
				}
				String numParamList = "";
				...
				for (final MessageKeywordWithRealParameters sel : this.message
					...
					if ( --size3 > 0 ) numParamList += ", ";
				}

				final String javaNameMethodTmpVar = NameServer
						.nextJavaLocalVariableName();
				pw.printIdent("String " + javaNameMethodTmpVar + " = ");
				pw.println(" CyanRuntime.getJavaNameOfMethod(new String[] { "
						+ paramArray + " }, new int[] {" + numParamList
						+ "} );");

				resultTmpVar = genJavaDynMessageSend(pw, messageSendTmpVar,
						"new CyString(" + cyanMethodNameTmpVar + ")",
						javaNameMethodTmpVar, numParamJavaMethod,
						paramListAsString, stringArray, null,
						messageSendEqualEqualToUnion);
				...
				if ( isMessageToDynExpr || precededbyInter ) {
					...
					if ( message.getkeywordParameterList().size() == 1
							&& cyanMethodName.endsWith(":") ) {
						originalCyanMethodName = cyanMethodName.substring(0,
								cyanMethodName.length() - 1);
					}
					else {
					...
					resultTmpVar = genJavaDynMessageSend(pw, messageSendTmpVar,
							"new CyString(\"" + cyanMethodName + "\")",
							"\"" + javaNameMethod + "\"", numParamJavaMethod,
							paramListAsString, stringArray,
					...
					if ( receiverType instanceof TypeIntersection ) {
						...
						interDec = methodSignatureForMessage
					...
					if ( methodSignatureForMessage
							.getReturnType(env) == Type.Nil ) {
						pw.printlnIdent(type.getJavaName() + " " + resultTmpVar
								+ " = _Nil.prototype;");
						pw.printlnIdent(messageSendTmpVar + "." + javaNameMethod
								+ "( " + paramPassing.toString() + ");");
					}
					else {
						pw.printIdent(type.getJavaName() + " " + resultTmpVar
								+ " = ");
						pw.println(messageSendTmpVar + "." + javaNameMethod
								+ "( " + paramPassing.toString() + ");");
					}
		...
		if ( currentMethod2 != null ) {
			final String currentMethodName = currentMethod2
					.getNameWithoutParamNumber();
			if ( isThrow && !currentMethodName.equals("init:")
					&& !currentMethodName.equals("init") ) {
				/*
				 * to prevent the Java compiler from issuing an error message
				 * like 'method should return a value', a return statement is
				 * added every message send 'throw: obj'
				 */
				Type returnType = currentMethod2.getMethodSignature()
						.getReturnType(env);
	 ...
	 * @return
	 */
	private String genJavaExprForEqEq_NotEq(PWInterface pw, Env env,
			String methodName) {
		String resultTmpVar = NameServer.nextJavaLocalVariableName();

		String messageSendTmpVar = null;
		Expr paramExpr = message.getkeywordParameterList().get(0).getExprList()
				.get(0);
		...
		pw.printIdent(
				type.getJavaName() + " " + resultTmpVar + " = (Object ) ");
		pw.println(messageSendTmpVar + " " + methodName + " (Object ) "
				+ paramCode + ";");
	...
	private String genJavaDynMessageSend(PWInterface pw,
			String receiverAsString, String cyanMethodName,
			String keywordJavaName, int numParameters, String paramListAsString,
			List<String> paramStrList, String originalCyanMethodName,
			boolean messageSendEqualEqualToUnion) {
		String tmp;
		...
		if ( keywordJavaName != null ) {
			pw.printlnIdent(aMethodTmp + " = CyanRuntime.getJavaMethodByName("
					+ receiverAsString + ".getClass(), " + keywordJavaName
					+ ", " + numParameters + ");");
			if ( originalCyanMethodName != null ) {
				pw.printlnIdent("    if ( " + aMethodTmp + " == null ) { "
						+ aMethodTmp + " = CyanRuntime.getJavaMethodByName("
		...
		if ( messageSendEqualEqualToUnion ) {
			pw.printlnIdent(tmp + " = "
					+ (messageSendEqualEqualToUnion ? "(CyBoolean ) " : "")
					+ aMethodTmp + ".invoke(" + receiverAsString
					+ paramListAsString + ");");
		}
		else {
			...
			pw.printlnIdent("else {");
			pw.add();
			pw.printlnIdent(tmp + " = " + aMethodTmp + ".invoke("
					+ receiverAsString + paramListAsString + ");");
			pw.sub();
			pw.printlnIdent("}");
		...
		pw.printlnIdent(tmp + " = "
				+ (messageSendEqualEqualToUnion ? "(CyBoolean ) " : "")
				+ dnuTmpVar + ".invoke(" + receiverAsString + ", "
	...
	public void calcInternalTypes(Env env) {
		...
		if ( receiverExpr != null ) {
			...
			if ( receiverType instanceof TypeJavaRef ) {
				...
				if ( receiverType instanceof TypeWithAnnotations ) {
					String methodNameWithParamNumber = message
							.getMethodNameWithParamNumber();

					List<MethodSignature> mList = Type.Any
							.searchMethodPublicPackage(
		...
		if ( message.getBackquote() ) {
			...
			if ( receiverExpr == null || receiverExpr instanceof ExprSelf ) {
				MethodDec currentMethod2 = env.getCurrentMethod();
				if ( currentMethod2 != null ) {
					String currentMethodName = currentMethod2
							.getNameWithoutParamNumber();
			...
			if ( isNew ) {
				final TupleTwo<String, Type> t = receiverExpr
						.ifPrototypeReturnsNameWithPackageAndType(env);

				if ( t == null || t.f2 == null ) {
					// is it a Java class ?
					env.error(true, this.message.getFirstSymbol(),
							"Message with keyword(s) '"
									+ methodNameWithParamNumber
					...
					methodSignatureList = ((ObjectDec) receiverType)
							.searchInitNewMethod(methodNameWithParamNumber);
				}
			}
				...
				env.error(this.getFirstSymbol(),
					...
					|| receiverExpr instanceof ExprSelf ) {
				/*
				 * message send to self
				 */

				if ( env.getEnclosingObjectDec() == null
						|| !(env.getCurrentMethod() != null && NameServer
								.isMethodNameEval(env.getCurrentMethod()
										.getNameWithoutParamNumber())) ) {
					/*
					 * inside a regular prototype that is NOT inside another
					 * prototype OR inside an inner prototype and inside a
					 * method that is not 'eval', 'eval:'
					 */
					if ( methodName.equals("init:") ) {
						// ## was "init:"
						methodSignatureList = env.getCurrentObjectDec()
								.searchInitNewMethod(methodNameWithParamNumber);
					}
					else {
				...
				if ( methodSignatureList == null
					...
					if ( env.getCompInstSet()
							...
							&& lookForMethodAtCompileTime(env, receiverType) ) {
						return;
					}
					else {
						env.error(true, getFirstSymbol(), "Method "
								+ methodNameWithParamNumber
								+ " was not found in the current prototype or its super-prototypes",
								methodNameWithParamNumber,
								ErrorKind.method_was_not_found_in_prototype_or_super_prototypes,
								receiverExpr == null ? "self"
										: receiverExpr.asString());
					}
				}
				...
				methodSignatureList = receiverExpr.getType(env)
						.searchMethodPublicPackageSuperPublicPackage(
								methodNameWithParamNumber, env);
				/*
				 * if the receiverType is an union that is subtype of Any, then
				 * methods declared in Any can be called
				 */
				...
				// #### && !methodName.equals("==") && !methodName.equals("!=")
					...
					// env.error(this.getFirstSymbol(), "The type of the message
					// receiver, '" + receiverExpr.asString() +
					// "' is an union type. Then the receiver cannot receive
					// messages as '" + methodName + "'", true, true);
				}
				else {
					Prototype pu = (Prototype) receiverType.getInsideType();
					List<Prototype> superList = pu
					...
					if ( methodSignatureList == null
						...
						final ObjectDec currentProto = env
								.getCurrentObjectDec();
						...
						if ( methodSignatureList == null
							...
							if ( env.getCompInstSet().contains(
									...
									&& lookForMethodAtCompileTime(env,
											receiverType) ) {
								return;
							}
							else {
								...
								if ( methodSignatureList != null
										&& methodSignatureList.size() > 0
										&& methodSignatureList.get(0)
												.getMethod()
												.getVisibility() == Token.PROTECTED ) {
									if ( methodSignatureList.get(0).getMethod()
											.getShared() ) {
			...
			if ( methodSignatureList == null
				...
				env.error(true, getFirstSymbol(), "Method "
						+ methodNameWithParamNumber
						+ " was not found in the type of the receiver object or in its super-prototypes",
						methodNameWithParamNumber,
						ErrorKind.method_was_not_found_in_prototype_or_super_prototypes,
						receiverExpr == null ? "self"
								: receiverExpr.asString());
			...
			/*
			 * MetaInfoServer.
			 * checkMessageSendWithMethodMetaobjectBeforeTypingMessage(
			 * methodSignatureList, receiverType, receiverExpr, receiverKind,
			 * message, env, this.message.getFirstSymbol());
			 */

			if ( receiverKind == ExprReceiverKind.SELF_R ) {
				// TODO replace currentMethod by this.currentMethod and remove
				// local variable declaration
				final MethodDec currentMethod1 = env.getCurrentMethod();
				if ( currentMethod1 != null ) {
					currentMethod1
							.addSelfMessagePassing(methodSignatureList.get(0));
					currentMethod1.setSelfLeak(true);
				}
			...
			if ( methodSignatureForMessage instanceof MethodSignatureWithKeywords ) {
				MethodSignatureWithKeywords signature = (ast.MethodSignatureWithKeywords) methodSignatureList
						.get(0);
				ObjectDec declObj = signature.getMethod().getDeclaringObject();
				if ( declObj == Type.Any
						&& signature.getName().equals("isA:1") ) {
			...
			methodSignatureForMessage = checkMessageSend(methodSignatureList,
					env);
			if ( methodSignatureForMessage == null ) {
				checkMessageSend(methodSignatureList, env);
				env.error(message.getFirstSymbol(),
						"Type error in message send with method '"
				...
				if ( aMethod != null ) {
					if ( aMethod.getShared() ) {
						/*
						 * check if the receiver is a prototype legal: getMax: 0
						 * (inside P that define getMax: ) P getMax: 0 illegal:
						 * self getMax: 0
						 */
						...
						if ( receiverExpr instanceof ExprSelf ) {
							...
							/**
							 * check if the receiver has the method. The method
							 * should be declared in the receiver itself, there
							 * is no inheritance of shared methods
							 */
					...
					if ( visibility == Token.PRIVATE ) {
						...
						if ( aMethod.getDeclaringObject().getCompilationUnit()
							...
							env.error(this.getFirstSymbol(), "Method '"
									...
									+ aMethod.getDeclaringObject()
											.getCompilationUnit()
											.getCyanPackage().getName()
									+ "'");
						}
					}
					else if ( visibility == Token.PROTECTED ) {
						if ( !aMethod.getDeclaringObject().isSupertypeOf(
					...
					if ( !isNew && receiverIsPrototype ) {
						...
						if ( receiverType != null
								&& receiverType instanceof Prototype ) {
							...
							if ( (initMSList == null || initMSList.size() == 0)
								...
								if ( !canBeCalledOnPrototypes
										&& !(receiverType instanceof InterfaceDec) ) {
				...
				if ( isNew ) {
					// ## was "init:"
					methodNameWithParamNumber = methodNameWithParamNumber
							.replace("new:", "init:");
					final List<MethodSignature> initMethodSignatureList = ((ObjectDec) receiverType)
							.searchInitNewMethod(methodNameWithParamNumber);
					if ( env.getProject().getCompilerManager()
						...
						MetaInfoServer.checkMessageSendWithMethodMetaobject(
								initMethodSignatureList, receiverType,
								receiverExpr, receiverKind, message, env,
								this.message.getFirstSymbol());
					}
					...
					if ( env.getProject().getCompilerManager()
						...
						for (final MethodSignature ms : initMethodSignatureList) {
							type = MetaInfoServer.replaceMessageSendIfAsked(ms,
									this, env, this.getFirstSymbol(), type);
						}
					...
					if ( env.getProject().getCompilerManager()
							.getCompilationStep() == CompilationStep.step_9 ) {
						MetaInfoServer.checkMessageSendWithMethodMetaobject(
								allMethodSignatureList, receiverType,
								receiverExpr, receiverKind, message, env,
								this.message.getFirstSymbol());
					}
					...
					if ( env.getProject().getCompilerManager()
						...
						// pu.getAllSuperPrototypes();
						// for ( Prototype current : superList ) {
						// List<MethodSignature> currentMSList =
						// current.searchMethodPublicPackage(methodName, env);
						// if ( currentMSList != null ) {
						// allMethodSignatureList.addAll(currentMSList);
						// }
						// }
						...
						// type =
						// MetaInfoServer.replaceMessageSendIfAsked(methodSignatureForMessage,
						// this,
	...
	private void calcInternalTypesJavaReceiver(Env env, Type receiverType) {
		...
		if ( message.getkeywordParameterList().size() != 1 ) {
			env.error(this.getFirstSymbol(),
					"The receiver is a Java object. There should "
							+ "be just one message keyword like '"
							+ message.getkeywordParameterList().get(0)
									.getkeywordName()
							+ "'");
			return;
		}
		TypeJavaRef javaReceiverType = (TypeJavaClass) this.receiverExpr
				.getType();
		final Class<?>[] parameterTypes = new Class<?>[message
				.getkeywordParameterList().get(0).getExprList().size()];
		TypeVariable<?>[] arrayGenParamType = null;

		try {
			arrayGenParamType = javaReceiverType
					.getClassLoad(env, message.getFirstSymbol())
					.getTypeParameters();
		}
		...
		for (final Expr paramExpr : message.getkeywordParameterList().get(0)
			...
			if ( exprType.getInsideType() instanceof Prototype ) {
				...
				parameterTypes[i] = ((TypeJavaClass) exprType).getClassLoad(env,
						this.getFirstSymbol());
			}
				...
				if ( et instanceof TypeJavaClass ) {
					parameterTypes[i] = ((TypeJavaClass) et).getClassLoad(env,
							this.getFirstSymbol());
				}
				else {
					env.error(paramExpr.getFirstSymbol(),
							"A parameter to a call to a Java method should be "
		...
		if ( methodName.equals("new") ) {
			...
			try {
				...
				for (Constructor<?> construc : javaClass.getConstructors()) {
					if ( construc
							.getParameterCount() != parameterTypes.length ) {
						continue;
					}
					...
					if ( Modifier.isPublic(modif) ) {
						...
						for (Class<?> paramType : construc
								.getParameterTypes()) {
							if ( !paramType
									.isAssignableFrom(parameterTypes[k]) ) {
								paramOk = false;
								break;
							}
							++k;
				...
				// /*
				// * if the execution is here, the method has not exactly the
				// types of the real parameters. Like in
				// *
				// var sb = StringBuffer new;
				// //var Appendable ap = sb;
				// var Formatter formatter = Formatter new: sb;
				// *
				// * The type of the contructor of Formatter is Appendable, not
				// StringBuffer. Then we
				// * have to try all possibilities of supertypes of
				// StringBuffer. If there are more
				// * than one parameter, we have to try all possibilities of the
				// combinations of all
				// * supertypes of the parameters. We consider that A is subtype
				// of A
				// *
				// */
				//
				// final Class<?> [][] allComb =
				// ExprMessageSend.allCombinations2(parameterTypes);
				// for ( final Class<?>[] classArray : allComb ) {
				// try {
				// constructor = javaReceiverType.getClassLoad(env,
				// this.getFirstSymbol()).getConstructor(classArray);
				// type = javaReceiverType;
				// foundMethod = true;
				// if ( this.receiverExpr instanceof ExprIdentStar &&
				// ((ExprIdentStar) this.receiverExpr).getIdentStarKind() !=
				// IdentStarKind.jvmClass_t &&
				// !(this.receiverExpr instanceof
				// ast.ExprGenericPrototypeInstantiation) ) {
				// env.error(getFirstSymbol(), "'new' messages can only be sent
				// to a class receiver");
				// }
				// final int modif = constructor.getModifiers();
				// if ( ! Modifier.isPublic(modif) ) {
			...
			if ( !(receiverType instanceof TypeJavaRef) ) {
				env.error(this.getFirstSymbol(),
						"A Java class was expected as receiver of the message '"
								+ message.getkeywordParameterList().get(0)
										.getkeywordName()
								+ ": args'");
			}
			javaReceiverType = (TypeJavaRef) receiverType;
			final Class<?> aClassJavaReceiverType = javaReceiverType
					.getClassLoad(env, this.getFirstSymbol());
			if ( arrayGenParamType == null || arrayGenParamType.length == 0 ) {
				 ...
				 */
				try {
					// Class<?> aClassJavaReceiverType =
					// javaReceiverType.getaClass(env, this.getFirstSymbol());
					// javaMethod = aClassJavaReceiverType.getMethod(methodName,
					// parameterTypes);
					javaMethod = MethodUtils.getMatchingAccessibleMethod(
							aClassJavaReceiverType, methodName, parameterTypes);
					...
					if ( javaMethod == null ) {
						if ( (aClassJavaReceiverType == int.class
							...
							type = TypeJavaRef.classJavaToTypeJavaRef(
									methodReturnType, env,
									this.getFirstSymbol());

						}
						else {
							javaMethod = MethodUtils
									.getMatchingAccessibleMethod(Object.class,
						...
						final Class<?> methodReturnType = javaMethod
								.getReturnType();
						type = TypeJavaRef.classJavaToTypeJavaRef(
								methodReturnType, env, this.getFirstSymbol());
					...
					if ( (methodName.equals("==") || methodName.equals("!="))
						...
						type = env.getProject().getProgram()
								.searchJavaBasicType("boolean");
						if ( type != null ) {
					...
					for (Method mm : aClassJavaReceiverType.getMethods()) {
						if ( mm.getName().equals(methodName) ) {
							if ( mm.getParameterCount() == parameterTypes.length ) {
								...
								int kk = 0;
								for (java.lang.reflect.Parameter pp : mm
										.getParameters()) {
									ClassUtils.isAssignable(pp.getType(),
											parameterTypes[kk]);
									if ( !pp.getType().isAssignableFrom(
											parameterTypes[kk]) ) {
										typeError = true;
										break;
									}
					...
					if ( !foundMethod ) {
						...
						javaMethod = MethodUtils.getMatchingAccessibleMethod(
								aClassJavaReceiverType, methodName,
								parameterTypes);

						/*
						 * if the execution is here, the method has not exactly
						 * the types of the real parameters. Like in
						 *
						 * //System.out.println(0);
						 *
						 * The type the parameter to println is Object, not int
						 * or Integer. Then we have to try all possibilities of
						 * supertypes of int. If there are more than one
						 * parameter, we have to try all possibilities of the
						 * combinations of all supertypes of the parameters. We
						 * consider that A is subtype of A
						 *
						 */
						// javaMethod = javaReceiverType.getClassLoad(env,
						// this.getFirstSymbol()).getMethod(methodName,
						// classArray);

						for (int ii = 0; ii < parameterTypes.length; ++ii) {
							final Class<?> paramType = parameterTypes[ii];
							if ( paramType.isPrimitive() ) {
								parameterTypes[ii] = NameServer
										.javaPrimitiveTypeToWrapperClass(
												parameterTypes[ii].getName());
							}
						}
						final Class<?>[][] allComb = ExprMessageSend
								.allCombinations2(parameterTypes);
						for (final Class<?>[] classArray : allComb) {
							try {
								javaMethod = javaReceiverType
										.getClassLoad(env,
												this.getFirstSymbol())
										.getMethod(methodName, classArray);
								final Class<?> methodReturnType = javaMethod
										.getReturnType();

								final java.lang.reflect.Type aReflectGenericType = javaMethod
										.getGenericReturnType(); // E
								final String aReflectGenericTypeName = aReflectGenericType
										.getTypeName();
					...
					if ( !foundMethod ) {
						...
						if ( lastExceptionThrown != null ) {
							env.error(this.getFirstSymbol(),
									"There was a security exception (class SecurityException) when searching for method "
											+ methodName);
						}
						else {
							env.error(this.getFirstSymbol(),
									"A Java method for the message passing '"
				...
				try {
					...
					final String aReflectGenericTypeName = aReflectGenericType
							.getTypeName();
					// javaGenericClass.getTypeParameters()
					setTypeMaybeGenericParameter(env, aClassJavaReceiverType,
							methodReturnType, aReflectGenericTypeName);
					checkStaticPublicMethod(env);
					 ...
					 * //System.out.println(0);
					 *
					 * The type the parameter to println is Object, not int or
					 * Integer. Then we have to try all possibilities of
					 * supertypes of int. If there are more than one parameter,
					 * we have to try all possibilities of the combinations of
					 * all supertypes of the parameters. We consider that A is
					 * subtype of A
					 *
					 */
					for (int ii = 0; ii < parameterTypes.length; ++ii) {
	...
	private void checkStaticPublicMethod(Env env) {
		...
		if ( Modifier.isStatic(modif) ) {
			...
			if ( this.receiverExpr instanceof ExprIdentStar
					...
					&& !(this.receiverExpr instanceof ast.ExprGenericPrototypeInstantiation) ) {
				env.error(getFirstSymbol(),
						"Static methods can only be called with a class receiver");
			}
		}
		else if ( !Modifier.isPublic(modif) ) {
			env.error(getFirstSymbol(),
					"Method '" + javaMethod.getName() + "' is not public");
		}
	}
	...
	private void setTypeMaybeGenericParameter(Env env,
		...
		int tnIndex = 0;
		for (final TypeVariable<?> aGenericTypeOfReceiverType : aClassJavaReceiverType
				.getTypeParameters()) {
			if ( aReflectGenericTypeName
					.equals(aGenericTypeOfReceiverType.getTypeName()) ) {
				foundGenType = true;
				break;
			}
			++tnIndex;
		...
		if ( foundGenType ) {
			/*
			 * look in the generic prototype instantiation which is the type
			 * associated to tn
			 */

			final ExprGenericPrototypeInstantiation gpi = ((TypeJavaRef) receiverExpr
					.getType()).getGPI();
			if ( gpi == null ) {
				/*
				 * information on the generic prototype instantiation is not
				 * available. This happens in code like (annot
				 * getRealParameterList) get: i in which there is no type
				 * List<..> in the code. Then the compiler is not able to
				 * discover the real type from a declaration. This happens in
				 * var List<Integer> array; var n = array get: 0; The compiler
				 * knows the declaration of array and knows its parameter is
				 * 'Integer'. In the first case, we consider that the type is
				 * Dyn
				 */
				type = Type.Dyn;
			}
			 ...
			 * env.error(this.getFirstSymbol(), "The " + tnIndex +
			 * "th real type parameter to '" + gpi.getName() + "', '" +
			 * ttt.getFullName() + "' should be a Java type. It is not");
			 *
			 * } / type = TypeJavaRef.classJavaToTypeJavaRef( ((TypeJavaRef)
			 * ttt).getaClass(env, gpi.getFirstSymbol()), env,
			 * this.getFirstSymbol());
			 */
		}
		else {
			type = TypeJavaRef.classJavaToTypeJavaRef(realJavaReturnType, env,
					this.getFirstSymbol());
		}
	}
	...
	private boolean isReturnTypeGeneric(Env env, Class<?> javaGenericClass,
		...
		if ( foundGenType ) {
			...
			final Type ttt = gpi.getRealTypeListList().get(0).get(tnIndex)
					.getType();
			if ( !(ttt instanceof TypeJavaRef) ) {
				env.error(this.getFirstSymbol(),
						"The " + tnIndex + "th real type parameter to '"
								+ gpi.getName() + "', '" + ttt.getFullName()
								+ "' should be a Java type. It is not");

			}
			type = TypeJavaRef.classJavaToTypeJavaRef(
					((TypeJavaRef) ttt).getClassLoad(env, gpi.getFirstSymbol()),
					env, this.getFirstSymbol());
		}
		else {
			type = TypeJavaRef.classJavaToTypeJavaRef(
					javaTypeGenericParam_or_NotGeneric, env,
					this.getFirstSymbol());
		}
		return true;
	}
	...
	/**
	 * javaGenericClass is a Java class, the type of the receiver object in a
	 * message send:<br>
	 * <code>
	 *      var List<Int> intArray = List<Int> new;<br>
	 *      intArray get: i <br>
	 * </code> javaTypeGenericParam_or_NotGeneric is
	 *
	 * @param env
	 * @param javaGenericClass
	 * @param javaTypeGenericParam_or_NotGeneric
	 ...
	 * @return
	 */
	private boolean javaTypeFor_a_MaybeJavaType(Env env,
			Class<?> javaGenericClass,
			Class<?> javaTypeGenericParam_or_NotGeneric,
			java.lang.reflect.Type javaTypeDescriptor) {
	...
	public boolean lookForMethodAtCompileTime(Env env, Type receiverType) {
		...
		final List<AnnotationAt> metaobjectAnnotationList = proto
		...
		for (final AnnotationAt annot : metaobjectAnnotationList) {
			...
			try {
				if ( other == null ) {
					...
					// doesNot._semAn__missingKeywordMethod_3(
					// receiverExpr == null ? null : receiverExpr.getI(),
					// message == null ? null : message.getI(), env.getI());
	...
	public static WrPrototype currentPrototypeFromAnnot(AnnotationAt firstMO) {
	...
	public static void checkAnnotIActionMethodMissing_semAn(Env env,
		...
		if ( !cyanMetaobject.attachedDecKindListSubsetOf(
			...
			env.error(annot.getFirstSymbol(),
					"The metaobject class of this annotation implements interface "
							+ "'"
							+ IActionMethodMissing_semAn.class
									.getCanonicalName()
							+ "'. Then this annotation can only"
	...
	static Object[] cast(EvalEnv ee, Object[] fromArray,
			Class<?>[] paramTypes) {

		Object[] objectArgList = new Object[fromArray.length];
		if ( fromArray.length != paramTypes.length ) {
			return null;
		}
	...
	public Object eval(EvalEnv ee) {
		...
		/*
		 * true if the receiver class is a Cyan prototype and the class was not
		 * found with the parameter types used in the program. For example,
		 * Tuple<Int, String, Char> will not be found. This method will try to
		 * find Tuple<Dyn, Dyn, Dyn> and therefore the Java class corresponding
		 * to this Cyan prototype should take Object parameters in its
		 * constructor.
		 */
		...
		if ( isNew ) {
			...
			if ( indexLessThan >= 0 ) {
				genericPrototypeReceiver_AndisNew = true;
			}
			// String rnameStripped = rname;
			/*
			 * possibilities: String java.lang.String java.util.Set<String>
			 * Set<String> Set<java.lang.String> java.util.Set<java.lang.String>
			 */
			// if ( indexLessThan >= 0 ) {
		...
		if ( this.javaReceiver ) {
			if ( message.getkeywordParameterList().size() > 1 ) {
				ee.error(message.getFirstSymbol(),
						"The receiver of this message passing is a "
								+ "Java object. Therefore there should be just one Cyan keyword in the message ('"
								+ message.getkeywordParameterList().get(0)
										.getkeywordName()
								+ "')");
			}
			size = message.getkeywordParameterList().get(0).getExprList()
					.size();

			// List<MethodSignature> methodSignatureList = null;
			// final String methodNameWithParamNumber =
			// message.getMethodNameWithParamNumber();
			//
			// methodSignatureList =
			// receiverExpr.getType(ee.env).searchMethodPublicPackageSuperPublicPackage(methodNameWithParamNumber,
			// ee.env);
			// if ( methodSignatureList == null ||
			...
			// methodSignatureList.get(0).getParameterList();
			// int i = 0;
			for (final Expr paramExpr : message.getkeywordParameterList().get(0)
					.getExprList()) {
				//
			...
			size = 0;
			for (final MessageKeywordWithRealParameters sel : message
					.getkeywordParameterList()) {
		...
		if ( this.javaReceiver ) {
			...
			if ( isNew ) {
				...
				try {
					...
					Object[] newObjArgList = ExprMessageSendWithKeywordsToExpr
							.cast(ee, objectArgList,
					...
					for (Constructor<?> method : receiverClass
							.getConstructors()) {
						// System.out.println(method.getParameters().length > 0
				...
				ee.error(message.getFirstSymbol(),
						"Error when calling the constructor '"
				...
				String javaMethodNameReceiverJavaObject = message
						.getJavaMethodNameReceiverJavaObject();
				String msgException = null;
				try {
					if ( !Character.isAlphabetic(methodName.charAt(0))
							&& methodName.charAt(0) != '_' ) {
						receiverValue = Statement
								.castJavaBasicToCyan(receiverValue);
						receiverClass = receiverValue.getClass();
						objectArgList[0] = Statement
								.castJavaBasicToCyan(objectArgList[0]);
						parameterTypes[0] = objectArgList[0].getClass();
						final String javaMethodName = message
								.getJavaMethodName();
						if ( methodName.equals("!=")
								|| methodName.equals("==") ) {
							parameterTypes[0] = Object.class;
						}
						javaMethod = Statement.getMethod(receiverClass,
								javaMethodName, 1);
						if ( javaMethod == null ) {
							ee.error(this.getFirstSymbol(),
									"Method '" + methodName
						...
						// javaMethod = receiverClass.getMethod(javaMethodName,
						// parameterTypes);
						...
						javaMethod = receiverClass.getMethod(
								javaMethodNameReceiverJavaObject,
								parameterTypes);
						javaMethod.setAccessible(true);
						ret = javaMethod.invoke(receiverValue, objectArgList);
						return ret;
					...
					try {
						...
						for (Method method : receiverClass.getMethods()) {
							if ( javaMethodNameReceiverJavaObject
									.equals(method.getName())
									&& method
											.getParameterCount() == sizeArgList ) {
								...
								try {
									...
									if ( sizeArgList > 0 ) {
										newObjArgList = ExprMessageSendWithKeywordsToExpr
												.cast(ee, objectArgList, method
														.getParameterTypes());
				...
				String[] methodNameList = new String[receiverClass
						.getMethods().length];
				int ii = 0;
				for (Method method : receiverClass.getMethods()) {
					if ( javaMethodNameReceiverJavaObject
						...
						for (java.lang.reflect.Parameter pp : method
								.getParameters()) {
				...
				ee.error(message.getFirstSymbol(), "Error when calling method '"
						+ methodName + "' of class '" + receiverClass.getName()
						+ "'. The Java name is '"
						+ message.getJavaMethodNameReceiverJavaObject()
						+ (msgException != null
								? "The exception message is " + msgException
								: "")
			...
			if ( this.getBackquote() ) {
				ee.error(this.getFirstSymbol(),
						"backquotes are not supported yet");
				/*
				...
				if ( isNew ) {
					...
					/*
					 * List<MethodSignature> methodSignatureList = null; final
					 * String methodNameWithParamNumber =
					 * message.getMethodNameWithParamNumber();
					 *
					 * methodSignatureList = receiverExpr.getType(ee.env).
					 * searchMethodPublicPackageSuperPublicPackage(
					 * methodNameWithParamNumber, ee.env); if (
					 * methodSignatureList == null ||
					 * methodSignatureList.get(0).getParameterList().size() !=
					 * message.getkeywordParameterList().get(0).getExprList().
					 * size() || (!(methodSignatureList.get(0) instanceof
					 * MethodSignatureWithKeywords) &&
					 * !(methodSignatureList.get(0) instanceof
					 * MethodSignatureOperator) ) ) {
					 * ee.error(message.getFirstSymbol(), "Method '" +
					 * methodName + "' not found in class '" +
					 * receiverClass.getName() + "'"); return null;
					 *
					 * } if ( methodSignatureList.get(0) instanceof
					 * MethodSignatureWithKeywords ) {
					 * MethodSignatureWithKeywords ms =
					 * (MethodSignatureWithKeywords )
					 * methodSignatureList.get(0); int k = 0; Object
					 * []newObjArgList = new Object[objectArgList.length]; for (
					 * ast.MethodKeywordWithParameters mk : ms.getKeywordArray()
					 * ) { List<ParameterDec> paramList = mk.getParameterList();
					 * if ( k >= objectArgList.length ) {
					 * ee.error(message.getFirstSymbol(), "Method '" +
					 * methodName + "' not found in class '" +
					 * receiverClass.getName() + "'"); } for ( ParameterDec
					 * param : paramList ) { newObjArgList[k] =
					 * Statement.castCyanJava(ee, param.getType().getClass(),
					 * objectArgList[k], this.getFirstSymbol()); ++k;
					 *
					 * } } objectArgList = newObjArgList;
					 *
					 * } else { // operator MethodSignatureOperator ms =
					 * (MethodSignatureOperator ) methodSignatureList.get(0);
					 * ms.getParameterList().get(0).getType() objectArgList[0] =
					 * Statement.castCyanJava(ee,
					...
					if ( message.getkeywordParameterList().size() == 1 ) {
						methodNameInJava = message.getkeywordParameterList()
								.get(0).getkeywordName();
						if ( methodNameInJava.endsWith(":") ) {
							methodNameInJava = methodNameInJava.substring(0,
									methodNameInJava.length() - 1);
						}
					}
					...
					try {
						if ( !Character.isAlphabetic(methodName.charAt(0))
							...
							try {
								javaMethod = receiverClass.getMethod(
										javaMethodName, parameterTypes);
								javaMethod.setAccessible(true);
								ret = javaMethod.invoke(receiverValue,
										objectArgList);
								return ret;
							}
							catch (IllegalAccessException
						...
						ret = Statement.sendMessage(receiverValue,
								javaMethodName, methodNameInJava, objectArgList,
								message.getMethodName(),
