...
//		}
		...
		final boolean isMessageToDynExpr = receiverExpr != null && this.receiverExpr.getType() == Type.Dyn;
		...
		final String currentMethodName = env.getCurrentMethod().getNameWithoutParamNumber();
		if ( isThrow && ! currentMethodName.equals("init:") && !currentMethodName.equals("init") ) {
			/*
		   ...
		   */
			...
			if ( isNew ) {
				...
				if ( !(receiverExpr instanceof ast.ExprGenericPrototypeInstantiation)
						&& receiverType.isNonTaggedUnionType() && !methodName.equals("==") && !methodName.equals("!=") ) {
				...
				if ( methodSignatureList == null || methodSignatureList.size() == 0 ) {
					...
					if ( methodSignatureList == null || methodSignatureList.size() == 0 )  {
						...
						if ( env.getCompInstSet().contains(CompilationInstruction.dsa_actions) && lookForMethodAtCompileTime(env, receiverType) ) {
							...
							env.error(true, getFirstSymbol(),
									"Method " + methodNameWithParamNumber + " was not found in prototype " + receiverExpr.getType(env).getName() + " or its super-prototypes",
