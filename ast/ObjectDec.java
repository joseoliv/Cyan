...
//		}
		...
		final boolean hasInitOnceMethod = this.searchMethodPrivate(MetaHelper.initOnce) != null;
		...
		if ( ! hasInitMethod ) {
			...
			if ( this.outerObject == null ) {
				...
				if ( hasInitOnceMethod ) {
					pw.printlnIdent( MetaHelper.getJavaName(MetaHelper.initOnce) + "();");
				}
...
//			final String s = m.getName();
				...
				for (int k = 0; k < initListSameParamNumber.size(); ++k ) {
					...
					for (int j = k + 1; j < initListSameParamNumber.size(); ++j ) {
						...
						for (int p = 0; p < numParam; ++p ) {
							final Type firstType = firstParamList.get(p).getType(env);
							final Type secondType = secondParamList.get(p).getType(env);
	...
	public boolean isSupertypeOf(Type otherType, Env env) {
		final Type originalType = otherType;
