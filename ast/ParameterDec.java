...
public class ParameterDec implements VariableDecInterface, GenCyan, ASTNode {
	...
	public void genJavaForMultiMethod(PWInterface pw) {
		...
		// String javaNameVar;
		if ( variableSymbol != null )
			javaName = MetaHelper.getJavaName(getName());
		else
			javaName = NameServer.nextJavaLocalVariableName();
	    pw.print("Object " + javaName);
	}
		...
		if ( (cs == CompilationStep.step_8 || cs == CompilationStep.step_9) &&
			 ...
			 * the object is something like
                object Fun_0__(ChooseFoldersCyanInstallation self__, Any &s12)  extends Function<Nil>
                    ...
    				fun new: ChooseFoldersCyanInstallation self__, Any &s12 -> Fun_0__ { ... }
    				...
    			end
	...
	public String codeAddsRuntimeTypeInfo()  {
		...
		String methodName = this.getDeclaringMethod().getMethodSignature().getFullNameWithReturnType();
		String fullJavaName = getJavaName() + (this.refType ? ".elem" : "");
		return "cyanruntime.CyanRuntime.addDynParameterInfo(\"" +
				prototypeFullName + "\", \"" + proto.getSHA256Code() + "\", " +
		          "\"" + methodName + "\", " +
			     getName() + "\", " + first.getLineNumber() + ", " +
				first.getColumnNumber() + ", " +
				fileOffset + ", " + wasTypeSupplied + ", " +
				fullJavaName + (this.refType ? ".elem" : "")+ " instanceof cyan.lang._Nil ? \"Nil\" : (" +
