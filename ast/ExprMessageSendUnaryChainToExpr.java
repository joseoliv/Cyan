...
public class ExprMessageSendUnaryChainToExpr extends ExprMessageSendUnaryChain {
	...
	public WrExprMessageSendUnaryChainToExpr getI() {
		if ( iExprMessageSendUnaryChainToExpr == null ) {
			iExprMessageSendUnaryChainToExpr = new WrExprMessageSendUnaryChainToExpr(this);
		}
	...
	public String genJavaExpr(PWInterface pw, Env env) {
		...
		if ( this.javaReceiver ) {
			...
			if ( !isJavaField && unarySymbol.getSymbolString().equals("new") ) {
				...
				if ( exceptionList != null && exceptionList.length == 0 ) {
					...
					pw.println( "new " + ((TypeJavaClass ) this.receiverExprOrFirstUnary.getType()).getName() + "();" );
				}
				else {
					...
					pw.printlnIdent( resultTmpVar + " = new " + ((TypeJavaClass ) this.receiverExprOrFirstUnary.getType()).getName() + "();" );
				}
			}
			else {
				...
				if ( exceptionList == null || exceptionList.length == 0 ) {
					messageSendTmpVar = receiverExprOrFirstUnary.genJavaExpr(pw, env);
					...
					pw.print(messageSendTmpVar + "." + this.unarySymbol.getSymbolString());
					if ( isJavaField ) {
						pw.println(";");
					}
					else {
					...
					if ( isVoid ) {
						pw.printlnIdent(messageSendTmpVar + "." + this.unarySymbol.getSymbolString()   + "();" );
					}
					else {
						pw.printlnIdent(resultTmpVar + " = " + messageSendTmpVar + "." + this.unarySymbol.getSymbolString()   + "();" );
					}
				}
			}
			if ( exceptionList != null && exceptionList.length != 0 ) {
		...
		boolean precededbyInter = tokenFirstkeyword == Token.INTER_ID || tokenFirstkeyword == Token.INTER_DOT_ID;
		...
		if ( hasBackquote ) {
			...
			if ( varDec.getType() == Type.Dyn ) {
				paramArray += "((CyString ) " + varDec.javaNameWithRef() + ").s";
			...
			String resultTmp = genJavaDynUnaryMessageSend(pw, receiverAsString, javaNameVariable, keywordJavaName, javaNameVariable);
...
//						if ( eis.getMethodSignatureForMessageSend().getReturnType(env) == Type.Nil ) {
...
//				}
			...
			if ( isNew ) {
				...
				String keywordJavaName = MetaHelper.getJavaNameOfkeyword(cyanMethodName);
				...
				if ( receiverType == Type.Dyn || precededbyInter ) {
					...
					tmp = genJavaDynUnaryMessageSend(pw, receiverAsString, cyanMethodName, keywordJavaName, originalCyanMethodName);
				}
				else {
					String methodName = this.unarySymbol.getSymbolString();
					List<MethodSignature> methodSignatureList =
							receiverType.searchMethodPublicPackageSuperPublicPackage(methodName, env);
					if ( methodSignatureList == null || methodSignatureList.size() == 0 ) {
						methodSignatureList =
								receiverType.searchMethodPrivateProtectedPublicPackageSuperProtectedPublicPackage(methodName, env);
					}
					if ( methodSignatureList == null || methodSignatureList.size() == 0 )  {
//						System.out.println("receiver Type: " + receiverType.getFullName() + " method name: " + methodName);
//						receiverType.searchMethodPublicPackageSuperPublicPackage(methodName, env);
					...
					if ( receiverType instanceof TypeIntersection ) {
						...
						interDec = methodSignatureList.get(0).getDeclaringInterface();
						if ( interDec == null ) {
							env.error( this.getFirstSymbol(),
									"Internal compiler error when generating code "
									+ "for a unary message passing, ExprMessageSendUnaryChainToExpr, "
							...
							receiverAsString = "((" + methodPrototype + ") " + receiverAsString + ")";
						}
					}
					boolean alreadyGeneratedCode = false;
					if ( this.receiverExprOrFirstUnary instanceof ExprIdentStar ) {
					...
					if ( !alreadyGeneratedCode ) {
						if ( methodSignatureList.get(0).getReturnType(env) == Type.Nil ) {
							pw.printlnIdent(type.getJavaName() + " " + tmp + " = _Nil.prototype;");
							pw.printlnIdent(receiverAsString + "." + keywordJavaName + "();");
						}
						else {
							pw.printIdent(type.getJavaName() + " " + tmp + " = ");
	...
	public Symbol getFirstSymbol() {
		if ( backquoteSymbol != null && (receiverExprOrFirstUnary instanceof ast.ExprMessageSendUnaryChainToExpr) ) {
	...
	public void calcInternalTypes(Env env) {
		...
		try {
			...
			if ( receiverExprOrFirstUnary instanceof ExprSelf ) {
				((ExprSelf ) receiverExprOrFirstUnary).calcInternalTypesDoNotCheckSelf(env);
			}
			else {
		...
		if ( exprType instanceof TypeJavaRef ) {
			...
			if ( methodName.equals("new") ) {
				...
				if ( receiverExprOrFirstUnary instanceof ExprIdentStar ) {
					...
					if ( eis.getIdentStarKind() != IdentStarKind.jvmClass_t ) {
						...
						if ( !(exprType instanceof TypeJavaClass) ) {
							env.error(this.getFirstSymbol(), "A Java class was expected as receiver of the message 'new'");
						}
						TypeJavaClass javaClass = (TypeJavaClass ) exprType;
						try {
							constructor = javaClass.getClassLoad(env, eis.getFirstSymbol()).getConstructor();
							type = javaClass;
							return ;
						}
					...
					if ( !(exprType instanceof TypeJavaClass) ) {
						...
						TypeJavaClass javaClass = (TypeJavaClass ) exprType;
						try {
							constructor = javaClass.getClassLoad(env, gpi.getFirstSymbol()).getConstructor();
							type = javaClass;
							return ;
						}
						catch (NoSuchMethodException
				...
				if ( !(exprType instanceof TypeJavaRef) ) {
					env.error(this.getFirstSymbol(), "A Java class was expected as receiver of the message 'new: args'");
				}
				if ( hasBackquote ) {
				...
				else if ( tokenFirstkeyword == Token.INTER_DOT_ID || tokenFirstkeyword == Token.INTER_ID || exprType == Type.Dyn ) {
					/*
				   ...
				   */
					...
					try {
						...
						javaMethod = javaClass.getClassLoad(env, this.getFirstSymbol()).getMethod(methodName);

						Class<?> methodReturnType = javaMethod.getReturnType();
						type = TypeJavaRef.classJavaToTypeJavaRef(methodReturnType, env, unarySymbol);
						...
						try {
							if ( javaClass instanceof TypeJavaRefArray ) {
								if ( methodName.equals("length") ) {
									// the 'length' field of arrays is not found by getField. Wierd!
									type = TypeJavaRef.classJavaToTypeJavaRef(int.class, env, unarySymbol);
									this.isJavaField = true;
									return ;
								}
							}
							Field f = javaClass.aClass.getField(methodName);
							type = TypeJavaRef.classJavaToTypeJavaRef(f.getType(), env, unarySymbol);
							this.isJavaField = true;
							checkPublicStatic(env, methodName, javaClass);
						}
						catch (NoSuchFieldException | SecurityException e1) {
						}
						env.error(this.getFirstSymbol(), "A Java method for the message send '" + methodName + "' was not found "
								+ "or there was a security exception (class SecurityException)");
					}
			...
			if ( exprType instanceof TypeWithAnnotations ) {
				List<MethodSignature> mList = Type.Any.searchMethodPublicPackage(methodName, env);
		...
		if ( env.getCurrentMethod() != null ) {
			...
			if ( currentMethodName.equals("init") || currentMethodName.equals("init:") ) {
				 ...
				 * statements. 'iv' is used for a field and 'im' for instance method.
				 *     iv m1;
				 *     im;
				 *     im m1 m2;
				 *     self im;
				...
				if ( this.receiverExprOrFirstUnary instanceof ExprIdentStar ) {
					...
					if ( e.getIdentStarKind() == IdentStarKind.instance_variable_t && ! ((FieldDec ) e.getVarDeclaration()).isShared() ) {
						if ( ! ((FieldDec ) e.getVarDeclaration()).getWasInitialized() ) {
...
//				}
						...
						! (((ExprSelfPeriodIdent ) this.receiverExprOrFirstUnary).getFieldDec().isShared() ) ) {

					if ( ! ((ExprSelfPeriodIdent ) this.receiverExprOrFirstUnary).getFieldDec().getWasInitialized() ) {
						env.error(this.getFirstSymbol(),  "Access to a non-initialized field");
				...
				/**
				 * inside an initShared method it is illegal to access 'self' as in the following
				 * statements. 'iv' is used for a field and 'im' for instance method.
				 *     iv m1;
				 *     im;
				 *     im m1 m2;
				...
				if ( this.receiverExprOrFirstUnary instanceof ExprIdentStar ) {
					...
					if ( e.getIdentStarKind() == IdentStarKind.instance_variable_t  ) {
						env.error(this.getFirstSymbol(),  "Access to a field in an expression inside an 'initShared'. This is illegal because the "
								+ "the field has not been initialized");

					}
					else if ( e.getIdentStarKind() == IdentStarKind.unaryMethod_t ) {
						env.error(this.getFirstSymbol(),  "Message send to 'self' inside an 'initShared' method. This is illegal because 'self' is being used and "
								+ "through it some field that has not been initialized may be accessed");
					}
				}
				else if ( this.receiverExprOrFirstUnary instanceof ast.ExprSelf  ) {
					env.error(this.getFirstSymbol(),  "Message send to 'self' inside an 'initShared' or 'init:' method. This is illegal because "
		...
		if ( hasBackquote ) {
			...
			if ( receiverExprOrFirstUnary == null || receiverExprOrFirstUnary instanceof ExprSelf ) {
				MethodDec currentMethod = env.getCurrentMethod();
				if ( currentMethod != null ) {
					String currentMethodName = currentMethod.getNameWithoutParamNumber();
					if ( currentMethodName.equals("init") || currentMethodName.equals("init:") ) {
		   ...
		   */
			...
			if ( receiverType instanceof TypeUnion ) {
				env.error(this.getFirstSymbol(), "The type of the message receiver, '" + receiverExprOrFirstUnary.asString() +
						"'  is an union type. Then the receiver cannot receive messages", true, true);
			}
			...
			if ( isNew ) {
				...
				if ( receiverExprOrFirstUnary instanceof ExprTypeof ) {
					...
					TupleTwo<String, Type> t = receiverExprOrFirstUnary.ifPrototypeReturnsNameWithPackageAndType(env);
					ok = t != null && t.f2 != null;
				}
				...
				if ( ! ok ) {
					...
					if ( methodSignatureList != null && methodSignatureList.size() > 0 ) {
						MethodDec aMethod = methodSignatureList.get(0).getMethod();
						if ( aMethod != null ) {
							if ( aMethod.getVisibility() == Token.PRIVATE  ) {
								if ( aMethod.getDeclaringObject() != env.getCurrentOuterObjectDec() ) {
									env.error(this.getFirstSymbol(), "This 'new' method is private. It can only be called "
											+ "inside prototype '" + aMethod.getDeclaringObject().getFullName() + "'", true, true);
								}
							}
								...
								if ( aMethod.getDeclaringObject().getCompilationUnit().getCyanPackage() !=
									...
									env.error(this.getFirstSymbol(), "This 'new' method has 'package' visibility. It can only be called "
											+ "inside package '" +
								...
								if ( ! aMethod.getDeclaringObject().isSupertypeOf(env.getCurrentObjectDec(), env)  ) {
									env.error(this.getFirstSymbol(), "This 'new' method has 'protected' visibility. It can only be called "
											+ "in subprototypes of '" +
					...
					if ( methodSignatureList != null && methodSignatureList.size() > 0 ) {
						...
						if ( aMethod != null ) {
							if ( aMethod.getVisibility() == Token.PRIVATE  ) {
								...
								if ( aMethod.getDeclaringObject().getCompilationUnit().getCyanPackage() !=
										env.getCurrentObjectDec().getCompilationUnit().getCyanPackage() ) {
									env.error(this.getFirstSymbol(), "This 'init' method has 'package' visibility. It can only be called "
											+ "inside package '" + aMethod.getDeclaringObject().getCompilationUnit().getCyanPackage().getName() + "'");
								}
							}
							else if ( aMethod.getVisibility() == Token.PROTECTED ) {
								if ( ! aMethod.getDeclaringObject().isSupertypeOf(env.getCurrentObjectDec(), env)  ) {
									env.error(this.getFirstSymbol(), "This 'init' method has 'protected' visibility. It can only be called "
											+ "in subprototypes of '" +
											aMethod.getDeclaringObject().getName() + "'", true, true);
								}
							}
				...
				if ( methodSignatureList == null || methodSignatureList.size() == 0 ) {
					...
					if ( methodSignatureList == null || methodSignatureList.size() == 0 )  {
						...
						if ( exprType == env.getCurrentObjectDec() ) {
							// in a prototype 'ChooseFoldersCyanInstallation', there may be something like
							// 'ChooseFoldersCyanInstallation sgetCount' in which sgetCount is a private shared method
							methodSignatureList = exprType.searchMethodPrivateProtectedPublicPackage(methodName, env);
							if ( methodSignatureList != null && methodSignatureList.size() > 0 ) {
...
//								}
			...
			if ( aMethod != null ) {
				if ( aMethod.getShared() ) {
					 ...
					 */
					if ( !(this.receiverExprOrFirstUnary instanceof ExprIdentStar)
							|| ((ExprIdentStar ) this.receiverExprOrFirstUnary).getIdentStarKind()
							!= IdentStarKind.prototype_t ) {
					...
					if ( receiverType instanceof Prototype ) {
						...
						if ( (initMSList == null || initMSList.size() == 0) &&
							...
							if ( !canBeCalledOnPrototypes ) {
								env.error(getFirstSymbol(), "Prototype '" + this.receiverExprOrFirstUnary.asString() + "' does "
			...
			/**
			 * if the method is 'new', then the checking of metaobjects attached to 'init' should be used.
			 *
			 * methodSignatureList = ((ObjectDec) exprType).searchInitNewMethod(methodName);
			 */
			...
			if ( isNew ) {
				initMethodSignatureList = ((ObjectDec) exprType).searchInitNewMethod("init");
				if ( env.getProject().getCompilerManager().getCompilationStep() == CompilationStep.step_9 ) {
					MetaInfoServer.checkMessageSendWithMethodMetaobject(initMethodSignatureList, exprType, receiverExprOrFirstUnary,
							receiverKind, env, this.unarySymbol);
				}

				if ( env.getProject().getCompilerManager().getCompilationStep().ordinal() < CompilationStep.step_7.ordinal() ) {
					type = MetaInfoServer.replaceMessageSendIfAsked(initMethodSignatureList.get(0),
							this,
				...
				if ( env.getProject().getCompilerManager().getCompilationStep() == CompilationStep.step_9 ) {
					MetaInfoServer.checkMessageSendWithMethodMetaobject(allMethodSignatureList, exprType, receiverExprOrFirstUnary,
	...
	private void checkPublicStatic(Env env, String methodName,
		...
		if ( Modifier.isStatic(modif) ) {
			// method  is static
			if ( (this.receiverExprOrFirstUnary instanceof ExprIdentStar &&
					((ExprIdentStar) this.receiverExprOrFirstUnary).getIdentStarKind() != IdentStarKind.jvmClass_t) &&
					!(this.receiverExprOrFirstUnary instanceof ast.ExprGenericPrototypeInstantiation) ) {
				env.error(getFirstSymbol(), "Static fields can only be used with a class as in '" +
						receiverExprOrFirstUnary.getType().getFullName() + "." + methodName + "'");
			}
		}
		else if ( ! Modifier.isPublic(modif) ) {
			env.error(getFirstSymbol(), "The field '" + methodName + "' of the Java class '"
...
//							this.receiverExprOrFirstUnary.getI(), this.unarySymbol.getI(), env.getI());
					...
					if ( !(f1 instanceof CyString) || !(f2 instanceof CyString) || !(f3 instanceof CyString) ) {
						env.error(annot.getFirstSymbol(), "This metaobject is returning a "
	...
	private Type getJavaTypeForMessageSend(Class<?> methodReturnType, Env env) {
		...
		if ( NameServer.isJavaBasicType(javaReturnTypeName )  ) {
			return env.getProject().getProgram().searchJavaBasicType(javaReturnTypeName);
		}
		else {
			...
			if ( jvmPackage == null ) {
				...
				if ( t == null ) {
					env.error(this.getFirstSymbol(), "The Java class of the return type of this method call is '"
							+ methodReturnType.getName() + "'. It was not found in its package. Maybe the package was not imported");
					return null;
				}
				else {
					if ( isArray ) {
	...
	public Object eval(EvalEnv ee) {
		 ...
		 *    cyanValue unary;
		 *    CyanProto unary;
		 *    cyan.lang.CyanProto unary;
		 *
		 *    javaValue unary;
		 *    StringBuffer unary;
		 *    java.lang.StringBuffer unary;
		...
		if ( receiverValue == null ) {
			...
			if ( isNew ) {
				...
				try {
					...
					ee.error(this.getFirstSymbol(), "Error when trying to create object from '" + rname + "'");
				}
			}
			else {
        ...
        ret = Statement.sendMessage(receiverValue, cyanMethodNameInJava,  methodNameInJava,
				null, this.unarySymbol.getSymbolString(), this.getFirstSymbol().getI(), ee );

		/*
        java.lang.reflect.Method unaryMethod = EvalEnv.getJavaMethodByName(receiverClass, cyanMethodNameInJava, 0);
        if ( unaryMethod != null ) {
            try {
                unaryMethod.setAccessible(true);
                ret = unaryMethod.invoke(receiverValue);
            }
            catch ( java.lang.reflect.InvocationTargetException tmp2359 ) {
            	ee.error(this.getFirstSymbol(), "InvocationTargetException exception "
            			+ "thrown by unary message passing '" +  this.asString() + "'");
            }
            catch (IllegalAccessException | IllegalArgumentException tmp2359) {
            	ee.error(this.getFirstSymbol(), "IllegalAccessException or IllegalArgumentException exception "
            			+ "thrown by unary message passing '" +  this.asString() + "'");
            }
        }
        */
