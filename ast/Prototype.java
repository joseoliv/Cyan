 ...
 *
 */
public abstract class Prototype extends Type implements Declaration, ASTNode, Cloneable {


	public Prototype(Token visibility, List<AnnotationAt> nonAttachedAnnotationList,
			List<AnnotationAt> attachedAnnotationList,  ObjectDec	outerObject) {

		Prototype.initPrototype(this, visibility, nonAttachedAnnotationList, attachedAnnotationList, outerObject);
...
//        this.this_and_all_superPrototypes = null;
	...
	public void calcInternalTypes(ICompiler_semAn compiler_semAn, Env env) {
		...
		for ( AnnotationAt annotation : this.getCompilationUnit().getCyanPackage().getAttachedAnnotationList() ) {
			annotation.setDeclaration(wrpu);
			annotation.semAnActions(env);
		}
		for ( AnnotationAt annotation : this.getCompilationUnit().getCyanPackage().getProgram().getAttachedAnnotationList() ) {
			annotation.setDeclaration(wrpu);
			annotation.semAnActions(env);
		}
		if ( this.getPrototypePackageProgramAnnotationList().size() > 0 ) {
			...
			if ( this instanceof ObjectDec ) {
				...
				meta.GetHiddenItem.getHiddenEnv(compiler_semAn.getEnv()).atBeginningOfObjectDec(objDec);
			}
			...
			for ( Annotation annotation : annotList ) {
				...
				if ( metaobject instanceof ICheckPrototype_bsa
						||
						(other != null && other instanceof _ICheckPrototype__bsa)
						) {
	 ...
	 * This list includes attachedAnnotationList and beforeEndNonAttachedAnnotationList.
	...
	public List<Annotation> getPrototypePackageProgramAnnotationList() {
		if ( prototypePackageProgramAnnotationList == null ) {
			...
			prototypePackageProgramAnnotationList.addAll(completeAnnotationList);
			...
			annotList = this.getCompilationUnit().getCyanPackage().getProgram().getAttachedAnnotationList();
			if ( annotList != null ) {
				for (AnnotationAt withAt : annotList ) {
					if ( ! (withAt instanceof IAction_dpp) ) {
						prototypePackageProgramAnnotationList.add(withAt);
	...
	public void afterSemAn_checkDeclaration(ICompiler_semAn compiler_afterResTypes, Env env) {
		...
		for ( Annotation annotation : annotList ) {
			...
			if ( metaobject instanceof ICheckDeclaration_afterSemAn
					||
					(other != null && other instanceof _ICheckDeclaration__afterSemAn)
					) {
				...
				if ( annotation instanceof AnnotationAt ) {
					if (((AnnotationAt) annotation).getI() == null ) {
						env.error(annotation.getFirstSymbol(),
								...
								+ "implements interface '" + ICheckDeclaration_afterSemAn.class.getName() + "'");
					}
				}
...
//						fp.afterSemAn_checkDeclaration(compiler_afterResTypes);
						...
						to.run( Executors.callable(
								() -> {
									((_ICheckDeclaration__afterSemAn ) other)._afterSemAn__checkDeclaration_1(compiler_afterResTypes);
								} ),
								timeoutMilliseconds, "afterSemAn_checkDeclaration",
								metaobject, env);
//						((_ICheckDeclaration__afterSemAn ) other)._afterSemAn__checkDeclaration_1(compiler_afterResTypes);
				...
				if ( method.getVisibility() == Token.PUBLIC && method.getHasOverride() ) {
					...
					for ( MethodSignature ms : superMS ) {
						...
						if ( superMethod == null ) {
							 ...
							 */
							superMSAttachedAnnotationList = ms.getAttachedAnnotationList();
						}
						else {
							/*
							 * method of a superprototype of 'this' or 'proto'
							 */
							superMSAttachedAnnotationList = superMethod.getAttachedAnnotationList();
						}
						if ( superMSAttachedAnnotationList != null ) {
							for ( AnnotationAt annotation : superMSAttachedAnnotationList ) {
								...
								if ( metaobject instanceof ICheckOverride_afterSemAn
										||
										(other != null && other instanceof _ICheckOverride__afterSemAn)
										) {
									...
									try {
										if ( other == null ) {
											...
											to.run( Executors.callable(
													() -> {
														fp.afterSemAn_checkOverride(compiler_semAn, method.getI());
													} ),
													timeoutMilliseconds, "afterSemAn_checkOverride",
													metaobject, env);
...
//											fp.afterSemAn_checkOverride(compiler_semAn, method.getI());
											...
											to.run( Executors.callable(
													() -> {
														fp._afterSemAn__checkOverride_2(compiler_semAn, method.getI());
													} ),
													timeoutMilliseconds, "afterSemAn_checkOverride",
													metaobject, env);
//											fp._afterSemAn__checkOverride_2(compiler_semAn, method.getI());
	 ...
	 * {@link meta.IAction_afterResTypes} or {@link meta.IActionNewPrototypes_afterResTypes}
	 */
	protected void makeAnnotationsCommunicateInPrototype(
		...
		for ( Annotation annotation : metaobjectAnnotationList ) {
			...
			if ( cyanMetaobject instanceof ICommunicateInPrototype_afterResTypes_semAn_afterSemAn
					||
					(other != null && other instanceof _ICommunicateInPrototype__afterResTypes__semAn__afterSemAn)
					) {
				...
				if ( other == null ) {
					sharedInfo = ((ICommunicateInPrototype_afterResTypes_semAn_afterSemAn ) cyanMetaobject).afterResTypes_semAn_afterSemAn_shareInfoPrototype(wrEnv);
				}
				else {
					sharedInfo = ((_ICommunicateInPrototype__afterResTypes__semAn__afterSemAn ) other)._afterResTypes__semAn__afterSemAn__shareInfoPrototype_1(wrEnv);
				}
				if (  sharedInfo != null ) {
		...
		if ( moInfoSet.size() > 0 ) {
			for ( Annotation annotation : metaobjectAnnotationList ) {
				...
				if ( cyanMetaobject instanceof ICommunicateInPrototype_afterResTypes_semAn_afterSemAn
					...
					// // cyanMetaobject.setAnnotation(annotation, 0);
					if ( other == null ) {
						((ICommunicateInPrototype_afterResTypes_semAn_afterSemAn ) cyanMetaobject).afterResTypes_semAn_afterSemAn_receiveInfoPrototype(moInfoSet, wrEnv);
					}
					else {
						_Set_LT_GP__Tuple_LT_GP_CyString_GP_CyInt_GP_CyInt_GP_Object_GT_GT tupleSet =
								new _Set_LT_GP__Tuple_LT_GP_CyString_GP_CyInt_GP_CyInt_GP_Object_GT_GT();
						...
						((_ICommunicateInPrototype__afterResTypes__semAn__afterSemAn ) other)._afterResTypes__semAn__afterSemAn__receiveInfoPrototype_2(
								tupleSet, wrEnv);
					}
...
//	private void afterResTypes_actions(ICompiler_afterResTypes compiler_afterResTypes, CompilerManager_afterResTypes compilerManager) {
//
...
//
//			/*
//			 * if the metaobject annotation has a suffix greater or equal to "AFTER_RES_TYPES" then the actions below
//			 * have already been taken (the compiler changed the suffix to "AFTER_RES_TYPES" or greater) or
...
//
//				addCodeAndSlotsTo(compiler_afterResTypes, compilerManager, env, hasLessThanInName, cyanAnnotation);
//
//
//			}
//
...
//	}
	...
	public void afterResTypes_actions(ICompiler_afterResTypes compiler_afterResTypes,
			CompilerManager_afterResTypes compilerManager) {

		Env env = meta.GetHiddenItem.getHiddenEnv(compiler_afterResTypes.getEnv());
		env.atBeginningOfObjectDec(this);

		WrProgram program = compiler_afterResTypes.getEnv().getProject().getProgram();
		WrCyanPackage cpackage = compiler_afterResTypes.getCompilationUnit().getCyanPackage();
...
//
         ...
         * </code>
         */
        List<Tuple4<Annotation, StringBuffer, List<ISlotSignature>, String>>
        	runManyList = null, runOnceList = null;


		List<Tuple2<WrAnnotation, List<ISlotSignature>>> infoListFist =
				new ArrayList<>();
		...
		for ( Annotation annot : metaobjectAnnotationList ) {
			...
			if ( annot.getPostfix() != null &&
				 !annot.getPostfix().lessThan(CompilerPhase.AFTER_RES_TYPES) ) {
				continue;
			}
			...
			if ( cyanMetaobject instanceof IActionNewPrototypes_afterResTypes
					||
					(other != null && other instanceof _IActionNewPrototypes__afterResTypes)
					) {
				CompilerManager.createNewPrototypes(compiler_afterResTypes, env, annot, cyanMetaobject, this.compilationUnit);
			}


			if ( cyanMetaobject instanceof IAction_afterResTypes
					|| (other != null && other instanceof _IAction__afterResTypes)
					) {
				...
				try {
					...
					if ( other == null ) {
						...
						codeSlot = to.run(
								...
								.afterResTypes_codeToAdd(compiler_afterResTypes, infoListFist),
...
//						_Tuple_LT_GP_CyString_GP_CyString_GT t = ((_IAction__afterResTypes ) other)
//								._afterResTypes__codeToAdd_2(compiler_afterResTypes,
//										new _Array_LT_GP__Tuple_LT_GP_Object_GP__Array_LT_GP_Object_GT_GT_GT());
						Timeout<_Tuple_LT_GP_CyString_GP_CyString_GT> to = new Timeout<>();
						_Tuple_LT_GP_CyString_GP_CyString_GT t =
								to.run( () -> ((_IAction__afterResTypes ) other)
								._afterResTypes__codeToAdd_2(compiler_afterResTypes,
					...
					if ( codeSlot == null ) {
						...
						if ( cyanMetaobject instanceof IAction_afterResTypes )  {
							boolean runUntilFixedPoint = to.run(
									() -> ((IAction_afterResTypes ) cyanMetaobject).runUntilFixedPoint(),
									timeoutMilliseconds, "runUntilFixedPoint", cyanMetaobject, env);
//							boolean runUntilFixedPoint = ((IAction_afterResTypes ) cyanMetaobject).runUntilFixedPoint();
							if ( runUntilFixedPoint ) {
								env.error(annot.getFirstSymbol(), "Method for generating code in phase AFTER_RES_TYPES of " +
							...
							boolean runUntilFixedPoint = to.run(
									() -> ((_IAction__afterResTypes ) other)._runUntilFixedPoint(),
									timeoutMilliseconds, "runUntilFixedPoint",
									cyanMetaobject, env);

							// boolean runUntilFixedPoint = ((_IAction__afterResTypes ) other)._runUntilFixedPoint();
							if ( runUntilFixedPoint ) {
								env.error(annot.getFirstSymbol(), "Method for generating code in phase AFTER_RES_TYPES of " +
							            "metaobject of annotation '"
										+ cyanMetaobject.getName() + "' returned null. This is illegal because"
					...
					else
					{
						...
						Tuple4<Annotation, StringBuffer, List<ISlotSignature>, String> t =
								new Tuple4<Annotation, StringBuffer, List<ISlotSignature>, String>(
...
//							boolean runUntilFixedPoint = ((_IAction__afterResTypes ) other)._runUntilFixedPoint();
						...
						infoList.add( new Tuple2<WrAnnotation, List<ISlotSignature>>(
								annot.getI(), slotList)
								);
					}
				}
				...
				try {
					if ( cyanMetaobject instanceof IAction_afterResTypes ) {
						...
						statsList = to.run(
								() -> ((IAction_afterResTypes ) cyanMetaobject).afterResTypes_beforeMethodCodeList(compiler_afterResTypes) ,
								timeoutMilliseconds, "afterResTypes_beforeMethodCodeList",
								cyanMetaobject, env);
//						statsList = ((IAction_afterResTypes ) cyanMetaobject).afterResTypes_beforeMethodCodeList(compiler_afterResTypes);
					}
					else if ( other != null ) {
						...
						_Array_LT_GP__Tuple_LT_GP_CyString_GP_CyString_GP_CyBoolean_GT_GT array =
							to.run(
							  () -> ((_IAction__afterResTypes ) other)._afterResTypes__beforeMethodCodeList_1( compiler_afterResTypes),
							  timeoutMilliseconds, "afterResTypes_beforeMethodCodeList",
							  cyanMetaobject, env);
				...
				try {
					if ( cyanMetaobject instanceof IAction_afterResTypes ) {
						...
						renameMethodList = to.run(
								() -> ((IAction_afterResTypes ) cyanMetaobject).afterResTypes_renameMethod(compiler_afterResTypes),
								timeoutMilliseconds, "afterResTypes_renameMethod",
								cyanMetaobject, env);
//						renameMethodList = ((IAction_afterResTypes ) cyanMetaobject).afterResTypes_renameMethod(compiler_afterResTypes);
					}
					else if ( other != null ) {
						Timeout<_Array_LT_GP__Tuple_LT_GP_CyString_GP__Array_LT_GP_CyString_GT_GT_GT> to = new Timeout<>();

						_Array_LT_GP__Tuple_LT_GP_CyString_GP__Array_LT_GP_CyString_GT_GT_GT array = to.run(
								() -> ((_IAction__afterResTypes ) other)._afterResTypes__renameMethod_1(compiler_afterResTypes),
								timeoutMilliseconds, "afterResTypes_renameMethod", cyanMetaobject, env);
...
//						}
		...
		if ( runManyList != null && runManyList.size() > 0 ) {
			...
			while ( somethingChanged ) {
	        	somethingChanged = false;
	    		for ( Tuple4<Annotation, StringBuffer, List<ISlotSignature>, String>
    				...
    				try {
    					...
    					if ( other == null ) {
        					...
        					codeSlot = to.run(
            						() -> ((IAction_afterResTypes ) cyanMetaobject)
        							.afterResTypes_codeToAdd(compiler_afterResTypes, infoListFinal) ,
            						timeoutMilliseconds, "afterResTypes_codeToAdd",
            						cyanMetaobject, env);
//        					codeSlot = ((IAction_afterResTypes ) cyanMetaobject)
//        							.afterResTypes_codeToAdd(compiler_afterResTypes, infoList);
    					}
    					else {
    						// from List<Tuple2<WrAnnotation, List<ISlotSignature>>>
    						// to   _Array_LT_GP__Tuple_LT_GP_Object_GP__Array_LT_GP_Object_GT_GT_GT
    						_Array_LT_GP__Tuple_LT_GP_meta_p_WrAnnotation_GP__Array_LT_GP_meta_p_ISlotSignature_GT_GT_GT x;

    						_Array_LT_GP__Tuple_LT_GP_meta_p_WrAnnotation_GP__Array_LT_GP_meta_p_ISlotSignature_GT_GT_GT arrayTuple =
    								new _Array_LT_GP__Tuple_LT_GP_meta_p_WrAnnotation_GP__Array_LT_GP_meta_p_ISlotSignature_GT_GT_GT();
//    						_Array_LT_GP__Tuple_LT_GP_Object_GP__Array_LT_GP_Object_GT_GT_GT arrayTuple =
//    								new _Array_LT_GP__Tuple_LT_GP_Object_GP__Array_LT_GP_Object_GT_GT_GT();
...
//        							_Array_LT_GP_Object_GT arraySlot = new _Array_LT_GP_Object_GT();
        							...
        							arrayTuple._add_1(
        									new _Tuple_LT_GP_meta_p_WrAnnotation_GP__Array_LT_GP_meta_p_ISlotSignature_GT_GT(t.f1, arraySlot)
        									);


//        							arrayTuple._add_1(
//        									new _Tuple_LT_GP_Object_GP__Array_LT_GP_Object_GT_GT(t.f1, arraySlot)
//        									);
        						}
    						}
        					Timeout<_Tuple_LT_GP_CyString_GP_CyString_GT> to = new Timeout<>();
        					...
        					_Tuple_LT_GP_CyString_GP_CyString_GT t =
        					        to.run( () -> ((_IAction__afterResTypes ) other)
            								._afterResTypes__codeToAdd_2(compiler_afterResTypes, arrayTuple),
            								timeoutMilliseconds, "afterResTypes_codeToAdd",
            								cyanMetaobject, env);
...
//    								._afterResTypes__codeToAdd_2(compiler_afterResTypes, arrayTuple);
    					...
    					if ( codeSlot == null ) {
							env.error(annot.getFirstSymbol(), "Method to generate code in phase AFTER_RES_TYPES of " +
						            "metaobject of annotation '"
									+ cyanMetaobject.getName() + "' returned null. This is illegal because"
											+ " method 'runUntilFixedPoint()' of this metaobject returned true");
    					}
	...
	private static int getMaxNumRounds(Env env) {
		...
		Object obj = env.getCurrentCompilationUnit().getCyanPackage().getPackageValueFromKey(MetaHelper.maxnumroundsfixmetastr);
		if ( obj != null ) {
			if ( obj instanceof Integer ) {
				...
				env.error(env.getCurrentPrototype().getFirstSymbol(),
						"Internal error: variable " + MetaHelper.maxnumroundsfixmetastr + "'"
						+ " should have an Integer value");
			}
		}
		else {
			obj = env.getProject().getProgramValueFromKey(MetaHelper.maxnumroundsfixmetastr);
			if ( obj != null ) {
				if ( obj instanceof Integer ) {
					maxNumRounds = (Integer ) obj;
				}
	 ...
	 * return a list of objects of WrMethodSignature and WrFieldDec
	   ...
	   @return
	 */
	public List<ISlotSignature> extractSlotListFrom(String slotCode, Annotation annot,
		...
		try {
			...
			while ( cp.getSymbol().token == Token.IDENT ||
				...
				if ( cp.getSymbol().token == Token.FUNC ||
					...
					boolean overloadKeyword = cp.getSymbol().token == Token.OVERLOAD;
					if ( overloadKeyword ) {
						cp.next();
					}
					boolean finalKeyword = cp.getSymbol().token == Token.FINAL;
					boolean abstractKeyword = cp.getSymbol().token == Token.ABSTRACT;
					if ( finalKeyword ) {
						cp.next();
						if ( cp.getSymbol().token != Token.FUNC ) {
							env.error(errSymbol, "Syntax error in the list of slots that metaobject '" +
...
//	   @param annotation
//	 */
//	private void addCodeAndSlotsTo(ICompiler_afterResTypes compiler_afterResTypes, CompilerManager_afterResTypes compilerManager, Env env,
...
//
//
//		CompilerManager.createNewPrototypes(compiler_afterResTypes, env, annotation, cyanMetaobject, this.compilationUnit);
//
//		if ( cyanMetaobject instanceof IAction_afterResTypes ) {
//
//
...
//
//			try {
//				code = ((IAction_afterResTypes ) cyanMetaobjectWithAt).afterResTypes_codeToAdd(compiler_afterResTypes);
//			}
//			catch ( error.CompileErrorException e ) {
//			}
//			catch ( NoClassDefFoundError e ) {
...
//
				...
				if ( ((ObjectDec) current).getInterfaceList() != null ) {
					for ( Expr inter : ((ObjectDec) current).getInterfaceList() ) {
						if ( ! puSet.contains(inter.getType()) ) {
							puStack.add( (Prototype ) inter.getType().getInsideType() );
						}
					}
				}
			}
	...
	private void checkInheritance_afterSemAn(ICompiler_semAn compiler_semAn, Env env) {
		...
		for ( Prototype pu : allSuperPrototypes2 ) {
			...
			for ( Annotation cyanAnnotation : annotList ) {
				CyanMetaobject cyanMetaobject = cyanAnnotation.getCyanMetaobject();

				_CyanMetaobject other = cyanMetaobject.getMetaobjectInCyan();
				if ( cyanMetaobject instanceof ICheckSubprototype_afterSemAn
						||
						(other != null && other instanceof _ICheckSubprototype__afterSemAn)
						) {
					...
					try {
						...
						if ( other == null ) {
							...
							to.run( Executors.callable(
									() -> { ((ICheckSubprototype_afterSemAn ) cyanMetaobject).afterSemAn_checkSubprototype(
												compiler_semAn, this.getI());

									} ),
									timeoutMilliseconds, "afterSemAn_checkSubprototype",
									cyanMetaobject, env);
...
//									compiler_semAn, this.getI());
							...
							to.run( Executors.callable(
									() -> { ((_ICheckSubprototype__afterSemAn ) other)._afterSemAn__checkSubprototype_2(
											compiler_semAn, this.getI());
									} ),
									timeoutMilliseconds, "afterSemAn_checkSubprototype",
									cyanMetaobject, env);
...
//									compiler_semAn, this.getI());
						...
						env.error(cyanAnnotation.getFirstSymbol(), e.getMessage() + " " + NameServer.messageClassNotFoundException);
					}
					catch ( RuntimeException e ) {
						// e.printStackTrace();
						env.thrownException(cyanAnnotation, cyanAnnotation.getFirstSymbol(), e);
					}
					finally {
	...
	public Expr asExpr(Symbol seed) {
		...
		if ( genericParameterListList != null && this.genericParameterListList.size() > 0 ) {
			...
			ExprGenericPrototypeInstantiation gpi = new ExprGenericPrototypeInstantiation(
					newIdentStar,
					realTypeListList, null, null, null);
			return gpi;
		}
		else {
			return newIdentStar;
	 ...
	 * demand
	 */
	protected void genJavaClassBodyDemandedByAnnotations(PWInterface pw, Env env) {
	...
	public void setBeforeEndNonAttachedAnnotationList(List<AnnotationAt> beforeEndNonAttachedAnnotationList) {
		...
		if ( this.attachedAnnotationList != null ) {
			for ( AnnotationAt annotation : attachedAnnotationList ) {
				...
				if ( cyanMetaobject instanceof meta.IAction_cge ) {
					// // annotation.getCyanMetaobject().setAnnotation(annotation, 0);
	...
	/**
	   @param pw
	 */
	protected void genJavaCodeStaticSectionAnnotations(PWInterface pw, Env env) {
		/*
	...
	*/
	...
	public void addDependentPrototype( Prototype dependentPrototype ) {
		...
		this.dependentPrototypeMap.put(dependentPrototype.getFullName(), dependentPrototype);
	}
	 ...
	 * phase AFTER_RES_TYPES. These are the types of metaobject annotations
	 */
	 ...
	 * +, -, *, /, %, <<, >>, or >.>>
	 * Currently, interfaces cannot define operator methods.
	 */
