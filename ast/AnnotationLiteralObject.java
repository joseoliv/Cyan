 ...
 *
 * [* 1:2, 3:1, 1:3 *] or n"c:\table\cup\file.txt" or r"[A-Z][0-9]+"
 *
 * @author José
 */
public class AnnotationLiteralObject extends Annotation {

	public AnnotationLiteralObject(CompilationUnitSuper compilationUnit,
			Prototype prototype,
	...
	public void genCyanReal(PWInterface pw, boolean printInMoreThanOneLine,
		...
		/*
		 * if ( replaceCodeByThis != null ) { pw.print(this.replaceCodeByThis);
		 * } else {}
		 */
	...
	/**
	 * The metaobject annotation has type
	 * <code>packageName.prototypeName</code>. This method returns
	 * <code>packageName</code> which may be a list of identifiers separated by
	 * dots.
	 *
	 * @return
	 */
	...
	public void calcInternalTypes(Env env) {
		...
		if ( !env.getCompInstSet()
			...
			/*
			 * literal objects should only exist till phase 6 of the compiler.
			 * After that they should be removed from the code.
			 */
			env.error(true, this.getFirstSymbol(), "Internal error", null,
			...
			try {
				...
				try {
					...
					_CyanMetaobjectLiteralObject other = (_CyanMetaobjectLiteralObject) cyanMetaobjectLiteralObject
							.getMetaobjectInCyan();
					if ( other == null ) {
						if ( Saci.timeLimitForMetaobjects ) {
							...
							cyanCode = cyanMetaobjectLiteralObject
									.semAn_codeToAdd(compiler_semAn);
						}

						// cyanCode =
						// cyanMetaobjectLiteralObject.semAn_codeToAdd(compiler_semAn);
					}
					else {
						cyanCode = to.run(() -> {
					...
					env.errorInMetaobjectCatchExceptions(
							cyanMetaobjectLiteralObject);
				}

				if ( cyanCode != null ) {
					type = Compiler.singleTypeFromString(
							cyanMetaobjectLiteralObject.getPackageOfType() + "."
									+ cyanMetaobjectLiteralObject
											.getPrototypeOfType(),
							this.getFirstSymbol(), "Error in literal object: ",
					...
					if ( this.isParsedWithCompiler() && env
						...
						/*
						 * literal objects are always removed from the source
						 * code
						 */
						...
						this.codeThatReplacesThisStatement = new StringBuffer(
								env.addCodeAtAnnotation(
										cyanMetaobjectLiteralObject, cyanCode,
										-1));
					}
				}
				...
				if ( this.cyanMetaobjectLiteralObject instanceof IActionNewPrototypes_semAn
					...
					try {
						...
						env.errorInMetaobjectCatchExceptions(
								this.cyanMetaobjectLiteralObject);
					}
	...
	public WrAnnotationLiteralObject getI() {
		if ( iCyanMetaobjectLiteralObjectAnnotation == null ) {
			iCyanMetaobjectLiteralObjectAnnotation = new WrAnnotationLiteralObject(
					this);
		}
	...
	public List<Tuple4<Integer, Integer, Integer, Integer>> getColorTokenList() {
		if ( colorTokenList == null ) {
			if ( this.cyanMetaobjectLiteralObject != null ) {
				colorTokenList = this.cyanMetaobjectLiteralObject
						.getColorTokenList(this.getI());
			}
		}
	...
	/**
	 * the original text of the metaobject annotation. In "101bin" it is
	 * "101bin". In "[* (1, 2), (2, 3), (3, 1) *]" it should be "(1, 2), (2, 3),
	 * (3, 1)" or "[* (1, 2), (2, 3), (3, 1) *]" (not sure which!!!).
	 */
