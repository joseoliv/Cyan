...
public class StatementIf extends Statement {
	...
	public void calcInternalTypes(Env env) {
		...
		for ( StatementList statement : ifStatementList ) {
			...
			if ( ! statement.alwaysReturn() ) {

				List<LocalVarInfo> list = env.getLocalVarInfoPreviousLevel();

				if ( !env.isEmptyRepetitionStatStack() && env.peekRepetitionStatStack() == 'r' && breakIsLastStatement ) {
					/**
					 * if the statement list ends with 'break', clean it from all variables
					 * of the statement list lexical level
					 */
				...
				/* else if (  !env.isEmptyRepetitionStatStack() && env.peekRepetitionStatStack() == 'r' ) {
					/*
					 * there is a break in the last statement and this 'if' is inside a repeat-until statement.
					 * All assignments of this branch should be considered valid
					 * /
		...
		if ( elseStatementList != null ) {
			...
			if ( !this.elseStatementList.alwaysReturn() ) {
