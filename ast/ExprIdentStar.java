...
//					if ( !env.getProjectResTypes() && !name.equals(NameServer.THISPROTOTYPE999) ) {
//					}
					return;
				}

			}
			else if ( env.getCurrentMethod() == null ) { //&& !name.equals(NameServer.THISPROTOTYPE999) ) {
...
//							}
		...
		if ( varDec != null ) {
			...
			if ( leftHandSideAssignment ) {
				...
				if ( varDec instanceof FieldDec ) {
					if (((FieldDec ) varDec).isReadonly() && ! env.getCurrentMethod().isInitMethod() && ! env.getCurrentMethod().isInitShared() ) {
						env.error(getFirstSymbol(),
		...
		if ( varDec != null ) {
						...
						if ( !fieldDec.isShared() ) {
							env.error(this.getFirstSymbol(),
									"Shared methods cannot access non-shared fields. Method '" +
											currentMethod1.getName() + "' is accessing field '" + fieldDec.getName() + "'");
						}
					}
			...
			if ( leftHandSideAssignment ) {
				...
				if ( currentMethodName.equals("initShared") ) {
					env.error( this.getFirstSymbol(),  "Illegal access to a field in an expression inside an 'initShared' method",
							true, true);
				}
			...
			if ( methodSignatureList != null  && methodSignatureList.size() > 0) {
				...
				if ( currentMethodName.equals("init") || currentMethodName.equals("init:") ) {
					if ( ! MethodDec.hasAttachedAnnotationNamed(methodSignatureList,
						...
						FieldDec fieldList = ((ObjectDec ) currentPrototype).searchField(nameiv);
						if ( fieldList != null ) {
				...
				identStarKind = IdentStarKind.unaryMethod_t;
				MethodDec currentMethod1 = env.getCurrentMethod();
				MethodDec methodFound = methodSignatureForMessageSend.getMethod();

				if ( methodSignatureForMessageSend.getMethod().getShared() ) {
					if ( env.getCurrentObjectDec() != methodFound.getDeclaringObject() ) {
