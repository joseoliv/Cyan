...
public class StatementAssignmentList extends Statement {
	...
	public void genJava(PWInterface pw, Env env) {
        ...
        if ( leftSide instanceof ExprIndexed ) {
        	...
        	*/
			//MethodSignatureWithKeywords ms = leftSideIndexedExpr.getIndexingMethod();
			final Type mustBeTypeIndex = leftSideIndexedExpr.getDeclaredType_at_Parameter();
			final Type mustBeRightExprType = leftSideIndexedExpr.getDeclaredType_put_Parameter();
			 ...
			 */
   			...
   			if ( indexedExpr.getType() != Type.Dyn && indexOfExpr.getType() != Type.Dyn ) {
                ...
                if ( mustBeTypeIndex == Type.Any && indexOfExpr.getType() instanceof InterfaceDec ) {
                	indexOfExprTmpVar = " (" + MetaHelper.AnyInJava + " ) " + indexOfExprTmpVar;
                }
                ...
                */
                s = Type.genJavaExpr_CastJavaCyan(env, s, rightType, mustBeRightExprType, getFirstSymbol());
    		...
    		if ( rightType == Type.Dyn && leftType != Type.Dyn ) {
                ...
                rightExprTmpVar = Type.genJavaExpr_CastJavaCyan(env, rightExprTmpVar, rightType, leftType, getFirstSymbol());

                ((LeftHandSideAssignment ) leftSide).genJavaCodeVariable(pw, env);
                if ( leftType == Type.Any && rightType instanceof InterfaceDec ) {
               		pw.println(" = (" + MetaHelper.AnyInJava + " ) " + rightExprTmpVar + ";");
                }
                else {
                ...
                if ( leftType instanceof TypeJavaRef && rightType.getInsideType() instanceof Prototype ) {
					...
					if ( NameServer.isBasicType(puName) ) {
						...
						rightExprTmpVar = rightExprTmpVar + "." + NameServer.getFieldBasicType(puName);
					}
                    ((LeftHandSideAssignment ) leftSide).genJavaCodeVariable(pw, env);
               		pw.println(" = " + rightExprTmpVar + ";");
                }
					...
					// **********************
               		...
               		if ( leftType instanceof ObjectDec ) {
               			ObjectDec leftObjType = (ObjectDec ) leftType;
                   		String thisName = leftType.getName();
    					int indexOfLessThan = thisName.indexOf('<');
    					if ( indexOfLessThan > 0 ) {
    						if ( thisName.subSequence(0,  indexOfLessThan).equals("Array") ) {
    							// an array in the left hand side. The right-hand side may be a Java Array
    							String otherCanonicalName = ((TypeJavaClass) rightType).getaClass(env, getFirstSymbol()).getCanonicalName();
    							if ( otherCanonicalName.endsWith("[]") ) {
    								String arrayElementName = otherCanonicalName.substring(0, otherCanonicalName.length()-2);
    								if ( arrayElementName.startsWith("java.lang") ) {
    									arrayElementName = arrayElementName.substring("java.lang".length());
    								}
    								Type thisArrayElementType =  leftObjType.getGenericParameterListList().get(0).get(0).getParameter().getType();
    								if ( TypeJavaRef.cyanTypeIsSupertypeofJavaTypeByName(thisArrayElementType.getName(), arrayElementName) ) {
    									String tmpCyanArray = NameServer.nextJavaLocalVariableName();
    									pw.printlnIdent(leftType.getJavaName() + " " + tmpCyanArray + " = " + leftType.getJavaName() + "();");
    									String tmpCyanArrayElem = NameServer.nextJavaLocalVariableName();
    									pw.printlnIdent("for ( " + thisArrayElementType.getJavaName() + " " + tmpCyanArrayElem + " : " +
    											rightExprTmpVar + ") {");
    									pw.add();
    									pw.printlnIdent(tmpCyanArray + "._add(" + tmpCyanArrayElem + ");");
    									pw.sub();
    									pw.printlnIdent("}");
    									arrayLeftRight = true;
    									rightExprTmpVar = tmpCyanArray;
    								}
    							}
    						}
    					}
               		...
               		if ( !arrayLeftRight ) {
                    	// cyan = java
    					String javaClass = rightType.getName();
    					rightExprTmpVar = "(new " + NameServer.cyanNameFromJavaBasicType(javaClass) + "(" + rightExprTmpVar + "))";
	...
	static void addLocalVariableTypeInfo(PWInterface pw, Env env,
		...
		if ( e.getIdentSymbolArray().size() == 1 && e.getVarDeclaration() != null ) {
	...
	public void calcInternalTypes(Env env) {
				...
				if ( fieldDec == null ) {
					...
					if ( cm != null && cm.getShared() ) {
						if ( !fieldDec.isShared() ) {
							...
							cm.getName() + " is accessing field '" + fieldDec.getName() + "'");
						}
					}
				}
			}
		...
		for (int i = size - 1; i >= 1; i--) {
			...
			if ( leftExpr instanceof ExprIdentStar && ((ExprIdentStar ) leftExpr).getIdentStarKind() == IdentStarKind.variable_t
					&& (((ExprIdentStar ) leftExpr).getVarDeclaration() instanceof ParameterDec) ) {
				env.error(true,  leftExpr.getFirstSymbol(),
						"Attempt to assign a value to the parameter '" +
				          leftExpr.asString() + "'. Parameters are read-only in Cyan", leftExpr.asString(), ErrorKind.attempt_to_assign_a_value_to_a_readonly_variable);
			}
			...
			boolean leftHandSideIsValid = true;
       		...
       		if ( leftExpr instanceof ExprIdentStar ) {
       			final IdentStarKind kind = ((ExprIdentStar ) leftExpr).getIdentStarKind();
       			leftHandSideIsValid = kind == IdentStarKind.instance_variable_t || kind == IdentStarKind.variable_t;
       			/*
       			if ( kind == IdentStarKind.instance_variable_t && m.getReadOnly() ) {
       				if ( m.getMethodSignature() instanceof ast.MethodSignatureOperator ) {
           				env.error(leftExpr.getFirstSymbol(), "This is a read-only method. "
           						+ "fields cannot be changed. All methods whose names are operators (such as +) are read-only");
       				}
       				else {
           				env.error(leftExpr.getFirstSymbol(), "This is a read-only method. fields cannot be changed");
       				}
       			}
       			*/
       		}
       		...
       		if ( ! isInit && ! isInitShared ) {
       			/*
       			 * inside 'init' or 'init:' methods you can assign values to fields that are
       			 * declared with 'let'. Outside these methods you cannot.
       			 */
    			if ( leftExpr instanceof ExprIdentStar ) {
    				final ExprIdentStar varId = (ExprIdentStar ) leftExpr;
    				if ( varId.getVarDeclaration() != null && varId.getVarDeclaration().isReadonly()  ) {
    					final VariableDecInterface iv = varId.getVarDeclaration();
    					if ( iv instanceof StatementLocalVariableDec &&
    							((StatementLocalVariableDec ) iv).getCastLocalVariable() ) {

    						StatementLocalVariableDec svar = (StatementLocalVariableDec ) iv;
        					env.error(true,  leftExpr.getFirstSymbol(),
        							"Attempt to assign a value to the read only variable '" +
        					          iv.getName() + "'. A variable declared in a 'cast' statement, between 'cast' and '{', "
        					          		+ "is always read only", iv.getName(), ErrorKind.attempt_to_assign_a_value_to_a_readonly_variable);
    					}
    					else {
        					env.error(true,  leftExpr.getFirstSymbol(),
    				...
    				final FieldDec iv = ((ExprSelf__PeriodIdent ) leftExpr).getFieldDec();
    				if ( iv.isReadonly() ) {
    					env.error(true,  leftExpr.getFirstSymbol(),
    							"Attempt to assign a value to a read only variable. To make this variable writable, declare it as 'var " +
    					    iv.getType().getFullName() + " " + iv.getName() + "'", iv.getName(), ErrorKind.attempt_to_assign_a_value_to_a_readonly_variable);
    			...
    			else if ( leftExpr instanceof ExprSelfPeriodIdent && ! leftSide_IsFieldMissing ) {
    				final FieldDec iv = ((ExprSelfPeriodIdent ) leftExpr).getFieldDec();
    				if ( iv.isReadonly() ) {
    					env.error(true,  leftExpr.getFirstSymbol(),
    							"Attempt to assign a value to a read only variable. To make this variable writable, declare it as 'var " +
    					    iv.getType().getFullName() + " " + iv.getName() + "'", iv.getName(), ErrorKind.attempt_to_assign_a_value_to_a_readonly_variable);
...
//					leftTypeExpr.isSupertypeOf(rightTypeExpr, env);
				...
				if ( env.getProject().getCompilerManager().getCompilationStep() == CompilationStep.step_6 ) {
			   		MetaInfoServer.checkAssignmentPluggableTypeSystem(env, leftTypeExpr, leftExpr,
			   				LeftHandSideKind.LeftSideAssignment_LHS, rightTypeExpr, rightExpr);

			   		if ( leftExpr instanceof ExprIdentStar &&
			   				((ExprIdentStar ) leftExpr).getIdentStarKind() == IdentStarKind.instance_variable_t ) {
			   			// FieldDec iv = (FieldDec ) ((ExprIdentStar ) leftExpr).getVarDeclaration();
			   			StatementAssignmentList.replaceSetFieldAccess(env, leftExpr, rightExpr, this,
			   					(FieldDec ) ((ExprIdentStar ) leftExpr).getVarDeclaration() );
			   		}
			   		else if ( leftExpr instanceof ExprSelfPeriodIdent ) {
			   			// FieldDec iv = (FieldDec ) ((ExprIdentStar ) leftExpr).getVarDeclaration();
			   			StatementAssignmentList.replaceSetFieldAccess(env, leftExpr, rightExpr,
			   					this, ((ExprSelfPeriodIdent ) leftExpr).getFieldDec() );
			   		}
				...
				if ( env.getProject().getCompilerManager().getCompilationStep() == CompilationStep.step_6 ) {
					...
					if ( replaceSetFieldMissingAccess( env, (ExprSelfPeriodIdent ) leftExpr, rightExpr ) ) {
						/*
						 * field is virtual, therefore no other checks apply to it.
						 */
						continue;
					}

					leftExpr.calcInternalTypes(env, true);
					if ( ! leftExpr.getType().isSupertypeOf(rightTypeExpr, env) ) {
						env.error(true, exprList.get(i-1).getFirstSymbol(),
								"The type of the right-hand side of this assignment, " + rightTypeExpr.getFullName() +
								" is not a subtype of the left-hand side, " + leftExpr.getType().getFullName(), null, ErrorKind.type_error_type_of_right_hand_side_of_assignment_is_not_a_subtype_of_the_type_of_left_hand_side);
					}
			...
			if ( leftExpr instanceof ExprIdentStar && ((ExprIdentStar ) leftExpr).getIdentStarKind() == IdentStarKind.variable_t ) {
				final StatementLocalVariableDec varDec = (StatementLocalVariableDec ) ((ExprIdentStar ) leftExpr).getVarDeclaration();
			...
			if ( cyanMetaobject instanceof IActionFieldAccess_semAn
				...
				try {
					...
					if ( other == null ) {
						...
						sb = to.run(
								() -> {
									return 	access.semAn_replaceSetField(leftExpr.getI(), rightExpr.getI(), env.getI());
								},
...
//						sb = access.semAn_replaceSetField(leftExpr.getI(), rightExpr.getI(), env.getI());
						...
						sb = to.run(
								() -> {
									return 	new StringBuffer( ((_IActionFieldAccess__semAn ) other)._semAn__replaceSetField_3(
...
//								leftExpr.getI(), rightExpr.getI(), env.getI()).s);
					...
					env.errorInMetaobject(cyanMetaobject, object.getFirstSymbol());
				}
				...
				if ( sb != null && sb.length() != 0 ) {
					...
					if ( rightExpr.getNextSymbol() != null && rightExpr.getNextSymbol().token == Token.SEMICOLON ) {
						offsetNext = rightExpr.getNextSymbol().getOffset() + 1;
					}
					env.removeAddCodeFromToOffset(object,
							offsetNext,
							annot, sb, null);
			...
			if ( cyanMetaobject instanceof IActionFieldMissing_semAn
				...
				try {
					...
					if ( other == null ) {
						...
						sb = to.run(
								() -> {
									return access.semAn_replaceSetMissingField(leftExpr.getI(), rightExpr.getI(), env.getI());
								},
...
//					final Type codeType = env.searchPackagePrototype(t.f1, t.f2);
//					if ( codeType == null ) {
//						env.error(leftExpr.getFirstSymbol(), "field '" + leftExpr.getIdentSymbol().getSymbolString() +
...
//		        	try {
//
//						method = indexedExprValue.getClass().getMethod(NameServer.javaNameAtPutMethod,
//								new Class<?> [] { indexOfExprValue.getClass(), rightValue.getClass() } );
//			        	method.invoke(indexedExprValue, indexOfExprValue, rightValue);
//
//		        	}
//		        	catch ( IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException | SecurityException e ) {
//		        		ee.error(this.getFirstSymbol(), "Error when calling indexing method for assignment");
//		        	}

	    			boolean found = false;
	    			for (Method method : indexedValueClass.getMethods()) {
	    				if ( method.getName().equals(NameServer.javaNameAtPutMethod) ) {
	    					method.setAccessible(true);
	    					try {
	    						method.invoke(indexedExprValue, new Object[] { indexOfExprValue, rightValue } );
	    						found = true;
	    						break;
	    					}
	    			...
	    			if ( !found ) {
	    				ee.error(this.getFirstSymbol(), "Method '" + NameServer.javaNameAtPutMethod +
	    			...
	    			if ( !(indexOfExprValue instanceof Integer) ) {
	    				ee.error(this.getFirstSymbol(), "Attempt to index a Java array with an object whose class"
	    						+ " is not Integer or int. Its class is '" + indexOfExprValue.getClass().getName() + "'");
	    				return null;
	    			}
	    			Array.set(indexedValueClass, (Integer ) indexOfExprValue, rightValue);
	    		}
	        	...
	        	if ( leftSide instanceof ExprIdentStar ) {
	        		...
	        		// rightValue = Statement.castCyanJava(ee, leftSide.getClass(), rightValue, this.getFirstSymbol());

	        		// public static Object castCyanJava(EvalEnv ee, Class<?> leftSideClass, Object rightValue, Symbol symbolForError)
	        		Object leftSideObj = leftSide.eval(ee);
	        		if ( leftSideObj != null ) {
		        		rightValue = Statement.castCyanJava(ee, leftSideObj.getClass(), rightValue);
	        		}
	        		...
	        		if ( varI != null ) {
	        			...
	        			ee.error(getFirstSymbol(), "Variable '" + varName + "' was not found");
	        		}
	        		//((ExprIdentStar ) leftSide).getVarDeclaration().setValueInInterpreter(rightValue);
	        	}
	        	else {
	        		ee.error(leftSide.getFirstSymbol(), "Left-hand side of this assignment, '" + leftSide.asString() + "' is not assignable");
	        	}
