...
public class ExprMessageSendWithKeywordsToSuper extends ExprMessageSendWithKeywords {
	...
	public String genJavaExpr(PWInterface pw, Env env) {
		...
		Type returnType = methodSignatureForMessage.getReturnType(env);
		if ( env.getCurrentObjectDec().getOuterObject() != null ) {
			...
			if ( returnType == Type.Nil ) {
				pw.printlnIdent("_Nil " + resultTmpVar + " = _Nil.prototype;");
				javaCallToSuper = javaCallToSuper + "( ";
			}
			else {
				javaCallToSuper = NameServer.getNamePrivateMethodForSuperclassMethod(javaCallToSuper);
				...
				if ( methodSignatureForMessage == null) {
					...
					if ( aMethod != null ) {
						if ( aMethod.getShared() ) {
							...
							env.error(this.getFirstSymbol(), "Method '" + aMethod.getMethodSignature().getFullName(env) +
