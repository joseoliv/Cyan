...
//								"This type does not belong to the union '" + typeOfExpr.getFullName() + "'", true, false);
			...
			if ( ! caseRecord.statementList.alwaysReturn()  ) {
		 ...
		 * executed
		 */
		if ( caseElseClausesAreExhaustive() ) {
	...
	public void genJava(PWInterface pw, Env env) {
		...
		if ( this.isTaggedUnion ) {
			...
			*/
			...
			if ( isUnion ) {
				...
				pw.printlnIdent("if ( " + tmpExpr + " instanceof " + MetaHelper.AnyInJava + " &&"
						+ " (("  + MetaHelper.AnyInJava +  " ) " + tmpExpr + ").getUnionElem() != null ) { "
						);
				pw.add();
		...
		if ( (alwaysReturn() || caseElseClausesAreExhaustive()) && this.elseStatementList == null ) {
			pw.printlnIdent("else {");
			pw.add();
			pw.printlnIdent("return null;");
			pw.sub();
	...
	private boolean caseElseClausesAreExhaustiveAux() {
	...
	private boolean caseElseClausesAreExhaustive() {
		if ( clausesAreExhaustive == null ) {
			clausesAreExhaustive = this.caseElseClausesAreExhaustiveAux();
		}
