...
//      .]
		...
		if ( e instanceof WrExprLiteralTuple ) {
			...
			if ( ! array.get(0).asString().equals("initialState") ||
				 !array.get(2).asString().equals("finalStates")   ||
				 !array.get(4).asString().equals("states") ||
				 !array.get(6).asString().equals("transitions")) {
				tupleError();
				return ;
			}
			if ( ! (array.get(1) instanceof WrExprLiteralString) ) {
				tupleError();
			}
			fsmData.initialState = meta.MetaHelper.removeQuotes(
					((WrExprLiteralString ) array.get(1)).getStringJavaValue().toString());
			if ( !(array.get(3) instanceof WrExprLiteralArray) ) {
				tupleError();
			}
			WrExprLiteralArray la = (WrExprLiteralArray ) array.get(3);
			fsmData.finalStateList = new ArrayList<String>();
			for ( WrExpr wrExpr : la.getExprList() ) {
				fsmData.finalStateList.add(meta.MetaHelper.removeQuotes(wrExpr.asString()));
			}
			if ( !(array.get(5) instanceof WrExprLiteralArray) ) {
				tupleError();
			}
			fsmData.stateList = new ArrayList<>();
			la = (WrExprLiteralArray ) array.get(5);
			for ( WrExpr wrExpr : la.getExprList() ) {
				fsmData.stateList.add(new Tuple3<String, Integer, Integer>(
						meta.MetaHelper.removeQuotes(wrExpr.asString()), 0, 0));
			}

			fsmData.transList = new ArrayList<>();
			Object obj =  array.get(7);
			la = (WrExprLiteralArray ) array.get(7);
			for ( WrExpr wrExpr : la.getExprList() ) {
				if ( !(wrExpr instanceof WrExprLiteralTuple) ) {
					tupleError();
					return ;
				}
				WrExprLiteralTuple tt = (WrExprLiteralTuple ) wrExpr;
				List<WrExpr> tupleList = tt.getExprList();
				fsmData.transList.add(new Tuple3<String, String, String>(
						meta.MetaHelper.removeQuotes(tupleList.get(0).asString()),
						meta.MetaHelper.removeQuotes(tupleList.get(1).asString()),
						meta.MetaHelper.removeQuotes(tupleList.get(2).asString())));
			}
