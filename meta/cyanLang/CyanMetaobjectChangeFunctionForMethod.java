...
/**
 * This metaobject should only be attached to methods 'functionForMethod:1' and 'functionForMethodWithSelf:1' of
 * Any. It replaces method calls to these methods by an anonymous function. If the parameter of 'functionForMethod:1' is
 * a literal string which is the name of method 'm:1', then the code that replaces the method call is
...
public class CyanMetaobjectChangeFunctionForMethod extends CyanMetaobjectAtAnnot implements IActionMessageSend_semAn {
	...
	public Tuple3<StringBuffer, String, String> semAn_analyzeReplaceKeywordMessage(
		...
		if ( !(annotation.getDeclaration() instanceof WrMethodDec) ) { ok = false; }
			...
			final String attachedMethodName = ((WrMethodDec) annotation.getDeclaration()).getName();
		...
		final WrExpr paramExpr = message.getkeywordParameterList().get(0).getExprList().get(0);
		String methodName;
		if ( paramExpr instanceof WrExprLiteralString) {
			...
			this.addError(messageSendExpr.getFirstSymbol(), "The parameter to the method 'functionForMethod:' or 'functionForMethodWithSelf:' "
		...
		final List<WrMethodSignature> msList = receiverType.searchMethodPublicSuperPublicProto(methodName, env);
		if ( msList == null || msList.size() == 0 ) {
			this.addError(messageSendExpr.getFirstSymbol(), "Method '" + methodName + "' that is parameter to the message passing whose keyword is"
		...
		if ( ms instanceof WrMethodSignatureUnary ) {
			...
			for ( final WrMethodKeywordWithParameters sel : mss.getKeywordArray() ) {
				...
				for ( @SuppressWarnings("unused") final WrParameterDec param : sel.getParameterList() ) {
		...
		return new Tuple3<StringBuffer, String, String>(code, type.getPackageName(), type.getName());
