 ...
 *
   @author jose
 */
public class CyanMetaobjectInline extends CyanMetaobjectAtAnnot implements IActionMessageSend_dsa, IParseWithCyanCompiler_dpa {

	public CyanMetaobjectInline() {
		super("inline", AnnotationArgumentsKind.ZeroOrMoreParameters,
	...
	@Override
	public void check() {
		final List<Object> javaParamList = this.getMetaobjectAnnotation().getJavaParameterList();
	...
	public Tuple3<StringBuffer, String, String> dsa_analyzeReplaceKeywordMessage(
		...
		code.append(" { ");
		List<String> realParamStr = new ArrayList<>();
		for ( WrMessageKeywordWithRealParameters sel : message.getkeywordParameterList() ) {
			for ( Expr e : sel.getExprList() ) {
				String ps = meta.MetaHelper.nextLocalVariableName();
				code.append("let " + ps + " = " + e.asString() + "; ");
				realParamStr.add(ps);
			}
		}
		int size = realParamStr.size();
		if ( size != method.getMethodSignature().getParameterList().size() ) {
			WrSymbol sym = messageSendExpr.getFirstSymbol();
			this.addError("Something wrong: the number of parameters of the call of line " + sym.getLineNumber() +
					" and column " + sym.getColumnNumber() + " of prototype " + sym.getCompilationUnit().getPackageName() + "." +
					((CompilationUnit ) sym.getCompilationUnit()).getPublicPrototype()
					+ "is different from the number of method parameters, " + size);
		}
		int i = 0;
		for ( ParameterDec param : method.getMethodSignature().getParameterList() ) {
			s = s.replace(param.getName(), realParamStr.get(i));
			++i;
		}
		code.append( " ^" + s + " } eval ");
		*/
		//final WrType type = ((WrMethodDec) annotation.getDeclaration()).getMethodSignature().getReturnType(env);
	...
	public void dpa_parse(ICompiler_dpa compiler_dpa) {
		...
		if ( compiler_dpa.getSymbol().token != Token.EOLO ) {
			this.addError(compiler_dpa.getSymbol(), "A single expression was expected. After it, I found '"
