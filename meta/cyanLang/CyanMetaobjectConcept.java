...
import meta.ICompilerProgramUnitLater_dpa;
...
abstract class Node {
	 ...
	 * return false if there was an error
	   ...
	   @return
	 */
	abstract public List<Function0> check(WrAnnotationAt annotation, ICompiler_dsa compiler);
	...
	abstract public List<String> collectGPData_dsa(HashMap<String, ProtoInfo> map);
	/**
	 ...
	 *
	   ...
	   @return
	 */
	public List<String> collectGPData_dpa(HashMap<String, ProtoInfo> map) {
		return null;
	}
...
class IsNode extends Node {
	...
	@Override
	public List<Function0> check(WrAnnotationAt annotation, ICompiler_dsa compiler) {
	...
	@Override
	public List<String> collectGPData_dsa(HashMap<String, ProtoInfo> map) {
...
class TypeInNode extends Node {
	...
	public List<Function0>  check(WrAnnotationAt annotation, ICompiler_dsa compiler) {
		...
		final WrType t = typeOrSymbol.getType();
		if ( !(t.getInsideType() instanceof WrProgramUnit) ) {
			return createFunctionList(
					() -> { compiler.errorAtGenericPrototypeInstantiation(
							"A concept associated to generic prototype '" + compiler.getEnv().getCurrentProgramUnit().getFullName() +
							"' expected that '" +
...
class NewTypeNode extends Node {
	...
	public void calcInternalTypes(WrEnv env, HashSet<String> gpSet) {
		...
		final WrProgramUnit pu = env.getCurrentCompilationUnit().getPublicPrototype();
...
class SubprototypeNode extends Node {
	...
	public List<Function0> check(WrAnnotationAt annotation, ICompiler_dsa compiler) {
		...
		if ( !(supertype.getInsideType() instanceof WrProgramUnit) ) {
			return createFunctionList(
					() -> { compiler.errorAtGenericPrototypeInstantiation(
							"A concept associated to generic prototype '" + compiler.getEnv().getCurrentProgramUnit().getFullName() +
							"' expected that '" +
		...
		final WrProgramUnit subType = (WrProgramUnit ) t;
...
class SupertypeNode extends Node {
	...
	public List<Function0> check(WrAnnotationAt annotation, ICompiler_dsa compiler) {
		...
		if ( !(subType.getInsideType() instanceof WrProgramUnit) ) {
			return createFunctionList(
					() -> { compiler.errorAtGenericPrototypeInstantiation("'" + subType.getFullName() + "' is not a prototype"); } );
		}

		final WrProgramUnit superType = (WrProgramUnit ) t;
...
class ConceptFileNode extends Node {
	...
	public List<String> collectGPData_dsa(HashMap<String, ProtoInfo> map) {
		...
		if ( conceptFileNodeList != null ) {
			for ( final Node node : conceptFileNodeList ) {
				final List<String>  emList = node.collectGPData_dsa(map);
				if ( emList != null ) {
	...
	public Tuple2<StringBuffer, Integer> axiomTestCode_dsa(int numberNextMethod) {
		StringBuffer s = null;
		for ( final Node node : conceptFileNodeList ) {
			final Tuple2<StringBuffer, Integer> t = node.axiomTestCode_dsa(numberNextMethod);
			if ( t != null ) {
...
class TypeHasNode extends Node {
	...
	public List<Function0>  check(WrAnnotationAt annotation, ICompiler_dsa compiler) {
		...
		final WrProgramUnit pu = (WrProgramUnit ) t.getInsideType();
...
public class CyanMetaobjectConcept extends CyanMetaobjectAtAnnot
       implements IParseWithCyanCompiler_dpa,
       			  ICheckProgramUnit_bsa,
       			  IActionProgramUnitLater_dpa,
       			  IListAfter_afti {
	...
	public void loadConceptFile(ICompiler_dpa comp, ICompiler_dpa originalCompiler_dpa, ConceptFileNode conceptNode, String conceptPackageName) {
		...
		final Tuple5<FileError, char[], String, String, WrCyanPackage> t = originalCompiler_dpa.readTextFileFromPackage(
		...
		final ICompiler_dpa icomp = CompilerManager.getCompilerToInternalDSL(text,
				t.f3, t.f4, t.f5);
	 ...
	 * P(Int).cyan), returns true;
	   ...
	   @return
	 */
	private static boolean isInstantiatedGenericPrototype(ICompilerProgramUnitLater_dpa compiler_dpa) {
		final String filename = ((CompilerGenericProgramUnit_dpa) compiler_dpa).getCompilationUnit().getFilename();
	...
	public void dpa_parse(ICompiler_dpa compiler_dpa) {
		...
		nodeList = parseConceptSourceCode(compiler_dpa, compiler_dpa, compiler_dpa.getCompilationUnit().getCyanPackage().getPackageName());
	...
	public List<Node> parseConceptSourceCode(ICompiler_dpa compiler_dpa, ICompiler_dpa originalCompiler_dpa, String currentPackageName) {
		...
		while ( compiler_dpa.startType(compiler_dpa.getSymbol().token) || compiler_dpa.getSymbol().token == Token.NOT ) {
			final boolean precededByNot = compiler_dpa.getSymbol().token == Token.NOT;
			if ( precededByNot ) {
				compiler_dpa.next();
			}

			final WrExpr typeOrSymbol = compiler_dpa.type();

			final String gpName = typeOrSymbol.asString();
			final boolean isIdentifier = MetaLexer.isIdentifier(gpName);


			sym = compiler_dpa.getSymbol();
			final String symStr = compiler_dpa.getSymbol().getSymbolString();
			if ( symStr.equals("interface") ) {
				...
				final IsPrototypeNode node = new IsPrototypeNode();
				node.typeOrSymbol = typeOrSymbol;
				nodeListAux.add(node);
				compiler_dpa.next();
			}
				...
				final WrExpr aNewTypeExpr = compiler_dpa.type();
				...
				compiler_dpa.next();
				final WrExpr isTypeExpr = compiler_dpa.type();
				...
				if ( compiler_dpa.getSymbol().token != Token.IDENTCOLON ) {
					compiler_dpa.error(compiler_dpa.getSymbol(), "A keyword name such as 'equalityTest:' was expected");
				}
				...
				while ( compiler_dpa.getSymbol().token == Token.IDENT ||
					...
					final WrExpr ident = compiler_dpa.parseIdent();
				...
				final WrSymbol leftSymbol = compiler_dpa.getSymbol();
				final String leftSeq = compiler_dpa.getSymbol().getSymbolString();
				compiler_dpa.next();
				...
				*/
				...
				compiler_dpa.next();
				final WrExpr exprInterface = compiler_dpa.type();
				...
				if ( isSymbol ) {
					...
					while ( compiler_dpa.getSymbol().token == Token.IDENT) {
						...
						sym = compiler_dpa.getSymbol();
						final WrExprIdentStar id = compiler_dpa.parseSingleIdent();
						if ( id == null ) {
					...
					while ( compiler_dpa.startType(compiler_dpa.getSymbol().token) ) {
						...
						final WrExpr exprType = compiler_dpa.type();
				...
				compiler_dpa.next();
				final WrExpr subtype = compiler_dpa.type();
				...
				compiler_dpa.next();
				final WrExpr supertype = compiler_dpa.type();
				final SupertypeNode supertypeNode = new SupertypeNode();
				...
				while ( compiler_dpa.getSymbol().getSymbolString().equals("func") ) {
					...
					compiler_dpa.next();
					final WrMethodSignature ms = this.methodSignature(compiler_dpa);

					methodSignatureList.add(ms);

					if ( compiler_dpa.getSymbol().token == Token.LITERALSTRING ) {
						errorMessageList.add(MetaHelper.removeQuotes(compiler_dpa.getSymbol().getSymbolString()) );
						compiler_dpa.next();
						if ( compiler_dpa.getSymbol().token == Token.COMMA ) {
							compiler_dpa.next();
						}
			...
			if ( compiler_dpa.getSymbol().token == Token.LITERALSTRING ) {
				nodeListAux.get(nodeListAux.size()-1).errorMessage = MetaHelper.removeQuotes(compiler_dpa.getSymbol().getSymbolString());
				compiler_dpa.next();
				if ( compiler_dpa.getSymbol().token != Token.COMMA ) {
					if ( compiler_dpa.getSymbol().token != Token.EOLO ) {
						compiler_dpa.error(compiler_dpa.getSymbol(), "',' expected");
	...
	@Override
	public void bsa_checkProgramUnit(ICompiler_dsa compiler){
		// // List<Node> nodeList = (List<Node> ) this.metaobjectAnnotation.getInfo_dpa();
		...
		for ( final Node node : nodeList ) {
			functionList = node.check( (WrAnnotationAt ) this.metaobjectAnnotation, compiler);
			if ( functionList != null ) {
	...
	private void createTestCasesForGenericParameters(List<Node> nodeList1, ICompiler_afti compiler, String dirName) {

		final HashMap<String, ProtoInfo> map = new HashMap<>();

		final IDeclaration dec = this.getMetaobjectAnnotation().getDeclaration();
		final WrProgramUnit pu = (WrProgramUnit ) dec;
	...
	private boolean checkPrototypeInfo(ProtoInfo info, ICompiler_afti compiler,
		...
		if ( info.superprototypeList != null ) {
			for ( final String supertypeName : info.superprototypeList ) {  // just one for ObjectDec. It may be several for InterfaceDe
				...
				if ( supertypeInfo != null ) {
					...
					*/
					if ( pu instanceof WrProgramUnit && ! ((WrProgramUnit ) pu).isInterface() ) {
						final WrProgramUnit supertype = (WrProgramUnit ) pu;
						if ( supertype.getIsAbstract(env) ) {
							/**
							 * collect the signatures of all abstract methods
							 */
							WrProgramUnit current = supertype;
							while ( current != null ) {
						...
						for ( final WrProgramUnit superPU : supertype.getAllSuperPrototypes(env) ) {
							/*
					...
					else if ( pu instanceof WrProgramUnit && ((WrProgramUnit ) pu).isInterface() ) {
						// an interface may have several super-prototypes
						final WrProgramUnit superInter = (WrProgramUnit ) pu;
						for ( final WrMethodSignature ms : superInter.getMethodSignatureList(env) ) {
							info.allMethodSignatures.add(ms);
						}
						for ( final WrProgramUnit superPU : superInter.getAllSuperPrototypes(env) ) {
		...
		if ( info.interfaceImplementList != null ) {
			for ( final String interfaceName : info.interfaceImplementList ) {
				...
				if ( interInfo != null ) {
					...
					final WrProgramUnit inter = (WrProgramUnit ) pu;
					/*
					 ...
					 */
					for ( final WrProgramUnit superPU : inter.getAllSuperPrototypes(env) ) {
	...
	public StringBuffer createTestCase(ICompiler_afti compiler, ProtoInfo p, HashMap<String, ProtoInfo> map, String packageName1) {
	...
	private WrMethodSignature methodSignature(ICompiler_dpa compiler_dpa) {
		...
		final WrSymbol identSymbol = compiler_dpa.getSymbol();

		final boolean isInit = false;
		final boolean isInitOnce = false;
		String name = "";

		switch ( compiler_dpa.getSymbol().token ) {
		case IDENT:
			name = compiler_dpa.getSymbol().getSymbolString();
			methodSignature = new WrMethodSignatureUnary(compiler_dpa.getSymbol(), compiler_dpa.getCurrentMethod() );
		...
		case IDENTCOLON:
			...
			while ( compiler_dpa.getSymbol().token == Token.IDENTCOLON ) {
				final WrMethodKeywordWithParameters keywordWithParameters = new WrMethodKeywordWithParameters(compiler_dpa.getSymbol());
				// if ( name.length() > 0 )
				name = name +  compiler_dpa.getSymbol().getSymbolString();
				compiler_dpa.next();

				if ( compiler_dpa.getSymbol().token == Token.LEFTPAR || compiler_dpa.startType(compiler_dpa.getSymbol().token) ) {
					parameterDecList(compiler_dpa,
			...
			methodSignature = new WrMethodSignatureWithKeywords(keywordList, compiler_dpa.getCurrentMethod(), indexingMethod);
			if ( keywordList.size() == 1 ) {
				...
				if ( name.equals("init:") ) {
					if ( keywordList.get(0).getParameterList() == null || keywordList.get(0).getParameterList().size() == 0 ) {
						compiler_dpa.error(keywordList.get(0).getkeyword(),
				...
				for ( final WrMethodKeywordWithParameters sel : keywordList ) {
					if ( sel.getName().equals("init:") || sel.getName().equals("new:") )
						compiler_dpa.error(sel.getkeyword(),
		...
		default:
			...
			if ( compiler_dpa.isOperator(compiler_dpa.getSymbol().token) ) {
				...
				final WrSymbol operatorSymbol = compiler_dpa.getSymbol();
				final WrMethodSignatureOperator mso = new WrMethodSignatureOperator(compiler_dpa.getSymbol(),
						compiler_dpa.getCurrentMethod());
				methodSignature = mso;
				compiler_dpa.next();
				if ( compiler_dpa.startType(compiler_dpa.getSymbol().token) ||
					...
					WrExpr typeInDec = compiler_dpa.type();
					...
					if ( compiler_dpa.getSymbol().token != Token.IDENT ) {
						...
						parameterSymbol = (WrSymbolIdent ) compiler_dpa.getSymbol();
					...
					final WrParameterDec parameterDec =
							new WrParameterDec( parameterSymbol, typeInDec, compiler_dpa.getCurrentMethod());
	...
	private void parameterDecList(ICompiler_dpa compiler_dpa, List<WrParameterDec> parameterList) {
	...
	private static void paramDec(ICompiler_dpa compiler_dpa,
		...
		final WrParameterDec parameterDec = new WrParameterDec(
				(WrSymbolIdent ) variableSymbol, typeInDec, compiler_dpa.getCurrentMethod() );
	...
	private static String foundSuch(ICompiler_dpa compiler_dpa) {
		...
		if ( compiler_dpa.getSymbol() instanceof WrSymbolKeyword ) {
	...
	@Override
	public void dpa_actionProgramUnitLater(ICompilerProgramUnitLater_dpa compiler) {
			...
			if ( p.equals("test")  ) {
				if ( compiler.getParsingPhase() == ParsingPhase.dpaGeneric ) {
					...
					if ( ! isInstantiatedGenericPrototype(compiler)) {
						compiler.addToListAfter_afti(this);
					}
	...
	@Override
	public void after_afti_action(ICompiler_afti compiler) {
		// // List<Node> nodeList = (List<Node> ) this.metaobjectAnnotation.getInfo_dpa();


		final String filename = ((WrProgramUnit ) this.getMetaobjectAnnotation().getDeclaration())
	...
	private List<ProtoInfo> topologicalSortingProtoInfoList(HashMap<String, ProtoInfo> map, WrEnv env) {
    	...
    	final List<ProtoInfo> programUnitList = new ArrayList<>();
    	/**
         ...
         */
        for ( final ProtoInfo pu : programUnitList ) {
         ...
         */
        final List<ProtoInfo> sortedProgramUnit = new ArrayList<>();
