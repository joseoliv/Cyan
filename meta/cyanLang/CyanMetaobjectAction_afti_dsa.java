...
<pre><code>	IAction_dsa, IActionNewPrototypes_dsa,
	IAction_afti, IActionNewPrototypes_afti,
...
In this DSL, there may be field declarations and methods. Each method should correspond to a method of a Java interface implemented by this metaobject class. And it should not have parameters. For example, there is a method <code>afti_codeToAddCurrentPrototype</code> in the Java interface <code>meta.IAction_afti</code>. However, this method takes two arguments and in the Cyan code below it has none. To simplify the coding of metaobjects, the formal parameters of the methods are implicitly declared. In the code below we list which are the parameter names. Inside the methods, two variables can be used:  <code>metaobject</code>, which is the metaobject that received the message (it would be <code>self</code>) and <code>env</code>, the environment. For those interface methods that have a <code>compiler</code> parameter, there is a variable <code>compiler</code> available in the corresponding <em>Myan</em> method.</p>
...
<p>Each Cyan method can also return a value. For example, <code>afti_codeToAddCurrentPrototype</code> should return a value of type</p>
...
<pre><code>    @action_afti_dsa{*
        func dsa_codeToAdd {
            runFile: #printProtoData_dsa;
        }
    *}
</code></pre>
<p>There should be a file <code>printProtoData_dsa.myan</code> in the <code>--data</code> directory of the current package. To use this file in other package, it is necessary to precede it with the package name. Import declarations of the Cyan source code are not taken into consideration here. Parameters can be passed to the file:</p>
...
<p>The Cyan code would be able to visit methods and statements of a protototype, for example.</p>
   ...
   @author jose
 */
public class CyanMetaobjectAction_afti_dsa extends CyanMetaobjectAtAnnot
              implements IInterpreterMethods_afti {

	public CyanMetaobjectAction_afti_dsa() {
		super("action_afti_dsa", AnnotationArgumentsKind.ZeroParameters);
	}
