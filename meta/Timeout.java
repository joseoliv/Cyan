...
//				Class<?> builder = urlCl.loadClass("nu.xom.Builder"); //nu.xom.Builder
//				builder.newInstance();
//			}
			return toCall.call();
		}
			...
			errorMessage = "Exception " + e.getCause().getClass().getName() + " thrown ";
		}
		finally {
			if ( prevCl != null ) {
	...
	public T addPackageMetaToClassPath_and_Run(CyanPackage cyanPackage, Callable<T> toCall) {
	 ...
	 * either the project or the environment must be non-null
	   ...
	   @return
	 */
	public T run(Callable<T> toCall, int timeoutMilliSec, String metaobjectMethodName,
			CyanMetaobject metaobject, Project project) {
		...
		return runAux(toCall, timeoutMilliSec, metaobjectMethodName, metaobject);
	}

	public T run(Callable<T> toCall, int timeoutMilliSec, String metaobjectMethodName,
			CyanMetaobject metaobject, Env env) {
		if ( metaobject.getCyanPackage() == null ) {
			String cyanPackageNameMO = metaobject.getPackageName();
	...
	public T runAux(Callable<T> toCall, int timeoutMilliSec, String metaobjectMethodName,
			CyanMetaobject metaobject) {
		try {
			if ( timeoutMilliSec > 0 ) {
				TimeBomb timeBomb = new TimeBomb(timeoutMilliSec, metaobjectMethodName, metaobject);
				final T t;
				t = addPackageMetaToClassPath_and_Run( metaobject.getCyanPackage(), () -> {
					return toCall.call();
				    });
				timeBomb.stop();
				return t;
				...
				return addPackageMetaToClassPath_and_Run( metaobject.getCyanPackage(), () -> {
					return toCall.call();
			    });
			}
		}
	...
	/**
	 * Run toCall. If the execution does not end within timeoutMilliSec milliseconds,
	 * an error message, using parameter  metaobjectMethodName, is put in a field
	 * that can be retrieved with method getErrorMessage. This method returns
	 * the value returned by toCall.
	 *
	 */
	public T runNotUsed(Callable<T> toCall, int timeoutMilliSec, String metaobjectMethodName,
			CyanMetaobject metaobject) {
		errorMessage = null;
		...
		try {
			...
			errorMessage = "Metaobject method '" + metaobjectMethodName + "' of metaobject " +
	...
	static public int getTimeoutMilliseconds(Env env, WrProgram program,
		...
		if ( cpackage != null ) {
			timeoutMetaobjectsInteger = cpackage.getPackageValueFromKey(MetaHelper.timeoutMillisecondsMetaobjectsStr);
		}
		if ( timeoutMetaobjectsInteger != null ) {
			if ( ! (timeoutMetaobjectsInteger instanceof Integer) ) {
				env.error(annotSymbol, "The package key value " + MetaHelper.timeoutMillisecondsMetaobjectsStr +
			...
			if ( program != null ) {
				timeoutMetaobjectsInteger = program.getProgramValueFromKey(MetaHelper.timeoutMillisecondsMetaobjectsStr);
				if ( timeoutMetaobjectsInteger != null ) {
					if ( ! (timeoutMetaobjectsInteger instanceof Integer) ) {
	 ...
	 * 'annotSymbol' is the symbol used for error messages
	 */
	static public int getTimeoutMilliseconds(saci.Compiler compiler, Program program,
			CyanPackage cpackage, Symbol annotSymbol) {
		int timeoutMilliseconds = MetaHelper.timeoutMillisecondsMetaobjectsDefaultValue;
		Object timeoutMetaobjectsInteger = null;
		if ( cpackage != null ) {
			timeoutMetaobjectsInteger = cpackage.getPackageValueFromKey(MetaHelper.timeoutMillisecondsMetaobjectsStr);
		...
		if ( timeoutMetaobjectsInteger != null ) {
			if ( ! (timeoutMetaobjectsInteger instanceof Integer) ) {
				...
				compiler.error2(annotSymbol, "The package key value " + MetaHelper.timeoutMillisecondsMetaobjectsStr +
						", set with @options, has a type that is not 'Integer'");
				return MetaHelper.timeoutMillisecondsMetaobjectsDefaultValue;
			}
			timeoutMilliseconds = (Integer ) timeoutMetaobjectsInteger;
...
class MyRun implements Callable<Integer> {
	...
	public Integer call() throws Exception {
		try {
			String s = null;
			if ( 1 < 2 ) { s = ""; }
			if ( 1 < 2 ) { System.out.println(s.charAt(0)); }
