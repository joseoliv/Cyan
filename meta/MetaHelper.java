...
public class MetaHelper {
	...
	static {
		...
		MetaHelper.cyanJavaBasicTypeTable = new Hashtable<String, String>();
	...
	/**
	   ...
	   @param tupleSet
	 */
	public static _Set_LT_GP__Tuple_LT_GP_Object_GP_Object_GP_Object_GP_Object_GT__GT
			cyanSetTupleStringIntIntDyn_toJava(
		...
		_Set_LT_GP__Tuple_LT_GP_Object_GP_Object_GP_Object_GP_Object_GT__GT tupleSet =
				new _Set_LT_GP__Tuple_LT_GP_Object_GP_Object_GP_Object_GP_Object_GT__GT();

		for ( Tuple4<String, Integer, Integer, Object> elem : annotationInfoSet ) {
			tupleSet._add_1( new _Tuple_LT_GP_Object_GP_Object_GP_Object_GP_Object_GT(
	...
	/**
	   @param r
	 */
	public static List<Tuple2<String, String[]>> cyanTupleStringArrayString_tojava(_Array_LT_GP__Tuple_LT_GP_CyString_GP__Array_LT_GP_CyString_GT__GT__GT array) {
		int size = array._size().n;
		if ( size == 0 ) {
			return null;
		}
		else {
			List<Tuple2<String, String[]>> renameMethodList = new ArrayList<Tuple2<String, String[]>>();
	...
	public static List<Tuple2<String, StringBuffer>> cyanArrayTupleStringString_toJava(
		...
		int size = array._size().n;
		if ( size != 0 ) {
			statsList = new ArrayList<Tuple2<String, StringBuffer>>();
			for (int k = 0; k < size; ++k) {
				_Tuple_LT_GP_CyString_GP_CyString_GT t = array._at_1(new CyInt(k));
				statsList.add( new Tuple2<String, StringBuffer>(
						t._f1().s, new StringBuffer(t._f2().s)
						));
			}
		}
		return statsList;
	...
	public static List<Tuple3<String, StringBuffer, Boolean>> cyanArrayTupleStringStringBoolean_toJava(
		...
		int size = array._size().n;
		if ( size != 0 ) {
			statsList = new ArrayList<Tuple3<String, StringBuffer, Boolean>>();
			for (int k = 0; k < size; ++k) {
				_Tuple_LT_GP_CyString_GP_CyString_GP_CyBoolean_GT t = array._at_1(new CyInt(k));
				statsList.add( new Tuple3<String, StringBuffer, Boolean>(
						t._f1().s, new StringBuffer(t._f2().s), t._f3().b
						));
			}
		}
		return statsList;
	...
	static public Object interpreterFor_MOPInterfaceMethod(
			IAbstractCyanCompiler compiler,
			WrEnv wrEnv,
			InterpreterPrototype interpreterPrototype,
			CyanMetaobjectAtAnnot cyanMetaobject,
			String interfaceMethodName,
			String []varNameList, Object []objList,
			Class<?> returnTypeInterfaceMethod, String returnTypeInterfaceMethodName) {

		if ( interpreterPrototype == null ) { return null; }
		...
		try {
			WrEvalEnv ee = MetaHelper.getNewWrEvalEnv(wrEnv, interpreterPrototype, cyanMetaobject,
			...
			if ( obj != null && returnTypeInterfaceMethod != null &&
				...
				cyanMetaobject.addError("Code for '" + interfaceMethodName +
						"' should return an object of class '" + returnTypeInterfaceMethodName +
	...
	static public Object interpreterFor_MOPInterfaceMethod(
		...
		try {
			...
			if ( obj != null && returnTypeInterfaceMethod != null &&
					!(returnTypeInterfaceMethod.isAssignableFrom(obj.getClass())) ) {
				cyanMetaobject.addError("Code for '" + interfaceMethodName +
						"' should return an object of class '" + returnTypeInterfaceMethod.getName() +
						"'. It returned an object of class '" +
						obj.getClass().getName() + "'");
				return null;
			}
	...
	public static List<WrStatement> loadCyanStatementList(IAbstractCyanCompiler cp,
		...
		if ( t5 != null && t5.f1 == FileError.package_not_found ) {
			throw new InterpretationErrorException(
					"Cannot find package '" + packageName1 + "' used to load statements for the Cyan interpreter", null);
		}
		if ( t5 == null  || t5.f1 != FileError.ok_e ) {
			throw new InterpretationErrorException(
				"Cannot read concept from file '" + filename + "." + MetaHelper.extensionMyanFile  +
		        "' from package '" + packageName1 + "'. This file should be in directory '" + DirectoryKindPPP.DATA +
		        "' of the package directory", null
		         );
		}
		...
		try {
			...
			throw new InterpretationErrorException(
					"File '" + packageName1 + "." + filename + "' has Cyan statements that were loaded probably by " +
							"a message send 'self runFile: filename'. "
							+ "One of the statements has thrown an exception '" + e.getClass().getName() +
					"' whose message is '" + e.getMessage() + "'. ", null);
		}
		return statList;
	}
	...
	public static Tuple2<String, String> extractPackageName(String package_Name) {
		...
		if ( indexLastDot < 0 ) {
			return new Tuple2<String, String>(null, package_Name);
		}
		else {
			return new Tuple2<String, String>(
					package_Name.substring(0, indexLastDot),
	...
	public static Object createSelfObject(IAbstractCyanCompiler compiler,
			final CyanMetaobjectAtAnnot thisMetaobject, WrEvalEnv ee, WrEnv env) {
		return new Object() {
...
//			}
			...
			public Object runFile(String moFileName, String ...args) {
				...
				if ( t.f1 == null ) {
					...
					thisMetaobject.addError("The first parameter of 'run:' "
							+ "is not correct. "
							+ "The parameter is '" + moFileName + "'");
					throw new InterpretationErrorException("The first parameter of 'runFile:' "
							+ "is not correct. "
							+ "The parameter is '" + moFileName + "'", null);
				}
				List<String> paramNameList = new ArrayList<String>();
				for ( String elem : args ) {
					paramNameList.add(elem);
				}
				List<WrStatement> statList = loadCyanStatementList(compiler,
						t.f1, t.f2,
						paramNameList);
	...
	static public String getJavaNameOfMethodWith(String keyword1, int numParam1, String keyword2, int numParam2) {
		return getJavaNameOfkeyword(keyword1) + numParam1 +
				getJavaNameOfkeyword(keyword2) + numParam2;
	}
	/**
	...
	static public String getJavaNameOfMethod(String []keywordList, int []numParamList) {
		...
		for ( String keyword : keywordList ) {
			javaName = javaName + getJavaNameOfkeyword(keyword) + "_" + numParamList[i];
			++i;
		}
		return javaName;
	}
	...
	static public String getJavaNameOfkeyword(String symbolString) {
		...
		String alpha = alphaName(symbolString);
		if ( alpha != null ) {
			return alpha;
			/*
	...
	public static ICompiler_parsing createNewCompiler_parsing(String cyanCode) {
		...
		*/
    	Project project = new Project( program, null, null,
		...
		final CyanPackage cyanPackage = new CyanPackage(program, "anonymousPackage",
				 project, "anonymousPackage", null, null, null);
	...
	static public WrEvalEnv getNewWrEvalEnv(WrEnv wrEnv,
			...
			WrSymbol symForError,
            String []varNameList, Object []objList, CyanMetaobject cyanMetaobject)
            {

    	WrEvalEnv ee = getNewWrEvalEnv(wrEnv, selfObject, symForError,
	...
	public static WrEvalEnv getNewWrEvalEnv(WrEnv wrEnv, Object selfObject,
		...
		if ( varNameList != null && objList != null ) {
			if ( varNameList.length != objList.length ) {
				wrEnv.error(symForError,
	...
	static public Tuple2<String, InterpreterPrototype> parseCyanCode(ICompiler_parsing cp) {
		...
		while ( cp.getSymbol().token != Token.EOLO ) {
			...
			if ( sym.token == Token.IMPORT ) {
				...
				if ( cp.getSymbol().token != Token.IDENT ) {
					...
					if ( importPackage.getName().startsWith(MetaHelper.cyanLanguagePackageName) ) {
						if ( importPackage.getName().equals(MetaHelper.cyanLanguagePackageName) ) {
							cp.error(importPackage.getFirstSymbol(), "Package 'cyan.lang' is automatically imported. It cannot be imported by the user");
						}
						else {
				...
				if ( cp.getSymbol().token == Token.IDENT ) {
					...
					if ( ! (typeVar instanceof WrExprIdentStar) ) {
						Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
								"In line " + cp.getSymbol().getLineNumber() + ", after 'var' it was expected something like "
				...
				Object ins;
				Ref<String> errMsg = new Ref<String>();
				try {
				...
				if ( cp.getSymbol().token != Token.SEMICOLON ) {
					Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
							"In line " + cp.getSymbol().getLineNumber() +
							   ", ';' expected after variable declaration", null);
				...
				if ( cp.getSymbol().token != Token.IDENT ) {
					Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
							"In line " + cp.getSymbol().getLineNumber() +
							", identifier expected after 'func'", null);
				...
				if ( cp.getSymbol().token != Token.LEFTCB ) {
					Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
							"In line " + cp.getSymbol().getLineNumber() +
							", '{' expected after the method name", null);
				...
				if ( cp.getSymbol().token != Token.RIGHTCB ) {
					Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
							"In line " + cp.getSymbol().getLineNumber() +
							", '}' expected after method body", null);
				...
				Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
						"In line " + cp.getSymbol().getLineNumber() +
						", 'func' or 'var' was expected instead of symbol '" + sym.getSymbolString() + "'", null);
				return t;
			}
		...
		return new Tuple2<String, InterpreterPrototype>(null, ip);
	}
		...
		if ( statList != null && statList.size() > 0 ) {
			for ( WrStatement is : statList ) {
				try {
					...
					ee.error(sym,
							   "Exception '" + e.getClass().getCanonicalName() + "' was thrown. Its message is '"
							   + e.getMessage() + "'.");
				}
