...
public class MetaHelper {
	...
	public static _Set_LT_GP__Tuple_LT_GP_Object_GP_Object_GP_Object_GP_Object_GT_GT cyanSetTupleStringIntIntDyn_toJava(
		...
		for (Tuple4<String, Integer, Integer, Object> elem : annotationInfoSet) {
			tupleSet._add_1(
					new _Tuple_LT_GP_Object_GP_Object_GP_Object_GP_Object_GT(
							new CyString(elem.f1), new CyInt(elem.f2),
		...
		if ( size != 0 ) {
			...
			for (int k = 0; k < size; ++k) {
				_Tuple_LT_GP_CyString_GP_CyString_GT t = array
						._at_1(new CyInt(k));
				statsList.add(
						new Tuple2<>(t._f1().s, new StringBuffer(t._f2().s)));
			}
		}
		return statsList;
	}
	...
	public static List<Tuple3<String, StringBuffer, Boolean>> cyanArrayTupleStringStringBoolean_toJava(
		...
		if ( size != 0 ) {
			statsList = new ArrayList<>();
			for (int k = 0; k < size; ++k) {
				_Tuple_LT_GP_CyString_GP_CyString_GP_CyBoolean_GT t = array
						._at_1(new CyInt(k));
				statsList.add(new Tuple3<>(t._f1().s,
						new StringBuffer(t._f2().s), t._f3().b));
			}
		}
		return statsList;
	}
	...
	/**
	 * interpreterPrototype should have a (key, value) pair whose key is
	 * interfaceMethodName. If it does, value is a statement list that is
	 * evaluated (interpreted). Errors are directed to cyanMetaobject.
	 * varNameList and objList are names and values of variables injected in the
	 * Cyan interpreter. If returnTypeInterfaceMethod is not null, the return
	 * value returned by the interpretation should be assignable to
	 * returnTypeInterfaceMethod. If returnTypeInterfaceMethodInner1 is not
	 * null, the object returned should be an object of<br>
	...
	static public Object interpreterFor_MOPInterfaceMethod(
			IAbstractCyanCompiler compiler, WrEnv wrEnv,
			InterpreterPrototype interpreterPrototype,
			CyanMetaobjectAtAnnot cyanMetaobject, String interfaceMethodName,
		...
		List<WrStatement> statList = interpreterPrototype.mapMethodName_Body
				.get(interfaceMethodName);
		...
		try {
			WrEvalEnv ee = MetaHelper.getNewWrEvalEnv(wrEnv,
					interpreterPrototype, cyanMetaobject, fs.getFirstSymbol(),
					varNameList, objList, cyanMetaobject);

			ee.addVariable("metaobject", cyanMetaobject);
			ee.addVariable("env", wrEnv);

			Object selfObject = MetaHelper.createSelfObject(compiler,
					cyanMetaobject, ee, wrEnv);
			...
			if ( obj != null && returnTypeInterfaceMethod != null
					&& !(returnTypeInterfaceMethod
							.isAssignableFrom(obj.getClass())) ) {
				cyanMetaobject.addError("Code for '" + interfaceMethodName
						+ "' should return an object of class '"
						+ returnTypeInterfaceMethodName
			...
			cyanMetaobject.addError(
					"Internal error in method MetaHelper::interpreterFor_MOPInterfaceMethod");
		}
		return null;
	}

	/**
	 * interpret 'statList' and returns what it returns with a 'return'
	 * statement.
	 *
	 * @param statList
	 ...
	 */
	static public Object interpreterFor_MOPInterfaceMethod(
			List<WrStatement> statList, ICompiler_semAn compiler_semAn,
			CyanMetaobjectAtAnnot cyanMetaobject, String interfaceMethodName,
			String[] varNameList, Object[] objList,
			Class<?> returnTypeInterfaceMethod) {
		...
		try {
			...
			WrEvalEnv ee = MetaHelper.getNewWrEvalEnv(env, null,
					fs.getFirstSymbol(), varNameList, objList, cyanMetaobject);
			ee.addVariable("metaobject", cyanMetaobject);
			ee.addVariable("env", compiler_semAn.getEnv());
			Object selfObject = MetaHelper.createSelfObject(compiler_semAn,
					cyanMetaobject, ee, compiler_semAn.getEnv());
			...
			if ( obj != null && returnTypeInterfaceMethod != null
				...
				cyanMetaobject.addError("Code for '" + interfaceMethodName
						+ "' should return an object of class '"
						+ returnTypeInterfaceMethod.getName()
						+ "'. It returned an object of class '"
						+ obj.getClass().getName() + "'");
				return null;
			}
	...
	static public boolean interpreterFor_runUntilFixedPoint(
			InterpreterPrototype interpreterPrototype,
		...
		List<WrStatement> statList = interpreterPrototype.mapMethodName_Body
				.get("runUntilFixedPoint");
		if ( statList == null || statList.size() == 0 ) {
			return false;
		}
		if ( statList.size() != 1
				|| !(statList.get(0) instanceof WrStatementReturn) ) {
			cyanMetaobject.addError(
					"Code for 'runUntilFixedPoint' should be either 'return true;' or 'return false;'");
			return false;
		}
	...
	public static List<WrStatement> loadCyanStatementList(
			IAbstractCyanCompiler cp, String packageName1, String filename,
			List<String> paramNameList) {

		final Tuple5<FileError, char[], String, String, WrCyanPackage> t5 = cp
				.readTextFileFromPackage(filename, MetaHelper.extensionMyanFile,
						packageName1, DirectoryKindPPP.DATA,
						paramNameList == null ? 0 : paramNameList.size(),
						paramNameList);

		if ( t5 != null && t5.f1 == FileError.package_not_found ) {
			throw new InterpretationErrorException("Cannot find package '"
					+ packageName1
					+ "' used to load statements for the Cyan interpreter",
					null);
		}
		if ( t5 == null || t5.f1 != FileError.ok_e ) {
			throw new InterpretationErrorException(
					"Cannot read concept from file '" + filename + "."
							+ MetaHelper.extensionMyanFile + "' from package '"
							+ packageName1
							+ "'. This file should be in directory '"
							+ DirectoryKindPPP.DATA
		...
		final ICompiler_parsing newCompiler_parsing = MetaHelper
				.createNewCompiler_parsing(cyanCode);
		...
		try {
			...
			List<WrUnitError> wrUnitErrorList = newCompiler_parsing
					.getCompilationUnit().getErrorList();
			if ( wrUnitErrorList != null && wrUnitErrorList.size() > 0 ) {
				// only first error
				throw new InterpretationErrorException(
						wrUnitErrorList.get(0).getMessage(), null);
			}
		}
		catch (Throwable e) {
			throw new InterpretationErrorException("File '" + packageName1 + "."
					+ filename
					+ "' has Cyan statements that were loaded probably by "
					+ "a message send 'self runFile: filename'. "
					+ "One of the statements has thrown an exception '"
					+ e.getClass().getName() + "' whose message is '"
					+ e.getMessage() + "'. ", null);
		}
		return statList;
	}

	public static Tuple2<String, String> extractPackageName(
			String package_Name) {
	...
	/**
	 * Create a new object with 'call' methods with one, two, three, and four
	 * parameter. All of them have type java.lang.String. The first one is the
	 * name of the action function (metaobject) to be called. The others are
	 * parameters to the action function.
	 *
	 * @param compiler
	 ...
	 * @return
	 */
	public static Object createSelfObject(IAbstractCyanCompiler compiler,
			final CyanMetaobjectAtAnnot thisMetaobject, WrEvalEnv ee,
			WrEnv env) {
		return new Object() {

			// @SuppressWarnings("unused")
			// public Object call(cyan.lang.CyString actionFunctionName, Object
			// ...args) {
			...
			public Object callArgs(String actionFunctionName, Object... args) {
				final IActionFunction actionFunction = env
						.searchActionFunction(actionFunctionName);
				if ( actionFunction == null ) {
					thisMetaobject.addError("Function metaobject '"
							+ actionFunctionName + "' was not found. "
							+ "If it is a file of a directory '"
							+ meta.DirectoryKindPPP.DATA.toString()
				...
				Object obj = actionFunction.eval(new Tuple6<>(compiler,
						thisMetaobject, paramList,
			...
			public Object runFile(String moFileName, String p1, String p2,
					String p3, String p4) {
				return this.runFile(moFileName,
			...
			public Object runFile(String moFileName, String p1, String p2,
					...
					String p8, String p9) {
				return this.runFile(moFileName,
						new String[] { p1, p2, p3, p4, p5, p6, p7, p8, p9 });
			}
			...
			public Object runFile(String moFileName, String... args) {
				Tuple2<String, String> t = MetaHelper
						.extractPackageName(moFileName);
				if ( t.f1 == null ) {
					t.f1 = ee.getCompilationUnit().getPackageName();
				}
				else if ( t.f2 == null ) {
					thisMetaobject.addError("The first parameter of 'run:' "
							+ "is not correct. " + "The parameter is '"
							+ moFileName + "'");
					throw new InterpretationErrorException(
							"The first parameter of 'runFile:' "
									+ "is not correct. " + "The parameter is '"
									+ moFileName + "'",
							null);
				}
				...
				List<WrStatement> statList = MetaHelper.loadCyanStatementList(
						compiler, t.f1, t.f2, paramNameList);
	...
	/**
	 * return the Java name of the method whose keywords are 'keyword1' and
	 * 'keyword2' with numParam1 and numParam2 parameters
	 */
	static public String getJavaNameOfMethodWith(String keyword1, int numParam1,
			String keyword2, int numParam2) {
		return MetaHelper.getJavaNameOfkeyword(keyword1) + numParam1
				+ MetaHelper.getJavaNameOfkeyword(keyword2) + numParam2;
	}

	/**
	 * return the Java name of the method whose keyword is 'keyword1' with
	 * numParam1 parameters
	 */
	static public String getJavaNameOfMethodWith(String keyword1,
			int numParam1) {
		return MetaHelper.getJavaNameOfkeyword(keyword1) + numParam1;
	}

	/**
	 * return the Java name of the method whose keywords are given in the list
	 * keywordList and whose number of parameters of each keyword is given by
	 * numParamList
	 */
	static public String getJavaNameOfMethod(String[] keywordList,
			int[] numParamList) {
		StringBuilder javaName = new StringBuilder();
		int i = 0;
		for (String keyword : keywordList) {
			javaName.append(MetaHelper.getJavaNameOfkeyword(keyword))
					.append("_").append(numParamList[i]);
			++i;
		}
	...
	/**
	 * get the Java name corresponding to this keyword. It is equal to
	...
	static public String getJavaNameOfkeyword(String symbolString) {
		...
		if ( alpha != null ) {
			return alpha;
			/*
			 * int size = symbolString.length(); alpha = ""; for (int i = 0; i <
			 * size; i++) { String s = symbolToAlpha.get("" +
			 * symbolString.charAt(i)); if ( s == null ) return null; alpha =
			 * alpha + "_" + s; } return alpha;
			 */
		}
		else {
	...
	/**
	 * return the alphanumeric name of a method composed of symbols. That is, if
	 ...
	 * @return
	 *
	 *         public static String getJavaNameFromCyanNameWithoutPackage(String
	 *         cyanName) { char ch; String innerProtoName = ""; String javaName
	 *         = ""; for (int n = 0; n < cyanName.length(); ++n) { ch =
	 *         cyanName.charAt(n); if ( Character.isWhitespace(ch) ) continue;
	 *         if ( Character.isAlphabetic(ch) || Character.isDigit(ch) ) {
	 *         innerProtoName += ch; } else if ( ch == '_' ) { innerProtoName +=
	 *         "__"; } else if ( ch == '<' ) { //++insideGPI; javaName +=
	 *         MetaHelper.removeCyanLangChange(innerProtoName) + "_LT_GP_";
	 *         innerProtoName = ""; } else if ( ch == ',' ) { javaName +=
	 *         MetaHelper.removeCyanLangChange(innerProtoName) + "_GP_";
	 *         innerProtoName = ""; } else if ( ch == '>' ) { javaName +=
	 *         MetaHelper.removeCyanLangChange(innerProtoName) + "_GT";
	 *         innerProtoName = ""; //--insideGPI; } else if ( ch == '.' ){
	 *         innerProtoName += "_p_"; } else if ( ch == '|') {
	 *         NameServer.print("" + 0/0); } else { String s =
	 *         MetaHelper.alphaName(cyanName); if ( s != null ) { return s; } }
	 *
	 *
	 *         } if ( innerProtoName.length() > 0 ) javaName +=
	 *         MetaHelper.removeCyanLangChange(innerProtoName);
	 *
	 *         return javaName; }
	 *
	 *
	 *         public static String
	 *         new_getJavaNameFromCyanNameWithoutPackage(String cyanName) { char
	 *         ch; String innerProtoName = ""; String javaName = ""; for (int n
	 *         = 0; n < cyanName.length(); ++n) { ch = cyanName.charAt(n); if (
	 *         Character.isWhitespace(ch) ) continue; if (
	 ...
	 *         MetaHelper.removeCyanLangChange(innerProtoName) + "_GT";
	 *         innerProtoName = ""; //--insideGPI; } else if ( ch == '.' ){
	 *         innerProtoName += "_p_"; } else { String s =
	 *         MetaHelper.alphaName(cyanName); if ( s != null ) { return s; } }
	 *
	 *
	 *         } if ( innerProtoName.length() > 0 ) javaName +=
	...
	static public Tuple2<String, String> getJavaNameTuple(String cyanName) {
		...
		while (k < size) {
			...
			if ( ch == '.' ) {
				...
				if ( lastDotIndex > 0 ) {
					...
					String javaBasicTypeName = MetaHelper.cyanJavaBasicTypeTable
							.get(protoName);
					if ( javaBasicTypeName != null ) {
		...
		// return new Tuple2<String, String>(packageName,
		// MetaHelper.getJavaNameFromCyanNameWithoutPackage(cyanName));
		return new Tuple2<>(packageName,
				CyanTypeCompiler.cyanType_noPackage_ToJavaName(cyanName)
		// MetaHelper.getJavaNameFromCyanNameWithoutPackage(cyanName)
		);
	...
	private static String setCyanLangDir() {
		if ( MetaHelper.cyanLangDir == null ) {
			...
			if ( MetaHelper.cyanLangDir == null ) {
				return "In order to interpret Cyan code at "
						+ "runtime it is necessary to set the "
			...
			if ( !f.exists() || !f.isDirectory() ) {
				return "Directory '" + MetaHelper.cyanLangDir
						+ "' does not exist. '" + MetaHelper.cyanLangDir
	...
	/**
	 * using Cyan syntax, return either [. null, map .] or [. errMessage, null
	 * .]
	 *
	 * errMessage is an error message. map is composed of method names and
	 ...
	 * @return
	 */
	static public Tuple2<String, InterpreterPrototype> getMapMethodName_Body(
			String cyanCode) {

		final ICompiler_parsing cp = MetaHelper
				.createNewCompiler_parsing(cyanCode);
		final Tuple2<String, InterpreterPrototype> err_proto = MetaHelper
				.parseCyanCode(cp);
		return err_proto;
	...
	public static ICompiler_parsing createNewCompiler_parsing(String cyanCode) {
		...
		/*
		 * project = new Project( program, String mainPackage, String
		 * mainObject, List<String> authorArray, List<String> cyanPathArray,
		 * List<String> importList, String projectCanonicalPath, String
		 * execFileName )
		 */
		Project project = new Project(program, null, null, null, null, null,
				null, null);
		...
		final CyanPackage cyanPackage = new CyanPackage(program,
				"anonymousPackage", project, "anonymousPackage", null, null,
				null);
		...
		final ICompiler_parsing cp = CompilerManager.getCompilerToInternalDSL(
				text, "anonymous source code",
				"anonymousPackage\\anonymous source code", cyanPackage.getI());
		return cp;
	}

	static public WrEvalEnv getNewWrEvalEnv(WrEnv wrEnv,
			InterpreterPrototype interpreterPrototype, Object selfObject,
			WrSymbol symForError, String[] varNameList, Object[] objList,
			CyanMetaobject cyanMetaobject) {

		WrEvalEnv ee = MetaHelper.getNewWrEvalEnv(wrEnv, selfObject,
				symForError, varNameList, objList, cyanMetaobject);
		if ( ee != null ) {
	 ...
	 */
	public static WrEvalEnv getNewWrEvalEnv(WrEnv wrEnv, Object selfObject,
			WrSymbol symForError, String[] varNameList, Object[] objList,
			CyanMetaobject cyanMetaobject) {
		...
		if ( (varNameList == null && objList != null)
				|| (varNameList != null && objList == null) ) {
		...
		if ( (varNameList != null && objList != null)
				&& (varNameList.length != objList.length) ) {
			wrEnv.error(symForError,
					"The number of variables passed as parameters in the interpreter "
	 ...
	 * {@link meta.CyanMetaobjectAction_afterResTypes_semAn} relating the
	 * contents of files with extension <code>mo</code> The compiler passed as
	 * parameter here is supposed to refer to a text with the same contents as a
	 * <code>mo</code> file. Using Cyan syntax, this method returns either [.
	 * null, proto .] or [. errMessage, null .]
	 *
	 * in which <code>proto</proto> represents a Cyan prototype for the
	 * interpreter.
	 */

	static public Tuple2<String, InterpreterPrototype> parseCyanCode(
			ICompiler_parsing cp) {
		...
		while (cp.getSymbol().token != Token.EOLO) {
			...
			if ( sym.token == Token.IMPORT ) {
				...
				if ( cp.getSymbol().token != Token.IDENT ) {
					...
					importList
							.add((new StatementImport(
									(ExprIdentStar) meta.GetHiddenItem
											.getHiddenExpr(importPackage),
									null)).getI());
				}
					...
					if ( !(typeVar instanceof WrExprIdentStar) ) {
						Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
								"In line " + cp.getSymbol().getLineNumber()
										+ ", after 'var' it was expected something like "
										+ "'Type id' or 'Type id = expr' or 'id = expr'",
								null);
				...
				try {
					...
					Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
							"When interpreting Cyan code, failed to create a String",
							null);
				...
				if ( cp.getSymbol().token != Token.IDENT ) {
					Tuple2<String, InterpreterPrototype> t = new Tuple2<>(
							"In line " + cp.getSymbol().getLineNumber()
									+ ", identifier expected after 'func'",
							null);
				...
				cp.next();
				while (cp.getSymbol().token != Token.EOLO
						&& cp.getSymbol().token != Token.RIGHTCB) {
					...
					if ( lastStat.demandSemicolon() ) {
						if ( cp.getSymbol().token == Token.SEMICOLON ) {
							...
							if ( sym.token == Token.EOF
									|| sym.token == Token.EOLO ) {
								sym = lastStat.getFirstSymbol();
							}
							throw new CompileErrorException(
									"Error in line " + symb.getLineNumber()
											+ "(" + symb.getColumnNumber()
											+ "): ';' expected");
						}
					}
				}
				...
				Tuple2<String, InterpreterPrototype> t = new Tuple2<>("In line "
						+ cp.getSymbol().getLineNumber()
						+ ", 'func' or 'var' was expected instead of symbol '"
						+ sym.getSymbolString() + "'", null);
				return t;
			}
	...
	public static List<WrStatement> parseCyanStatementList(
			ICompiler_parsing cp) {
		List<WrStatement> statList = new ArrayList<>();
		while (cp.getSymbol().token == Token.IMPORT) {
			...
			if ( cp.getSymbol().token != Token.IDENT ) {
				...
				statList.add(
						(new StatementImport((ExprIdentStar) meta.GetHiddenItem
								.getHiddenExpr(importPackage), null)).getI());
			}
			if ( cp.getSymbol().token == Token.SEMICOLON ) {
				cp.next();
			}
	...
	/**
	 * return true if the prototype name is one of the basic types or Array or
	 * Tuple
	 */
	public static boolean isSelectedCyanLangProtototype(String prototypeName) {
		if ( NameServer.isBasicPrototype_andString(prototypeName) ) {
			...
			return prototypeName.equals("Array")
					|| prototypeName.equals("Tuple");
		}
	}
	...
	public static String					cyanLanguagePackageDirectory				= "cyan"
			+ File.separator + "lang";
	public static final int					sizeCyanLanguagePackageName_p_Dot			= NameServer.cyanLanguagePackageName_p_Dot
			.length();
	...
	// the extension of Pyan source files should be "pyan" as in
	// "myProject.pyan"
	...
	static final public int					sizeCyanSourceFileExtensionPlusOne			= MetaHelper.cyanSourceFileExtension
			.length() + 1;
	/**
	 * name of type DYN
	 */
	...
	static final public String				javaName_assign								= MetaHelper
			.getJavaNameOfMethodWith("assign:", 1);
	// "nil" in Java
	public static final String				NilInJava									= MetaHelper
			.getJavaName("Nil");
	public static final String				BooleanInJava								= MetaHelper
			.getJavaName("Boolean");
	public static final String				CharInJava									= MetaHelper
			.getJavaName("Char");
	public static final String				ByteInJava									= MetaHelper
			.getJavaName("Byte");
	public static final String				ShortInJava									= MetaHelper
			.getJavaName("Short");
	public static final String				IntInJava									= MetaHelper
			.getJavaName("Int");
	public static final String				LongInJava									= MetaHelper
			.getJavaName("Long");
	public static final String				FloatInJava									= MetaHelper
			.getJavaName("Float");
	public static final String				DoubleInJava								= MetaHelper
			.getJavaName("Double");
	public static final String				StringInJava								= MetaHelper
			.getJavaName("String");
	// public static final String CySymbolInJava = getJavaName("CySymbol");
	public static final String				AnyInJava									= MetaHelper
			.getJavaName("Any");
	static final public String				javaName_eq									= MetaHelper
			.getJavaNameOfMethodWith("eq:", 1);
	...
	/*
	 * true if the current program is being debugged
	 */
